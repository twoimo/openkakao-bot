...
`/Applications/OpenKakao Jarvis.app`이 메뉴바 앱으로 실행되며, 패널에는 골드 코어만 보입니다. 방 선택, 쉬운 답변 모드, 음성 시작, 대화 검색, 최근 답변은 메뉴바 트레이 아이콘을 우클릭해 여는 960×880 설정 창에서 다룹니다. 넓은 화면은 황금비에 가까운 61.8:38.2 두 열과 16px 간격을 쓰고, 보조 열은 최소 300px로 유지합니다. 800px 아래에서는 한 열로 바뀝니다. 기술 진단·권한·대량 점검 화면은 두지 않습니다. 기존 Swift Extra는 설치 시 백업 후 비활성화됩니다.

    ...
    *,
    ...
    if len(chat_selectors) > 1:
        raise SystemExit(
            ...
            "[auto_reply].chats and omit --chat"
        )
...
def _rotate_oversized_logs(state_root) -> None:
    """Keep long-running session logs from growing without bound."""
    if state_root is None:
        return
    try:
        candidates = list(Path(state_root).rglob("*.log"))
    except OSError:
        return
...
def main() -> int:
    ...
    _rotate_oversized_logs(args.state_root)

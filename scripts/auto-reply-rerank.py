...
def main() -> int:
    ranker = None
    # Emit a protocol line before FlagReranker import/load. The worker treats a
    # missing first line as `warming` and otherwise blocks 90s on a hung load.
    sys.stdout.write(json.dumps(_fail("warming")) + "\n")
    sys.stdout.flush()
    try:
        ranker = _load_ranker()

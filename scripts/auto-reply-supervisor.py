#!/usr/bin/env python3
"""Run the database-authoritative AutoReply children behind one readiness fence."""
...
def _toml_section(value: dict[str, Any], *names: str) -> dict[str, Any] | None:
    for name in names:
...
LOG_DIR = Path(
    os.environ.get(
        ...
        os.environ.get(
            ...
            str(Path.home() / "Library/Application Support/openkakao/auto-reply"),
        ),
    )
)
CONFIG_PATH = Path(
...
def read_attested_config() -> tuple[str, dict[str, Any]]:
    ...
    if os.environ.get("OPENKAKAO_AUTO_REPLY_CLI") == "1" and (
        ...
        raise SystemExit("AutoReply config attestation or parsing failed")
...
def auto_reply_config(config: dict[str, Any]) -> tuple[bool, str]:
    ...
    opted_in = isinstance(safety, dict) and (
        safety.get("allow_auto_reply") is True
        or safety.get("allow_bujamentor_auto_reply") is True
    )
    if not opted_in:
...
def _geeknews_only_room() -> bool:
    """긱뉴스만 켠 방인지 카탈로그에서 확인한다.

    그런 방은 답장을 보내지 않으므로 DB 감시자(계정당 하나)를 요구하지 않는다.
    """
    raw = target_chat_id_config()
    if not raw:
        return False
    chat_id = _positive_int(raw)
    if chat_id is None:
        return False
    try:
        payload = json.loads(
            (LOG_DIR.parent.parent / "menubar-room-catalog.json").read_text(
                encoding="utf-8"
            )
        )
    ...
    for room in rooms or []:
        if not isinstance(room, dict) or room.get("chat_id") != chat_id:
            continue
        return room.get("auto_reply") is not True and room.get("geeknews") is True
    return False
...
def target_chat_id_config() -> str:
    ...
    if value is None:
        raise SystemExit("auto_reply target_chat_id must be a positive integer")
    return str(value)
...
def acquire_owner() -> None:
    ...
    if LOG_DIR.is_symlink():
        raise SystemExit("AutoReply state root is unsafe")
    ...
    if (
        ...
        raise SystemExit("AutoReply state root is not private")
    ...
    try:
        ...
        if (
            ...
            raise SystemExit("AutoReply supervisor owner lock is unsafe")
        os.fchmod(fd, 0o600)
        metadata = os.fstat(fd)
        if stat.S_IMODE(metadata.st_mode) != 0o600:
            raise SystemExit("AutoReply supervisor owner lock is not private")
        ...
        if owner_lock is not None:
            owner_lock.close()
            owner_lock = None
        raise SystemExit("AutoReply supervisor owner collision") from exc
    except BaseException:
        if fd >= 0:
            os.close(fd)
        raise
...
def main() -> int:
    ...
    if args.state_root:
        ...
        os.environ["OPENKAKAO_AUTO_REPLY_STATE_ROOT"] = str(LOG_DIR)
    ...
    allow_link_fetch = (
        isinstance(auto_reply_section, dict)
        and auto_reply_section.get("allow_link_fetch") is True
    )
    os.environ["OPENKAKAO_ALLOW_LINK_FETCH"] = "1" if allow_link_fetch else "0"
    allow_image_analysis = (
        isinstance(auto_reply_section, dict)
        and auto_reply_section.get("allow_image_analysis") is True
    )
    ...
    # 두 방이 동시에 요구하면 서로를 밀어낸다(2026-09-12 실측). 답장 전송도 함께
    # 꺼지지만 워커는 계속 살아 긱뉴스 슬롯을 처리한다.
    if database_started and (
        str(os.environ.get("OPENKAKAO_GEEKNEWS_ONLY") or "").strip() == "1"
        or _geeknews_only_room()
    ):
    ...
    os.environ["OPENKAKAO_AUTO_REPLY_LOCK"] = str(LOG_DIR / ".owner-generation.lock")
    os.environ["OPENKAKAO_AUTO_REPLY_GENERATION_LOCK"] = str(
    ...
    try:
        ...
        raise SystemExit("AutoReply reply queue initialization failed") from exc
    ...
    start(
        _python_service_command(
            "scripts/auto-reply-apple-watch.py", "--interval", str(args.interval)
        ),
        "apple-watch.log",
        role="ax_watch",
    )
    if database_started:
        start(
            _python_service_command(
                "scripts/auto-reply-db-watch.py", "--interval", str(args.interval)
            ),
            "db-watch.log",
            role="db_watch",
        )
    reply_worker = start(
        _python_service_command("scripts/auto-reply-worker.py", "--worker"),
        "reply-worker.log",
        role="reply_worker",
    )
    _status_context = {

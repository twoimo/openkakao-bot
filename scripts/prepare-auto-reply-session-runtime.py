...
def _config_selectors(config: Path, expected: list[str]) -> str:
    ...
    section = _toml_section(value, "auto_reply", "auto-reply", "bujamentor")
    ...
    if (
        ...
        raise PackagingError("[auto_reply].chats must be an explicit string array")
    normalized, _ = _selectors(configured)
    if normalized != expected:
        raise PackagingError(
            "packaged --chat selectors must exactly match ordered [auto_reply].chats"
        )
    ...
    *,
    ...
    try:
        ...
        tui_argv = [
            ...
            "-B",
            "-S",
            str(scripts_runtime / "auto-reply-tui.py"),
            "--state-root",
            str(state_root),
        ]
        ...
        tui_path = runtime / "open-auto-reply-tui.command"

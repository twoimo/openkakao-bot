#!/usr/bin/env python3
"""DB-authoritative AutoReply ingress.
...
HOOK = Path(os.environ.get("OPENKAKAO_REPLY_HOOK", str(ROOT / "scripts/auto-reply-worker.py")))
QUEUE = Path(
    os.environ.get(
...
class SqliteBusyTransient(DbFence):
    """SQLCipher SQLITE_BUSY / locked; retry while delivery stays fenced."""
...
def _sqlite_busy_text(value: object) -> bool:
...
def _fixed_fence_reason(exc: BaseException) -> str:
    if isinstance(exc, PollSnapshotTransient):
        return "poll_fence"
    if isinstance(exc, SqliteBusyTransient) or (
        isinstance(exc, DbFence) and _sqlite_busy_text(exc)
    ):
...
def run_json(args: list[str], timeout: float = 5.0) -> object:
    with perf.measure("db_watch.cli") as metric:
        try:
            ...
            metric.outcome = "error"
            metric.error_class = "timeout"
            raise
        except OSError as exc:
            metric.outcome = "error"
            metric.error_class = "subprocess"
            raise DbFence("database command failed") from exc
        ...
        if returncode != 0:
            ...
            message = (
                f"database command failed: {suffix}"
                if suffix
                else "database command failed"
            )
            if _sqlite_busy_text(stderr_bytes.decode("utf-8", "replace")):
                raise SqliteBusyTransient(message)
            raise DbFence(message)
        try:
    ...
    with generation_guard:
        if owner:
            ...
            if (
                ...
                or not 0 < target < MAX_INT64
            ):
                # Cold bootstrap: a freshly adopted state carries no persisted
                # target yet. Mirror _owner_epoch_current and fall back to the
                # configured target so the very first heartbeat can persist.
                try:
                    target = configured_target_chat_id()
                except DbFence:
                    return False
    ...
    allowed_capabilities = (
        {
            ...
            ("fenced", False, "db_unavailable", "database_timeout"),
        }
        if retry_fence
        else {
...
def _cli_enrollment_target() -> dict | None:
    ...
    if kind == "local_name":
        ...
        if (
            ...
            or not 2 <= len(matched_log_ids) <= 20
            ...
            or identity.get("matched_utf8_bytes") < 0
            or (
                len(matched_log_ids) >= 3
                and identity.get("matched_utf8_bytes") < 24
            )
...
def _state(state: dict) -> dict:
    ...
    invalid_state = False
    # Freshness means "nothing persisted yet", not "dict is empty": main()
    # annotates the in-memory state (context-sync timestamps) before the
    # first _state() adoption, and those annotations must not defeat the
    # configured owner/epoch adoption on cold bootstrap.
    fresh_state = not state or (
        "_load_error" not in state and "source_epoch" not in state
    )
    ...
    # Cold bootstrap: the persisted target identity only exists after the
    # first save. Adopt the configured target up front so the very first
    # heartbeat persists a state that downstream clean/leftover validators
    # (which compare target_chat_id against the room) accept.
    if not isinstance(defaults.get("target_chat_id"), int) or not 0 < defaults["target_chat_id"] < MAX_INT64:
        try:
            defaults["target_chat_id"] = configured_target_chat_id()
        except DbFence:
            pass
    ...
    if cli_mode and state:
        ...
        leftover_orphaned_in_flight = (
            in_flight_candidate is None
            or (
                isinstance(in_flight_candidate, dict)
                and bool(in_flight_candidate.get("owner_id"))
                and in_flight_candidate.get("owner_id") == state.get("owner_id")
    ...
    if leftover_cli_takeover:
        candidate_phase = "idle"
        candidate = None
    elif candidate_phase not in {"idle", "pending", "hooking", "acknowledging"}:
        invalid_state = True
        candidate_phase = "idle"
...
def find_chat() -> dict:
    ...
    chats = run_json(["local-chats", "--limit", "200"], timeout=30.0)
...
def _validated_download_manifest(
    ...
    *,
    ...
    if (
        ...
        or kakao_image_kind(manifest.get("message_type")) != kakao_image_kind(expected_message_type)
    ...
    *,
    ...
    try:
        ...
        if (
            ...
            or kakao_image_kind(result.get("message_type")) is None
            or kakao_image_kind(result.get("message_type")) != kakao_image_kind(message_type)
        ...
        print(f"[db-watch] image download failed: {type(exc).__name__}: {exc}", file=sys.stderr)
...
_DURABLE_SKIP_ACK_REASONS = frozenset(
    {"self_author", "author_not_allowlisted", "empty_message"}
)


def _emit_with_ack_fence(*args: object, **kwargs: object) -> str:
    """Return a proven hook ACK; skip-only losses must not fence the generation."""
    skip_reason = str(kwargs.get("skip_reason") or "")
    durable_skip = skip_reason in _DURABLE_SKIP_ACK_REASONS
    try:
        ack = emit(*args, **kwargs)
    except Exception as exc:
        if durable_skip:
            print(
                f"[db-watch] skip_ack_unconfirmed:{skip_reason}:{type(exc).__name__}",
                file=sys.stderr,
                flush=True,
            )
            return "skipped"
    ...
    if ack in {"accepted", "duplicate", "skipped"}:
        return ack
    if durable_skip and ack is None:
        print(
            f"[db-watch] skip_ack_unconfirmed:{skip_reason}:none",
            file=sys.stderr,
            flush=True,
        )
        return "skipped"
...
def _kakao_daum_link_host(hostname: str) -> bool:
...
def _https_kakao_daum_url(raw: object) -> str | None:
    text = str(raw or "").strip().rstrip(".,)>")
    if not text.startswith("https://") or " " in text or "\n" in text:
        return None
    host = text[8:].split("/", 1)[0].split("?", 1)[0].split("#", 1)[0].lower().rstrip(".")
    if not _kakao_daum_link_host(host):
        return None
    return text
...
def parse_sharp_search_card(attachment: object, message_type: object) -> dict[str, Any] | None:
    try:
        parsed_type = int(message_type or 0)
    except (TypeError, ValueError):
        return None
    if parsed_type != SHARP_SEARCH_MESSAGE_TYPE:
        return None
    raw = attachment
    ...
    if isinstance(raw, str):
        ...
        return None
    content = _json_mapping(payload.get("C"))
    header = _json_mapping(content.get("HD"))
    title_card = _json_mapping(content.get("TI"))
    title_block = _json_mapping(title_card.get("TD"))
    header_title = _json_mapping(header.get("TD"))
    feed_items = content.get("ITL")
    itl_title = ""
    itl_source = ""
    itl_url = None
    if isinstance(feed_items, list) and feed_items:
        last = _json_mapping(feed_items[-1])
        last_td = _json_mapping(last.get("TD"))
        itl_title = _json_text(last_td.get("T"))
        itl_source = _json_text(last_td.get("D"))
        last_link = _json_mapping(last.get("L"))
        itl_url = _https_kakao_daum_url(last_link.get("LPC")) or _https_kakao_daum_url(
            last_link.get("LMO")
        )
    headline = itl_title or _json_text(title_block.get("T"))
    source = itl_source or _json_text(title_block.get("D"))
    # Never treat the hashtag/search query as the article headline.
    search_query = _json_text(header_title.get("T"))
    if headline and search_query and headline == search_query:
        headline = _json_text(title_block.get("T")) if itl_title == search_query else headline
    urls: list[str] = []
    seen: set[str] = set()

    def add_url(raw: object) -> None:
        url = _https_kakao_daum_url(raw)
        if url is None or url in seen:
            return
        seen.add(url)
        urls.append(url)

    title_link = _json_mapping(title_card.get("L"))
    add_url(title_link.get("LPC"))
    add_url(title_link.get("LMO"))
    add_url(itl_url)
    thumbnails = content.get("THL")
    if isinstance(thumbnails, list):
        for item in thumbnails:
            thumb_link = _json_mapping(_json_mapping(item).get("L"))
            add_url(thumb_link.get("LPC"))
            add_url(thumb_link.get("LMO"))
    platform = _json_mapping(payload.get("P"))
    add_url(platform.get("DID"))
    add_url(_json_mapping(platform.get("L")).get("LPC"))
    add_url(_json_mapping(platform.get("L")).get("LMO"))
    add_url(_json_mapping(header.get("L")).get("LPC"))
    add_url(_json_mapping(header.get("L")).get("LMO"))
    urls = urls[:2]
    if not headline and not urls:
        return None
    return {"headline": headline, "source": source, "urls": urls}


def _sharp_search_message_and_urls(message: dict) -> tuple[str, list[str]]:
    text = str(message.get("message") or "")
    card = parse_sharp_search_card(message.get("attachment"), message.get("message_type"))
    if card is None:
        return text, []
    headline = str(card.get("headline") or "").strip()
    if headline and headline not in text:
        text = f"{text}\n{headline}".strip() if text else headline
    urls = [url for url in card.get("urls") or [] if isinstance(url, str)]
    return text, urls[:2]
...
def _recent_messages(
    ...
    *,
    ...
    """Return the current message and up to RECENT_MESSAGE_LIMIT-1 preceding messages."""
...
def emit(
    ...
    *,
    ...
    attachment = kakao_image_kind(message.get("message_type", 0)) is not None and bool(message.get("attachment"))
    ...
    if isinstance(reply_to, dict) and isinstance(reply_to.get("quoted_source"), dict):
        ...
        reply_to = {
            key: value
            for key, value in reply_to.items()
            if key != "quoted_source"
        }
    event: dict[str, Any] = {
...
def poll_once(
    ...
    *,
    ...
    try:
        ...
        _stop_poll_stream()
        if isinstance(exc, (PollSnapshotTransient, SqliteBusyTransient)) or (
            isinstance(exc, (DbFence, subprocess.TimeoutExpired))
            and _fixed_fence_reason(exc) in {"poll_fence", "database_timeout"}
        ):
    ...
    if in_flight is not None:
        ...
        if not any(int(item.get("log_id", 0)) == in_flight_log_id for item in candidates):
            # An incomplete local-poll page is not a cursor identity break.
            # Keep the leftover/hooking candidate and retry instead of
            # minting reconcile_required, which exits the watcher.
            _stop_poll_stream()
            state.update(
                capability_state="fenced",
                delivery_enabled=False,
    ...
    try:
        ...
        while True:
            try:
                ...
                save_state(state)
                print(
                    f"[db-watch] {_fixed_fence_reason(exc)}:{type(exc).__name__}:{exc}",
                    flush=True,
                )
                return 1
            time.sleep(interval)

...
class AutoReplyCliRuntimeTests(unittest.TestCase):
    ...
    def test_reply_worker_never_creates_or_migrates_queue(self):
        module = self._load_auto_reply_module(
            "auto_reply_worker_queue_authority_test"
        )
    ...
    def test_response_delay_requires_room_statistics(self):
        module = self._load_auto_reply_module("auto_reply_timing_required_test")
    ...
    def test_response_delay_samples_bounded_gaussian_mixture(self):
        module = self._load_auto_reply_module("auto_reply_timing_gaussian_test")
    ...
    def test_questions_and_advice_use_only_the_immediate_component(self):
        module = self._load_auto_reply_module("auto_reply_question_timing_test")
        stats = self._timing_stats(module)
    ...
    def test_seeded_live_like_mixture_reproduces_modes_and_immediate_mass(self):
        module = self._load_auto_reply_module("auto_reply_timing_monte_carlo_test")
        sample_count = 10_472
        stats = {
            "chat": module.CHAT,
            "source": "source:opaque",
    ...
    def test_identity_questions_are_reserved_for_the_owner(self):
        module = self._load_auto_reply_module("auto_reply_identity_policy_test")
        for message in (
            "AI?",
            "너 AI야?",
            "이거 자동답변임?",
            ...
            self.assertIsNone(module.obvious_non_reply(message))
        self.assertIsNone(module.obvious_non_reply("AI 뉴스 봤어?"))
        self.assertIsNone(module.obvious_non_reply("ㅋㅋ"))
        self.assertEqual(module.obvious_non_reply("   "), "empty")

    def test_numeric_author_enrollment_rejects_self_collision_and_name_drift(self):
        module = self._load_auto_reply_module("auto_reply_numeric_author_test")
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with mock.patch.dict(os.environ, environment, clear=False):
                ...
                self.assertEqual(
                    module.numeric_author_identity_status(
                        {**event, "is_self": True, "author_id": 0}
                    ),
                    "drift",
                )
                self.assertEqual(
                ...
                self.assertEqual(
                    module.numeric_author_identity_status(
                        {**event, "author_nickname": "손님", "author_id": 8}
                    ),
                    "not_allowlisted",
                )
    ...
    def test_reply_worker_accepts_replay_floor_before_attested_tail(self):
        module = self._load_auto_reply_module("auto_reply_replay_enrollment_test")
        with tempfile.TemporaryDirectory() as temporary:
            environment = self._write_numeric_enrollment(
                module,
                temporary,
    ...
    def test_author_identity_drift_is_durable_skip_before_model_or_send(self):
        module = self._load_auto_reply_module("auto_reply_identity_drift_job_test")
        with tempfile.TemporaryDirectory() as temporary:
    ...
    def test_identity_claims_are_rejected_even_if_model_ignores_policy(self):
        module = self._load_auto_reply_module(
            "auto_reply_identity_output_guard_test"
        )
        supplied = {"recent:1"}
        for reply in (
            "응 내가 직접 쓴 거야",
            ...
            parsed = module._parse_model_decision(
                {
                    "should_reply": True,
                    "reply": reply,
                    "reason": "answer",
            ...
            self.assertEqual(parsed["reason"], "identity_question_requires_owner")

    def test_korean_laughter_policy_is_enforced_at_model_boundary(self):
        module = self._load_auto_reply_module(
            "auto_reply_laughter_output_guard_test"
        )
        base = {
            "should_reply": True,
            "reply": "좋네",
        ...
        for reply in (
            ...
            with self.subTest(reply=reply):
                ...
                self.assertIsNotNone(parsed)
                self.assertTrue(parsed["should_reply"])
                self.assertEqual(parsed["reply"], "좋네")
        for reply in (
            "좋네",
            "좋네ㅋㅋㅋ",
            "좋네ㅋㅋㅋㅋ",
    ...
    def test_reaction_register_blocks_unprompted_laughter(self):
        ...
        self.assertEqual(
            module.youtube_reaction_register()["evidence_id"],
    ...
    def test_photo_fallback_varies_and_media_placeholder_is_not_echo(self):
        ...
        self.assertTrue(
            module._policy_valid_draft("화면이 좀 깨지네", "사진", [], [])
        )
    ...
    def test_unsolicited_plan_is_rejected(self):
        ...
        self.assertTrue(
            module._outbound_unsolicited_plan("아쉽네 다음 주에 보자", "휴가 2일 남앗는데")
        )
        self.assertFalse(
            module._outbound_unsolicited_plan("아쉽네 다음 주에 보자", "다음 주에 볼까")
        )
        self.assertFalse(
            module._policy_valid_draft("아쉽네 다음 주에 보자", "머하고 놀지", [])
        )
        self.assertTrue(
    ...
    def test_youtube_title_label_is_rejected(self):
        ...
        self.assertTrue(
            module._outbound_youtube_title_label("백종원 유부우동 레시피 영상", inbound)
        )
        self.assertFalse(
            module._outbound_youtube_title_label("유부우동 먹방인가 보네", inbound)
        )
        self.assertFalse(
            module._policy_valid_draft("백종원 유부우동 레시피 영상", inbound, [])
        )
        self.assertEqual(
            module._rewrite_youtube_title_label("풀옵션 럭셔리 캠핑카 숙박 여행 영상."),
            "풀옵션 럭셔리 캠핑카 숙박",
        )
        rewritten = module._rewrite_youtube_title_label("백종원 유부우동 레시피 영상")
        self.assertEqual(rewritten, "백종원 유부우동")
        self.assertTrue(
            module._policy_valid_draft(rewritten, inbound, [])
        )
        camping = "https://www.youtube.com/watch?v=QCkbtzXbD-w 무슨 내용이야????"
        rewritten_camping = module._rewrite_youtube_title_label(
            "풀옵션 럭셔리 캠핑카 숙박 여행 영상."
        )
        self.assertTrue(
            module._policy_valid_draft(
                ...
                camping,
                [],
                [{"url": camping.split()[0], "title": "풀옵션 럭셔리 캠핑카 숙박 여행", "text": "캠핑카", "complete": True}],
            )
        )
        people = "사람은 몇명이야?"
        ...
        self.assertEqual(ranked.get("fallback"), "question_unknown")
    ...
    def test_outbound_question_follows_inbound_register(self):
        ...
        self.assertTrue(module._inbound_asks_question("약속도 맘대로 잡아버림?"))
        self.assertTrue(module._inbound_asks_question("개쩌넫ㄷㄷㅌ?"))
        self.assertTrue(module._inbound_asks_question("더 쉽게 설명해줘"))
        self.assertTrue(module._outbound_question_allows("뭘 더 말해", "이거 답변해줘"))
        self.assertFalse(module._outbound_question_allows("어디 지원하는 거예요?", "변경해서 낼 해야겟네여"))
        self.assertTrue(module._outbound_question_allows("이미 쓴 거 고쳐서 내면 되죠", "변경해서 낼 해야겟네여"))
        self.assertTrue(module._outbound_question_allows("그거 어느 포지션이에요?", "어디 지원할까?"))
        self.assertTrue(module._inbound_accuses_bot("너 봇이지"))
        self.assertTrue(module._outbound_question_allows("그래? 어떤 부분이 AI처럼 느껴졌는데", "너 봇이지"))
        self.assertFalse(module._outbound_question_allows("그래? 어떤 부분이 AI처럼 느껴졌는데", "변경해서 낼 해야겟네여"))
        self.assertFalse(module._outbound_question_allows("사람 맞아?", "너 봇이지"))
        self.assertTrue(module._inbound_accuses_bot("Ai라서 안알려줌"))
        self.assertTrue(module._outbound_question_allows("그래? 어떤 부분이 AI처럼 느껴졌는데", "Ai라서 안알려줌"))
        self.assertIsNone(module._identity_policy_decision("Ai라서 안알려줌"))
        self.assertTrue(
            module._looks_like_identity_probe("그래? 어떤 부분이 AI처럼 느껴졌는데")
        )
        self.assertTrue(module._looks_like_identity_probe("뭐가 그렇게 느껴졌는데"))
        self.assertTrue(module._inbound_identity_requires_owner("진짜 주인 데려오셈"))
        self.assertEqual(
            module._identity_policy_decision("진짜 주인 데려오셈")["reason"],
            "identity_question_requires_owner",
        )

    def test_ai_detection_learns_prior_self_tell(self):
        module = self._load_auto_reply_module("auto_reply_style_tell_learn_test")
        inbound = "그럼 이제 저걸로 ai 판단할수잇겟네ㅋ"
        self.assertFalse(module._inbound_accuses_bot(inbound))
        self.assertTrue(module._inbound_signals_ai_detection(inbound))
        self.assertTrue(module._inbound_signals_ai_detection("아 형 알아보는 방법 알아냄"))
        self.assertFalse(
            module._inbound_signals_ai_detection(
                "ai 뉴스 봤음 한도 올랐대"
            )
        )
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            avoids = module.record_learned_style_tells(
                inbound, recent, author="문승현", event_id="db:1:2"
            )
    ...
    def test_outbound_restatement_and_interrogative_register(self):
        ...
        self.assertFalse(
            module._outbound_reaction_allows("ㅇㅇ 일단 되는 동안 계속 써봐야지", inbound)
        )
        self.assertFalse(
            module._outbound_reaction_allows("그렇네 일단 유지되면 그대로 둬야할듯", inbound)
        )
        self.assertFalse(
            module._policy_valid_draft("ㅇㅇ 일단 되는대로 써보죠", inbound)
        )
        self.assertTrue(module._outbound_reaction_allows("월말까지는 그 한도로 가면 되네", inbound))
        self.assertTrue(module._outbound_reaction_allows("ㅇㅇ 저장해둘게", "번호 저장해줘"))
        self.assertTrue(module._style_sample_is_generic_ack("ㅇㅇ"))
        self.assertTrue(module._style_sample_is_generic_ack("ㅇㅇㅋㅋㅋㅋ"))
        self.assertFalse(module._style_sample_is_generic_ack("ㅇㅇ 저장해둘게"))
        self.assertTrue(
            module._style_sample_is_assistant_tell(
                "응, 사진 메시지도 확인해서 맥락에 맞게 답할 수 있어!"
            )
        )
        self.assertFalse(
            module._outbound_question_allows(
                "저런 건 어떻게 다 찾아내는 거임", "토지대장에 국 찍혀있네"
            )
        )
        self.assertFalse(module._outbound_question_allows("또 뭐 찾았는데", "지도에 그냥 다 뜨네"))
        self.assertFalse(
            module._outbound_question_allows("어케 하는지 알려줘봐", "찾는 방법 간단함")
        )
        self.assertFalse(
            module._outbound_question_allows("뭔지 궁금하긴 하네", "찾는 방법 간단함")
        )
        self.assertTrue(module._outbound_question_allows("캡차 솔버도 있네", "토지대장에 국 찍혀있네"))

    def test_outbound_share_preview_caption_is_rejected(self):
        module = self._load_auto_reply_module("auto_reply_share_caption_test")
        inbound = "https://www.instagram.com/reel/DcS9ppUz4j4/"
        previews = [
            {
                "url": inbound,
        ...
        self.assertTrue(module._outbound_labels_shared_link("스파이더맨 브랜드 뉴 데이 릴스네요", inbound))
        self.assertTrue(
            module._outbound_echoes_share_preview(
                "스파이더맨 브랜드 뉴 데이 릴스네요",
                inbound,
                previews,
            )
    ...
    def test_partner_streak_and_similar_self_register(self):
        module = self._load_auto_reply_module("auto_reply_monologue_register_test")
        inbound = "법무부네 이건"
        self.assertFalse(module._policy_valid_draft("이번엔 법무부네", inbound))
        self.assertFalse(
            module._policy_valid_draft(
                "대만 44면 가격 싼 편인데",
                "흠?",
        ...
        self.assertTrue(module._style_sample_is_spectator_narration("대만 여행 알아보나 보네"))
        self.assertTrue(module._style_sample_is_spectator_narration("법무부면 교도소나 구치소 쪽인가보네"))
        self.assertFalse(
            module._style_sample_is_spectator_narration("사용량 많으면 플러스가 낫긴 하지")
        )
        recent = [
            {
                "author_nickname": "문승현",
        ...
        self.assertIsNone(
            module._partner_streak_hold_reason(
                {"author_nickname": "문승현", "sent_at": 1_800},
                "큰거온다...",
                [
                    {
                        "author_nickname": "문승현",
        ...
        self.assertIsNone(
            module._partner_streak_hold_reason(
                {"author_nickname": "문승현", "sent_at": 1_803},
                "큰거온다...",
                [
                    {
                        "author_nickname": "문승현",
        ...
        self.assertEqual(
            module._partner_streak_hold_reason(
                {"author_nickname": "문승현", "sent_at": 25},
                "큰거온다...",
                [
                    {
                        "author_nickname": "문승현",
    ...
    def test_partner_streak_uses_recent_sent_jobs(self):
        module = self._load_auto_reply_module("auto_reply_partner_sent_jobs")
        snapshot = [
            {
                "author_nickname": "문승현",
                "is_self": False,
        ...
        self.assertIsNone(
            module._partner_streak_hold_reason(event, "이게 머고!!!!", snapshot)
        )
        extra = [
            {
                "author_nickname": "최연우",
        ...
        merged = module._merge_recent_sent_self(snapshot, extra)
        self.assertEqual(
            module._partner_streak_hold_reason(event, "이게 머고!!!!", merged),
            "already_commented",
        )
        other = module._merge_recent_sent_self(
            snapshot,
            [
                {
                    "author_nickname": "최연우",
        ...
        # Injected self still sits after this partner's last turn, so the
        # query helper must filter by author instead of merging foreign sends.
        self.assertEqual(
            module._partner_streak_hold_reason(event, "이게 머고!!!!", other),
            "already_commented",
        )
        ...
        connection.executemany(
            """
            INSERT INTO reply_jobs(event_json, reply, updated_at, reason, status)
            VALUES (?, ?, ?, ?, ?)
            """,
            [
                (
        ...
        connection.commit()
        turns = module._recent_sent_self_turns_for_author(connection, event)
        self.assertEqual([row["message"] for row in turns], ["생각보다 잘 뽑힘"])
        queued = module._merge_recent_sent_self(snapshot, turns)
        self.assertEqual(
            module._partner_streak_hold_reason(event, "이게 머고!!!!", queued),
            "already_commented",
        )
        later = dict(event, sent_at=2000)
        self.assertEqual(
            module._recent_sent_self_turns_for_author(connection, later),
            [],
        )
        hyunjun = {
            "author_nickname": "현준",
        ...
        self.assertEqual(
            [row["message"] for row in module._recent_sent_self_turns_for_author(connection, hyunjun)],
            ["현준 쪽에 답한 거"],
        )
        connection.close()


    def test_sharp_search_card_enriches_summary_without_raw_blob(self):
        db_watch = self._load_db_watch_module("auto_reply_sharp_search_card_test")
        attachment = json.dumps(
            {
                "P": {
                    "ME": "샵검색: #대만 44만원",
                    "DID": "https://m.search.daum.net/kakao?w=tot&q=%EB%8C%80%EB%A7%8C",
                    "A": {"code": "SECRETCODEBLOB", "createdAt": 1},
                    "RF": "sharp_search",
                },
                "C": {
                    "THC": 1,
                    "THL": [
                        {
                            "TH": {"THU": "https://search4.kakaocdn.net/argon/x.jpg"},
                            "L": {
                                "LPC": "https://v.kakao.com/v/20260818174953765",
                                "LMO": "https://v.kakao.com/v/20260818174953765",
                            },
                        }
                    ],
                    "HD": {"TD": {"T": "대만 44만원"}},
                    "TI": {
                        "TD": {
                            "T": "AI 호황에도 양극화 커진 대만 1인당 44만원으로 ‘민심 달래기’",
                            "D": "서울경제 · 다음뉴스",
                        },
                        "L": {
                            "LPC": "https://v.kakao.com/v/20260818174953765",
        ...
        self.assertEqual(
            summary["urls"][0],
            "https://v.kakao.com/v/20260818174953765",
        )
        self.assertFalse(summary["attachment"])
    ...
    def test_shop_search_article_guess_and_rerank_query(self):
        ...
        inbound = (
            "샵검색: #대만 44만원\n"
            "AI 호황에도 양극화 커진 대만 1인당 44만원으로 ‘민심 달래기’"
        )
        previews = [
            {
                "url": "https://v.kakao.com/v/20260818174953765",
                "title": "AI 호황에도 양극화 커진 대만 1인당 44만원으로 ‘민심 달래기’",
                "text": "라이칭더 총통이 1인당 1만 대만달러(약 44만원)를 지급하는 AI 배당",
                "complete": True,
            }
        ]
        self.assertFalse(
            module._policy_valid_draft(
                "대만 여행 알아보나 보네", inbound, [], previews
            )
        )
        self.assertFalse(
            module._policy_valid_draft(
                "대만 44면 좀 비싼 느낌인데", inbound, [], previews
            )
        )
        self.assertFalse(
            module._policy_valid_draft(
                "대만 44면 가격 싼 편인데", inbound, [], previews
            )
        )
        self.assertTrue(
            module._policy_valid_draft(
                "AI 배당이라 이거지", inbound, [], previews
            )
        )
        query = module._rerank_query_text(
            {"urls": ["https://v.kakao.com/v/20260818174953765"]},
            inbound,
            [
                {
                    "author_nickname": "현준",
        ...
        self.assertIn("민심 달래기", query)
        self.assertIn("라이칭더", query)

    def test_composer_occupied_preflight_defers_without_send(self):
        module = self._load_auto_reply_module("auto_reply_composer_occupied_defer")
        event = {"_composer_occupied": True, "sent_at": int(time.time()) - 10, "response_window_upper_seconds": 30}
        self.assertEqual(module.COMPOSER_OCCUPIED_DEFER_SECONDS, 20.0)
        now = time.time()
        retry = []
        original = module.settle_processing_transition
        def capture(*args, **kwargs):
            retry.append(kwargs.get("due_at"))
            return True
        module.settle_processing_transition = capture
        try:
            module.defer_scheduled_pre_send_unavailable(event, "db:42:1", None, now=now)
        finally:
    ...
    def test_model_evidence_ids_are_exact_unique_and_known(self):
        module = self._load_auto_reply_module(
            "auto_reply_model_evidence_strict_test"
        )
        base = {
            "should_reply": True,
            "reply": "확인해볼게",
        ...
        parsed_fabricated = module._parse_model_decision(fabricated, {"recent:1"})
        self.assertIsNotNone(parsed_fabricated)
        self.assertEqual(parsed_fabricated["evidence_ids"], ["recent:1"])
        duplicate = dict(base, evidence_ids=["recent:1", "recent:1"])
        parsed_duplicate = module._parse_model_decision(duplicate, {"recent:1"})
        self.assertIsNotNone(parsed_duplicate)
        self.assertEqual(parsed_duplicate["evidence_ids"], ["recent:1"])

    def test_parse_model_decision_coerces_string_should_reply(self):
        module = self._load_auto_reply_module("parse_string_should_reply")
        parsed = module._parse_model_decision(
            {
                "should_reply": "true",
                "reply": "확인해볼게",
        ...
        self.assertIsNotNone(parsed)
        self.assertTrue(parsed["should_reply"])
        self.assertEqual(parsed["reply"], "확인해볼게")

    def test_parse_model_decision_accepts_null_reply_on_skip(self):
        module = self._load_auto_reply_module("parse_null_reply_skip")
        parsed = module._parse_model_decision(
            {
                "should_reply": False,
                "reply": None,
        ...
        self.assertFalse(parsed["should_reply"])
        self.assertEqual(parsed["reply"], "")
        self.assertEqual(parsed["reason"], "bot_command")
        coerced = module._parse_model_decision(
            {
                "should_reply": True,
                "reply": None,
                "category": "social",
        ...
        self.assertFalse(coerced["should_reply"])
        self.assertEqual(coerced["reply"], "")
        self.assertEqual(coerced["evidence_ids"], [])

    def test_parse_model_decision_strips_uninvited_awe_without_invalidating(self):
        module = self._load_auto_reply_module("parse_strip_awe")
        parsed = module._parse_model_decision(
            {
                "should_reply": True,
                "reply": "그건 좀 오 ㄷㄷ",
        ...
        self.assertIsNotNone(parsed)
        self.assertTrue(parsed["should_reply"])
        self.assertEqual(parsed["reply"], "그건 좀 오")
        self.assertNotIn("ㄷㄷ", parsed["reply"])

    def test_parse_model_decision_policy_skip_is_not_none(self):
        module = self._load_auto_reply_module("parse_policy_skip")
        parsed = module._parse_model_decision(
            {
                "should_reply": True,
                "reply": "ㄷㄷ",
        ...
        self.assertFalse(parsed["should_reply"])
        self.assertEqual(parsed["reply"], "")
        self.assertEqual(parsed["reason"], "reply_laughter_policy_violation")
        self.assertEqual(parsed["evidence_ids"], [])

    def test_link_fetch_requires_separate_opt_in_and_never_ignores_overflow(self):
        module = self._load_auto_reply_module("auto_reply_link_egress_policy_test")
        now = int(time.time())
    ...
    def test_recipient_register_hint_is_evidence_based(self):
        module = self._load_auto_reply_module("auto_reply_recipient_register_test")
        ...
        self.assertEqual(
            module._recipient_register_hint(
                informal, used_fallback=False, recipient="현준"
            )["register"],
            "honorific",
        )

    def test_hyunjun_honorific_register_rejects_banmal(self):
        module = self._load_auto_reply_module("auto_reply_hyunjun_honorific_test")
        inbound = "23일 밤초기화라"
        self.assertEqual(module._reply_register("그럼 딱 맞겠네"), "informal")
        self.assertEqual(module._reply_register("그럼 딱 맞겠네요"), "honorific")
        self.assertEqual(module._reply_register("그럴듯여"), "honorific")
        self.assertEqual(module._reply_register("ㅋㅋㅋㅋㅋㅋㅋㅋ"), "unmarked")
        self.assertFalse(
            module._policy_valid_draft(
                "그럼 딱 맞겠네",
                inbound,
        ...
        self.assertEqual(chosen["reply"], "그럴듯여")
        self.assertNotIn("그럼 딱 맞겠네", chosen["drafts"])

    def test_context_sync_requires_authoritative_exact_room_result(self):
        db_watch = self._load_db_watch_module("auto_reply_context_sync_test")
        valid = {
            "schema_version": 1,
    ...
    def test_context_sync_exhausted_snapshot_retry_is_narrowly_classified(self):
        db_watch = self._load_db_watch_module(
            "auto_reply_context_sync_transient_exit_test"
        )
        with (
            mock.patch.object(
                db_watch,
        ...
        with mock.patch.object(
            db_watch,
            "_run_bounded_cli",
            side_effect=db_watch.subprocess.TimeoutExpired(["context-sync-local"], 90.0),
        ):
            with self.assertRaises(db_watch.subprocess.TimeoutExpired):
                db_watch.run_json(["context-sync-local"], timeout=90.0)

        with mock.patch.object(
            db_watch,
            "_run_bounded_cli",
            return_value=(1, b"", b"Error: sqlite busy\n"),
        ):
            with self.assertRaisesRegex(
                db_watch.SqliteBusyTransient,
                "database command failed: Error: sqlite busy",
            ):
                db_watch.run_json(["local-chats", "--limit", "200"], timeout=30.0)
        self.assertEqual(
            db_watch._fixed_fence_reason(
                db_watch.SqliteBusyTransient(
                    "database command failed: Error code 5: The database file is locked"
                )
            ),
            "database_timeout",
        )
    ...
    def test_periodic_context_sync_transient_fences_without_exiting(self):
        db_watch = self._load_db_watch_module(
            "auto_reply_periodic_context_sync_transient_test"
        )
        db_watch.SELF = "self"
        valid = {
            "schema_version": 1,
    ...
    def test_startup_context_sync_transient_recovers_without_process_exit(self):
        db_watch = self._load_db_watch_module(
            "auto_reply_startup_context_sync_transient_recovery_test"
        )
        db_watch.SELF = "self"
        valid = {
            "schema_version": 1,
    ...
    def test_startup_context_sync_persistent_transient_never_becomes_ready(self):
        db_watch = self._load_db_watch_module(
            "auto_reply_startup_context_sync_persistent_test"
        )
        db_watch.SELF = "self"
        saved = []
        waited = []
    ...
    def test_startup_context_sync_retry_refreshes_heartbeat_while_fenced(self):
        db_watch = self._load_db_watch_module(
            "auto_reply_startup_context_sync_heartbeat_test"
        )
        state = {
    ...
    def test_context_sync_deferral_keeps_watcher_alive_and_fail_closed(self):
        db_watch = self._load_db_watch_module("auto_reply_context_sync_deferred_test")
        db_watch.SELF = "self"
        deferred = {
            "schema_version": 1,
            "action": "context_sync_local",
    ...
    def test_authoritative_context_sync_deferral_preserves_watcher_readiness(self):
        db_watch = self._load_db_watch_module(
            "auto_reply_context_sync_authoritative_deferred_test"
        )
        db_watch.SELF = "self"
        deferred = {
            "schema_version": 1,
    ...
    def test_clean_transient_poll_fence_is_persisted_before_bounded_retry(self):
        db_watch = self._load_db_watch_module("auto_reply_clean_poll_retry_test")
        clean = {
            "schema_version": 3,
            "target_chat_id": 42,
    ...
    def test_poll_retry_is_persistent_only_for_clean_typed_snapshot_gap(self):
        db_watch = self._load_db_watch_module("auto_reply_poll_retry_terminal_test")
        clean = {
            "schema_version": 3,
            "target_chat_id": 42,
            "target_chat_name": db_watch.CHAT,
    ...
    def test_clean_poll_retry_wait_refreshes_fenced_heartbeat(self):
        db_watch = self._load_db_watch_module(
            "auto_reply_poll_retry_heartbeat_test"
        )
        state = {
            "capability_state": "fenced",
            "delivery_enabled": False,
    ...
    def test_only_exact_local_poll_snapshot_gap_gets_typed_retry_marker(self):
        db_watch = self._load_db_watch_module(
            "auto_reply_typed_poll_snapshot_gap_test"
        )
        enrollment = {
            "chat_id": 42,
    ...
    def test_send_readiness_fence_accepts_matching_privacy_digest_and_rejects_drift(self):
        module = self._load_auto_reply_module("auto_reply_send_privacy_digest_test")
        digest = "a" * 64
        now = time.time()
        supervisor = {
            "schema_version": 1,
    ...
    def test_send_fence_token_ignores_later_room_tail(self):
        module = self._load_auto_reply_module("auto_reply_send_fence_later_tail")
        digest = "a" * 64
        now = time.time()
        supervisor = {
            "schema_version": 1,
        ...
        first = module._send_fence_token(supervisor, db_state)
        db_state["acked_watermark"] = 99
        db_state["last_observed_log_id"] = 99
        db_state["observed_log_ids"] = [42, 99]
        db_state["acked_log_ids"] = [42, 99]
        second = module._send_fence_token(supervisor, db_state)
        self.assertEqual(first, second)

    def test_recipient_bundle_is_strict_and_exposes_register_hint(self):
        module = self._load_auto_reply_module("auto_reply_recipient_bundle_test")
        ...
        payload = {
            ...
            "styles": [
                {"message": "그렇군요", "user": "최연우"},
                {"message": "그럼 딱 맞겠네", "user": "최연우"},
            ],
            "prior_decisions": [],
        ...
        with (
            mock.patch.object(
                module,
                "sync_live_context_index",
                return_value={
                    "schema_version": 1,
                    "action": "context_sync_local",
                    "chat_id": 42,
                    "chat": module.CHAT,
                    "checkpoint_log_id": 99,
                    "pages": 1,
                    "authoritative": True,
                    "deferred": None,
                    "totals": {"messages": 1},
        ...
        command = run_json.call_args_list[0].args[0]
        self.assertEqual(
            [
                command[index + 1]
        ...
        self.assertEqual(
            [item["message"] for item in bundle["styles"]],
            ["그렇군요"],
        )
        self.assertEqual(
        ...
        with (
            mock.patch.object(module, "sync_live_context_index", return_value={"schema_version": 1, "action": "context_sync_local", "chat_id": 42, "chat": module.CHAT, "network": False}),
    ...
    def test_persist_reply_evidence_ledger_writes_grounding_receipt(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            module.EVIDENCE_LEDGER = Path(temporary) / "reply-evidence.jsonl"
            event = {
                "event_id": "db:42:99",
                "author_nickname": "현준",
                "log_id": 99,
            ...
            analysis = {
                "decision": "reply",
                "reason": "useful_information",
                "category": "social",
                "reply": "사용량 많으면 플러스가 낫긴 하지",
                "evidence_ids": ["recent:99", "context:1"],
                "recent_conversation": [{"evidence_id": "recent:99", "message": "플러스로 바꿔야하나"}],
                "context_match_count": 1,
                "style_match_count": 2,
                "best_context_score": 0.8,
                "best_style_score": 0.4,
                "prior_similarity": 0.1,
                "provenance": {"context_sync": {"checkpoint_log_id": 99}},
            }
            path = module.persist_reply_evidence_ledger(
                event, analysis, status="scheduled", delay_seconds=8.0
            )
            self.assertEqual(path, module.EVIDENCE_LEDGER)
            rows = [
                json.loads(line)
                for line in module.EVIDENCE_LEDGER.read_text(encoding="utf-8").splitlines()
                if line
            ]
            self.assertEqual(len(rows), 1)
            self.assertEqual(rows[0]["event_id"], "db:42:99")
            self.assertEqual(rows[0]["evidence_ids"], ["recent:99", "context:1"])
            self.assertEqual(rows[0]["context_sync"]["checkpoint_log_id"], 99)
    def test_later_self_already_replied_detects_newer_self_row(self):
        module = self._load_auto_reply_module("auto_reply_later_self_test")
        rows = [
            {"log_id": 10, "is_self": False, "message": "개꿀잼"},
            {"log_id": 11, "is_self": True, "message": "그거 ㄹㅇ 웃김"},
        ]
        with mock.patch.object(
            module,
            ...
            self.assertFalse(module.later_self_already_replied(10))
            self.assertTrue(module.later_self_already_replied(10, "그거 ㄹㅇ 웃김"))
            self.assertFalse(module.later_self_already_replied(10, "다른 말"))
            self.assertFalse(module.later_self_already_replied(11, "그거 ㄹㅇ 웃김"))
            self.assertFalse(module.later_self_already_replied(None))
    def test_inbound_reply_to_skips_short_pointers_and_unbound_quotes(self):
        module = self._load_auto_reply_module("auto_reply_reply_to_filter_test")
        self.assertEqual(
            module._inbound_reply_to_text({"message": "question", "proactive": False}),
            "question",
        )
        self.assertIsNone(module._inbound_reply_to_text({"message": "댓글", "message_type": 1}))
        self.assertIsNone(
            module._inbound_reply_to_text({"message": "첫번째 기사", "message_type": 26})
        )
        self.assertEqual(
            module._inbound_reply_to_text(
                {
                    "message": "첫번째 기사",
                    "message_type": 26,
                    "quoted_source": "샵검색: #트럼프 이재명",
                }
            ),
            "샵검색: #트럼프 이재명",
        )
        self.assertIsNone(module._inbound_reply_to_text({"message": "사진"}))
        self.assertIsNone(module._inbound_reply_to_text({"proactive": True, "message": "hello there"}))
        self.assertEqual(
            module._inbound_reply_to_text({"message": "에서 쉰내가", "message_type": 1}),
            "에서 쉰내가",
        )

    def test_select_ranked_reply_uses_scores_and_identity_hardcodes(self):
        module = self._load_auto_reply_module("auto_reply_rerank_select_test")

        class FakeClient:
            def rank(self, query, drafts):
                self.query = query
                return {"ok": True, "fallback": "none", "scores": [0.1, 0.9], "winner_index": 1}
        ...
        recent = [{"author_nickname": "문승현", "message": "이겅", "is_self": False}]
        chosen = module.select_ranked_reply(
            "그래서 뭐임",
            "그건 좀",
            ["그건 좀", "그냥 그거임"],
        ...
        self.assertEqual(chosen["reply"], "그냥 그거임")
        self.assertEqual(chosen["winner_index"], 1)
        mention = module.select_ranked_reply(
            "@최연우",
            "어",
            ["어", "왜"],
            event,
        ...
        self.assertEqual(mention["reply"], "왜")
        identity = module.select_ranked_reply(
            "ai가 아니엿음 ㄹㅇ?",
            "그건 좀 아닌데",
            ["그건 좀 아닌데", "에이 아닌데"],
            event,
        ...
        self.assertEqual(identity["reply"], "에이 아닌데")
        query = module._rerank_query_text(event, "이겅", recent)
        self.assertIn("연우일까 ai일까", query)
        self.assertNotIn("이겅", query)

        class TimeoutClient:
            def rank(self, query, drafts):
                return {"ok": False, "fallback": "timeout", "scores": [], "winner_index": None}

        timed = module.select_ranked_reply(
            "그래서 뭐임",
            "그건 좀",
            ["그건 좀", "그냥 그거임"],
            {},
    ...
    def test_idle_runner_status_hash_is_bounded_and_module_local(self):
        with tempfile.TemporaryDirectory(dir=Path.home()) as temporary:
            ...
            module, _runner = self._load_trusted_codex_module(
                "auto_reply_runner_idle_cache_test", root
            )
            connection = self._worker_queue_connection(module)
            try:
                with mock.patch.object(
                ...
                second, _second_runner = self._load_trusted_codex_module(
                    "auto_reply_runner_cache_isolation_test", root
                )
                self.assertIsNot(
    ...
    def test_runner_metadata_drift_rehashes_immediately_and_stays_unavailable(self):
        with tempfile.TemporaryDirectory(dir=Path.home()) as temporary:
            ...
            module, runner = self._load_trusted_codex_module(
                "auto_reply_runner_metadata_drift_test", root
            )
            connection = self._worker_queue_connection(module)
            try:
                with (
    ...
    def test_generate_reply_always_bypasses_runner_hash_cache(self):
        with tempfile.TemporaryDirectory(dir=Path.home()) as temporary:
            root = Path(temporary)
            module, _runner = self._load_trusted_codex_module(
                "auto_reply_runner_force_hash_test", root
            )
            response = {
                "should_reply": False,
                "reply": "",
    ...
    def test_deferred_media_survives_response_window_and_terminal_cleanup_is_immediate(self):
        module = self._load_db_watch_module("auto_reply_deferred_media_ttl_test")
    ...
    def test_codex_luna_max_fast_runner_is_pinned_and_parses_jsonl(self):
        with tempfile.TemporaryDirectory(dir=Path.home()) as temporary:
            ...
            with mock.patch.dict(os.environ, environment, clear=False):
                module = self._load_auto_reply_module("auto_reply_codex_runner_test")
            module.QUEUE = Path(temporary) / "reply-queue.sqlite3"
            ...
            self.assertEqual(
                ...
                str(SCRIPTS / "auto-reply-schema.json"),
            )
    ...
    def test_direct_target_prompt_is_strong_but_allows_low_value_skip(self):
        with tempfile.TemporaryDirectory(dir=Path.home()) as temporary:
            module, _runner = self._load_trusted_codex_module(
                "auto_reply_direct_target_prompt_test",
                temporary,
            )
            recent = [
                {
            ...
            target = {
                ...
                "source_message": "unique bounded source",
            ...
            self.assertGreaterEqual(stdin_text.count("unique bounded source"), 1)
            instructions = "\n".join(prompt["instructions"])
            self.assertIn("strong signal", instructions)
            self.assertIn("quoted source_message", instructions)
    ...
    def test_direct_target_reply_requires_exact_target_evidence(self):
        ...
        def run_decision(response, module_suffix):
            with tempfile.TemporaryDirectory(dir=Path.home()) as temporary:
                module, _runner = self._load_trusted_codex_module(
                    f"auto_reply_target_evidence_{module_suffix}",
                    temporary,
                )
                event = {
                    "type": "item.completed",
        ...
        self.assertTrue(unrelated_only["should_reply"])
        self.assertIn("recent:500", unrelated_only["evidence_ids"])
        self.assertNotEqual(unrelated_only.get("model_failure_class"), "invalid_output")
    ...
    def test_model_failure_classifier_separates_limits_from_generic_exit(self):
        module = self._load_auto_reply_module("auto_reply_model_failure_classifier_test")
        quota = {
            "type": "turn.failed",
            "error": {
        ...
        self.assertEqual(
            module._classify_model_failure(
                1,
                b"",
                b"Error: No API key for google-antigravity/gemini-3.7-flash-tiered\n",
            )[0],
            "authentication",
        )
        self.assertNotEqual(
            module._model_circuit_key("omlx/Qwen3.6-35B-A3B-8bit"),
            module._model_circuit_key("google-antigravity/gemini-3.7-flash-tiered"),
        )
    ...
    def test_retry_after_http_date_is_structured_clock_safe_and_bounded(self):
        module = self._load_auto_reply_module("auto_reply_retry_http_date_test")
        now = 1_800_000_000.0
    ...
    def test_codex_rate_limit_circuit_persists_without_sensitive_output(self):
        with tempfile.TemporaryDirectory(dir=Path.home()) as temporary:
            ...
            with mock.patch.dict(os.environ, environment, clear=False):
                module = self._load_auto_reply_module(
                    "auto_reply_rate_limit_circuit_test"
                )
            calls = []
            ...
            with mock.patch.dict(os.environ, environment, clear=False):
                restarted = self._load_auto_reply_module(
                    "auto_reply_rate_limit_restart_test"
                )
    ...
    def test_model_call_lease_allows_only_one_probe(self):
        module = self._load_auto_reply_module("auto_reply_model_call_lease_test")
        with tempfile.TemporaryDirectory() as temporary:
            module.QUEUE = Path(temporary) / "reply-queue.sqlite3"
    ...
    def test_model_call_circuit_is_shared_across_independent_room_queues(self):
        module = self._load_auto_reply_module(
            "auto_reply_multi_room_model_circuit_test"
        )
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
    ...
    def test_shared_model_success_rearms_only_in_flight_pending_jobs(self):
        module = self._load_auto_reply_module(
            "auto_reply_shared_model_success_rearm_test"
        )
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            root.chmod(0o700)
    ...
    def test_shared_model_failure_rebounds_to_cooldown_or_original_deadline(self):
        module = self._load_auto_reply_module(
            "auto_reply_shared_model_cooldown_rearm_test"
        )
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            root.chmod(0o700)
    ...
    def test_shared_model_rearm_queue_update_is_atomic(self):
        module = self._load_auto_reply_module(
            "auto_reply_shared_model_rearm_atomic_test"
        )
        with tempfile.TemporaryDirectory() as temporary:
            module.QUEUE = Path(temporary) / "reply-queue.sqlite3"
            queue = self._worker_queue_connection(module)
    ...
    def test_account_circuit_refuses_nonempty_legacy_room_breaker(self):
        module = self._load_auto_reply_module(
            "auto_reply_legacy_model_circuit_guard_test"
        )
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            room = root / "rooms" / "101"
    ...
    def test_luna_capacity_probe_uses_exact_snapshot_and_synthetic_input(self):
        with tempfile.TemporaryDirectory(dir=Path.home()) as temporary:
            root = Path(temporary)
            module, runner = self._load_trusted_codex_module(
                "auto_reply_luna_capacity_probe_success_test",
                root,
            )
            now = time.time()
    ...
    def test_luna_capacity_probe_failure_reopens_exact_lease_without_raw_output(self):
        with tempfile.TemporaryDirectory(dir=Path.home()) as temporary:
            root = Path(temporary)
            module, _runner = self._load_trusted_codex_module(
                "auto_reply_luna_capacity_probe_failure_test",
                root,
            )
            now = time.time()
            open_until = now + 6 * 60 * 60
    ...
    def test_luna_capacity_probe_requires_exact_false_synthetic_response(self):
        with tempfile.TemporaryDirectory(dir=Path.home()) as temporary:
            root = Path(temporary)
            module, _runner = self._load_trusted_codex_module(
                "auto_reply_luna_capacity_probe_exact_response_test",
                root,
            )
            now = time.time()
            open_until = now + 6 * 60 * 60
    ...
    def test_luna_capacity_probe_cas_refuses_drift_other_class_and_live_lease(self):
        module = self._load_auto_reply_module(
            "auto_reply_luna_capacity_probe_cas_refusal_test"
        )
        with tempfile.TemporaryDirectory() as temporary:
            module.QUEUE = Path(temporary) / "reply-queue.sqlite3"
            now = 10_000.0
    ...
    def test_luna_capacity_probe_interruption_leaves_durable_lease(self):
        with tempfile.TemporaryDirectory(dir=Path.home()) as temporary:
            root = Path(temporary)
            module, _runner = self._load_trusted_codex_module(
                "auto_reply_luna_capacity_probe_interruption_test",
                root,
            )
            now = time.time()
            open_until = now + 6 * 60 * 60
    ...
    def test_luna_capacity_probe_cli_requires_offline_attestation_and_snapshot(self):
        module = self._load_auto_reply_module(
            "auto_reply_luna_capacity_probe_cli_test"
        )
        available = {
    ...
    def test_model_limit_defers_queue_without_terminal_context_projection(self):
        module = self._load_auto_reply_module("auto_reply_model_queue_defer_test")
        stats = self._timing_stats(module)
        with tempfile.TemporaryDirectory() as temporary:
            module.QUEUE = Path(temporary) / "reply-queue.sqlite3"
            now = int(time.time())
    ...
    def test_repeated_model_defers_keep_first_response_deadline(self):
        module = self._load_auto_reply_module(
            "auto_reply_repeated_model_queue_defer_test"
        )
    ...
    def test_usage_and_quota_cooldowns_are_not_aggressive_retries(self):
        module = self._load_auto_reply_module("auto_reply_model_cooldown_policy_test")
        with mock.patch.object(module.random, "random", return_value=0.0):
            self.assertEqual(
    ...
    def test_bounded_process_duplexes_input_larger_than_pipe_capacity(self):
        module = self._load_auto_reply_module("auto_reply_bounded_stdin_test")
    ...
    def test_bounded_process_stdin_timeout_and_output_overflow_fail_closed(self):
        module = self._load_auto_reply_module("auto_reply_bounded_failure_test")
    ...
    def test_bounded_process_broken_stdin_pipe_fails_closed(self):
        module = self._load_auto_reply_module("auto_reply_broken_stdin_test")
        payload = b"sensitive" * 128 * 1024
    ...
    def test_bounded_process_rejects_stdin_over_cap_before_spawn(self):
        module = self._load_auto_reply_module("auto_reply_stdin_cap_test")
        with (
    ...
    def test_isolated_model_process_group_reaps_sigterm_ignoring_descendant(self):
        module = self._load_auto_reply_module("auto_reply_model_group_reap_test")
        with tempfile.TemporaryDirectory() as temporary:
    ...
    def test_codex_runner_rejects_model_or_digest_drift(self):
        with tempfile.TemporaryDirectory(dir=Path.home()) as temporary:
            ...
            with mock.patch.dict(os.environ, environment, clear=False):
                module = self._load_auto_reply_module("auto_reply_codex_drift_test")
    ...
    def test_dynamic_ax_script_uses_exact_escaped_chat_name(self):
        ...
        try:
            import auto_reply_ax_ui

            module = importlib.reload(auto_reply_ax_ui)
            ...
            importlib.reload(auto_reply_ax_ui)
    ...
    def test_empty_ax_snapshot_is_healthy_only_for_db_authoritative_window_liveness(self):
        module = self._load_apple_watch_module("auto_reply_apple_db_liveness_test")
        state = {}
        with (
            mock.patch.dict(
                os.environ,
    ...
    def test_db_authoritative_empty_snapshot_degrades_when_exact_window_is_unavailable(self):
        module = self._load_apple_watch_module("auto_reply_apple_db_missing_test")
        with (
            mock.patch.dict(
                os.environ,
                {"OPENKAKAO_DB_AUTHORITATIVE": "1"},
    ...
    def test_apple_watch_hook_python_contract_and_exact_command(self):
        module = self._load_apple_watch_module(
            "auto_reply_apple_hook_contract_test"
        )
    ...
    def test_apple_watch_hook_ignores_shebang_and_disables_bytecode(self):
        module = self._load_apple_watch_module(
            "auto_reply_apple_hook_isolation_test"
        )
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
...
""",
    ...
    def test_apple_watch_hook_rejects_mismatched_or_unsafe_python(self):
        module = self._load_apple_watch_module(
            "auto_reply_apple_hook_rejection_test"
        )
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
    ...
    def test_supervisor_db_ready_uses_current_environment(self):
        supervisor = self._load_supervisor_module("auto_reply_supervisor_db_ready_env_test")
        captured = {}
    ...
    def test_supervisor_uses_one_attested_config_buffer(self):
        spec = importlib.util.spec_from_file_location(
            "auto_reply_supervisor_attestation_test",
            SCRIPTS / "auto-reply-supervisor.py",
        )
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            try:
                ...
                self.assertTrue(parsed["safety"]["allow_auto_reply"])
    ...
    def test_supervisor_unexpected_child_exit_is_nonzero(self):
        supervisor = self._load_supervisor_module(
            "auto_reply_supervisor_unexpected_exit_test"
        )
    ...
    def test_supervisor_abnormal_stop_never_promotes_stopped_clean(self):
        supervisor = self._load_supervisor_module(
            "auto_reply_supervisor_abnormal_stop_test"
        )
        supervisor.children.clear()
    ...
    def test_supervisor_stopped_clean_requires_intent_terminal_queue_and_children(self):
        supervisor = self._load_supervisor_module(
            "auto_reply_supervisor_stopped_clean_test"
        )
    ...
    def test_supervisor_owner_lock_is_private_and_rejects_links(self):
        supervisor = self._load_supervisor_module(
            "auto_reply_supervisor_private_owner_lock_test"
        )
        with tempfile.TemporaryDirectory() as temporary:
    ...
    def test_generation_and_reply_state_locks_are_private_and_no_follow(self):
        modules = (
            (
                "supervisor",
                self._load_supervisor_module("auto_reply_supervisor_lock_helper_test"),
                ".owner-generation.lock",
            ),
            (
                "db_watch",
                self._load_db_watch_module("auto_reply_db_lock_helper_test"),
                ".owner-generation.lock",
            ),
            (
                "auto_reply",
                self._load_auto_reply_module("auto_reply_reply_lock_helper_test"),
    ...
    def test_supervisor_python_children_use_isolated_interpreter_flags(self):
        supervisor = self._load_supervisor_module(
            "auto_reply_supervisor_python_isolation_test"
        )
        ...
        try:
            ...
            self.assertEqual(
                supervisor._python_service_command(
                    "scripts/auto-reply-worker.py", "--worker"
                ),
                [
                    "/trusted/python3",
                    "-E",
    ...
    def test_supervisor_python_isolation_ignores_hostile_sitecustomize(self):
        supervisor = self._load_supervisor_module(
            "auto_reply_supervisor_sitecustomize_isolation_test"
        )
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
    ...
    def test_pre_ax_deferral_requires_exact_pending_descriptor(self):
        ...
        auto_reply = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(auto_reply)
        ...
        result = auto_reply.pre_ax_delivery_result(
            readiness="not_ready",
        ...
        generic = auto_reply.pre_ax_delivery_result(
            readiness="not_ready",
            candidate_proven=True,
    ...
    def test_leftover_ack_resume_adopts_new_owner_from_fenced_reconcile_gap(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            module = self._load_db_watch_module("auto_reply_leftover_ack_resume_test")
    ...
    def _leftover_resume_env(self, temporary, module_name, fence_state, identity=None):
        room_root = Path(temporary) / "rooms" / "42"
        room_root.mkdir(parents=True, exist_ok=True)
        enrollment_path = Path(temporary) / "enrollment.json"
        enrollment = {
        ...
        module = self._load_db_watch_module(module_name)
        module.STATE = room_root / "db-watch-state.json"
        module.CHAT = "enrolled-room"
        enrollment_target = {
            "chat_id": 42,
        ...
        return module, enrollment_target, fence_state

    def test_leftover_ack_resume_adopts_new_owner_with_pending_log_ids(self):
        previous = {
            key: os.environ.get(key)
            for key in (
                "OPENKAKAO_AUTO_REPLY_CLI",
        ...
        with tempfile.TemporaryDirectory() as temporary:
            try:
                module, enrollment_target, _ = self._leftover_resume_env(
                    temporary,
                    "auto_reply_leftover_ack_resume_pending_test",
                    None,
                )
                with mock.patch.object(module, "_cli_enrollment_target", return_value=enrollment_target):
                    adopted = module._state(
                        {
                            ...
                            "cursor_floor": 123,
                            "acked_watermark": 123,
                            "last_observed_log_id": 250,
                            "acked_log_ids": [123],
                            "observed_log_ids": [123, 250],
                            "pending_log_ids": [250],
                            "pending_gaps": [],
                            "candidate_phase": "idle",
                            "in_flight_candidate": None,
                ...
                self.assertNotEqual(adopted["fence_reason"], "reconcile_required")
            finally:
                for key, value in previous.items():
                    if value is None:
                        os.environ.pop(key, None)
    ...
    def test_leftover_ack_resume_adopts_ready_idle_orphaned_owner(self):
        previous = {
            key: os.environ.get(key)
            for key in (
                "OPENKAKAO_AUTO_REPLY_CLI",
        ...
        with tempfile.TemporaryDirectory() as temporary:
            try:
                module, enrollment_target, _ = self._leftover_resume_env(
                    temporary,
                    "auto_reply_leftover_ack_resume_ready_idle_test",
                    None,
                )
                with mock.patch.object(module, "_cli_enrollment_target", return_value=enrollment_target):
                    adopted = module._state(
                ...
                self.assertEqual(adopted["capability_state"], "starting")
                self.assertEqual(adopted["fence"], "starting")
                self.assertNotEqual(adopted.get("cursor_authority", {}).get("kind"), "stopped_clean_ack_replay")
            finally:
                for key, value in previous.items():
                    if value is None:
                        os.environ.pop(key, None)
    ...
    def test_leftover_ack_resume_adopts_orphaned_hooking_in_flight(self):
        previous = {
            key: os.environ.get(key)
            for key in (
                "OPENKAKAO_AUTO_REPLY_CLI",
        ...
        with tempfile.TemporaryDirectory() as temporary:
            try:
                module, enrollment_target, _ = self._leftover_resume_env(
                    temporary,
                    "auto_reply_leftover_ack_resume_hooking_test",
                    None,
                )
                with mock.patch.object(module, "_cli_enrollment_target", return_value=enrollment_target):
                    adopted = module._state(
                ...
                self.assertEqual(adopted["source_epoch"], 9)
                self.assertEqual(adopted["acked_watermark"], 123)
                self.assertIsNone(adopted["in_flight_candidate"])
    ...
    def test_leftover_ack_resume_accepts_two_row_strong_transcript(self):
        previous = {
            key: os.environ.get(key)
            for key in (
                "OPENKAKAO_AUTO_REPLY_CLI",
        ...
        with tempfile.TemporaryDirectory() as temporary:
            try:
                module, _, _ = self._leftover_resume_env(
                    temporary,
                    "auto_reply_leftover_ack_resume_two_row_test",
                    None,
                    identity={
                        "schema_version": 1,
                        "kind": "ax_transcript",
                ...
                enrollment_path = Path(temporary) / "enrollment.json"
                value = json.loads(enrollment_path.read_text(encoding="utf-8"))
                value["targets"][0]["last_log_id"] = 123
                raw = json.dumps(value, sort_keys=True).encode("utf-8")
                enrollment_path.write_bytes(raw)
                os.environ["OPENKAKAO_ENROLLMENT_SHA256"] = hashlib.sha256(raw).hexdigest()
                parsed = module._cli_enrollment_target()
                self.assertEqual(parsed["identity"]["matched_count"], 2)
                self.assertEqual(parsed["identity"]["matched_utf8_bytes"], 19)
                adopted = module._state(
                    {
                        "schema_version": 3,
                        "target_chat_id": 42,
    ...
    def test_leftover_ack_resume_rejects_in_flight_candidate(self):
        previous = {
            key: os.environ.get(key)
            for key in (
                "OPENKAKAO_AUTO_REPLY_CLI",
        ...
        with tempfile.TemporaryDirectory() as temporary:
            try:
                module, enrollment_target, _ = self._leftover_resume_env(
                    temporary,
                    "auto_reply_leftover_ack_resume_inflight_test",
                    None,
                )
                with mock.patch.object(module, "_cli_enrollment_target", return_value=enrollment_target):
                    rejected = module._state(
                        {
                            ...
                            "pending_gaps": [],
                            "candidate_phase": "idle",
                            "in_flight_candidate": {"event_id": "in-flight"},
                            "capability_state": "fenced",
                            "delivery_enabled": False,
                            "fence": "reconcile_required",
                            "fence_reason": "reconcile_required",
                ...
                self.assertEqual(rejected["owner_id"], "other-owner")
                self.assertEqual(rejected["fence"], "reconcile_required")
                self.assertEqual(rejected["capability_state"], "fenced")
            finally:
                for key, value in previous.items():
                    if value is None:
                        os.environ.pop(key, None)
    ...
    def test_cli_poll_uses_enrolled_id_without_enumerating_all_chats(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            try:
                spec = importlib.util.spec_from_file_location(
                    "auto_reply_db_enrolled_poll_test",
                    SCRIPTS / "auto-reply-db-watch.py",
                )
                db_watch = importlib.util.module_from_spec(spec)
                assert spec.loader is not None
    ...
    def test_message_summary_requires_positive_author_id_for_self(self):
        db_watch = self._load_db_watch_module("auto_reply_proven_self_summary")
        base = {
            "chat_id": 42,
            "log_id": 10,
            "author_id": 0,
        ...
        self.assertFalse(db_watch._message_summary(base)["is_self"])
        self.assertTrue(
    ...
    def test_db_watch_emits_only_content_bound_quoted_reply_pointer(self):
        db_watch = self._load_db_watch_module(
            "auto_reply_quoted_reply_descriptor_test"
        )
        source = {
            "chat_id": 42,
            "log_id": 200,
        ...
        auto_reply = self._load_auto_reply_module(
            "auto_reply_quoted_reply_envelope_bridge_test"
        )
        ...
        self.assertEqual(
            auto_reply._conversation_target(prepared, recent_evidence),
            {
                ...
                "source_author_nickname": "최연우",
                "source_author_id": 900,
                "source_message": "bounded source evidence",
                "source_message_type": 1,
                "directed_at_self": True,
            },
        )
    ...
    def test_db_watch_quoted_reply_descriptor_mismatch_fails_closed(self):
        db_watch = self._load_db_watch_module(
            "auto_reply_quoted_reply_descriptor_mismatch_test"
        )
        source = {
            "chat_id": 42,
            "log_id": 300,
    ...
    def test_db_poll_uses_numeric_self_and_enrolled_author_identity(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            try:
                db_watch = self._load_db_watch_module(
                    "auto_reply_numeric_identity_poll_test"
                )
    ...
    def test_burst_view_requires_contiguous_author_identity_and_safe_media(self):
        module = self._load_auto_reply_module("auto_reply_burst_view_test")
        ...
        self.assertEqual(
            module._prepare_burst_event(distinct)["burst_source_log_ids"],
            [106],
        )
        glue = self._burst_event(
            module,
            107,
            "어",
            1_001,
    ...
    def test_recent_conversation_excludes_single_current_row(self):
        module = self._load_auto_reply_module("auto_reply_recent_single_test")
    ...
    def test_recent_conversation_excludes_all_coalesced_burst_rows(self):
        module = self._load_auto_reply_module("auto_reply_recent_burst_test")
        ...
        second = self._burst_event(module, 123, "ㅋ", 1_201)
    ...
    def test_recent_conversation_deduplicates_and_fails_closed_on_bad_ids(self):
        module = self._load_auto_reply_module("auto_reply_recent_malformed_test")
    ...
    def test_quoted_reply_target_requires_exact_recent_evidence_binding(self):
        module = self._load_auto_reply_module(
            "auto_reply_quoted_reply_target_test"
        )
        ...
        expected = {
            ...
            "source_author_nickname": "최연우",
            "source_author_id": 900,
            "source_message": "source evidence",
            "source_message_type": 1,
            "directed_at_self": True,
        }
        ...
        self.assertIn("source evidence", json.dumps(expected))
        ...
        off_tail = json.loads(json.dumps(current))
        off_tail["recent_messages"] = [self._recent_row(current)]
        off_tail["quoted_source"] = {
        ...
        off_recent = module._recent_conversation(off_tail)
        self.assertTrue(any(row.get("quoted_source") for row in off_recent))
        off_target = module._conversation_target(off_tail, off_recent)
        self.assertEqual(off_target, expected)

    def test_analysis_passes_only_validated_direct_target_to_model(self):
        module = self._load_auto_reply_module(
            "auto_reply_quoted_reply_analysis_test"
        )
        now = int(time.time())
        source = self._burst_event(
            module,
    ...
    def test_analysis_prompt_contains_current_message_only_as_incoming(self):
        module = self._load_auto_reply_module("auto_reply_recent_prompt_test")
        now = int(time.time())
        previous = self._burst_event(
            module,
            140,
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            try:
                ...
                self.assertEqual(
                    ...
                    [
                        (first["event_id"], "pending"),
                        (second["event_id"], "pending"),
    ...
    def test_superseded_text_does_not_consume_successor_media(self):
        module = self._load_auto_reply_module("auto_reply_burst_media_owner_test")
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
    ...
    def test_supersession_race_linearizes_before_sending(self):
        module = self._load_auto_reply_module("auto_reply_burst_race_test")
        with tempfile.TemporaryDirectory() as temporary:
            module.QUEUE = Path(temporary) / "reply-queue.sqlite3"
        ...
        module = self._load_auto_reply_module("auto_reply_burst_send_wins_test")
        with tempfile.TemporaryDirectory() as temporary:
            module.QUEUE = Path(temporary) / "reply-queue.sqlite3"
    ...
    def test_gaussian_due_is_anchored_to_event_creation(self):
        module = self._load_auto_reply_module("auto_reply_burst_timing_test")
        self.assertEqual(
    ...
    def test_conversation_advancement_uses_double_read_authoritative_watermark(self):
        module = self._load_auto_reply_module("auto_reply_conversation_advanced_test")
    ...
    def test_analyze_event_keeps_recent_conversation_when_retrieval_fails(self):
        ...
        with (
            ...
            mock.patch.object(
                ...
                side_effect=module.RetrievalError("retrieval_command_failed"),
            ),
            mock.patch.object(module, "fetch_link_previews", return_value=[]),
            mock.patch.object(
                module,
    ...
    def test_process_schedules_from_incoming_message_creation(self):
        module = self._load_auto_reply_module("auto_reply_message_due_test")
        stats = self._timing_stats(module)
        now = int(time.time())
    ...
    def test_scheduled_question_restart_preserves_timing_without_resampling(self):
        module = self._load_auto_reply_module(
            "auto_reply_question_restart_timing_test"
        )
        now = int(time.time())
    ...
    def test_scheduled_stale_backlog_is_skipped_before_send(self):
        module = self._load_auto_reply_module("auto_reply_scheduled_stale_test")
        now = int(time.time())
    ...
    def test_formed_scheduled_reply_gives_up_after_pre_send_grace(self):
        module = self._load_auto_reply_module("auto_reply_formed_reply_grace_test")
        now = int(time.time())
        event = self._burst_event(module, 446, "old scheduled", now - 400)
        event["recent_messages"] = [self._recent_row(event)]
        prepared = module._prepare_burst_event(event)
        prepared["response_window_upper_seconds"] = 60.0
        with tempfile.TemporaryDirectory() as temporary:
    ...
    def test_formed_scheduled_reply_retries_within_pre_send_grace(self):
        module = self._load_auto_reply_module("auto_reply_formed_reply_within_grace_test")
        now = int(time.time())
        event = self._burst_event(module, 446, "old scheduled", now - 70)
        event["recent_messages"] = [self._recent_row(event)]
        prepared = module._prepare_burst_event(event)
        prepared["response_window_upper_seconds"] = 60.0
        with tempfile.TemporaryDirectory() as temporary:
            ...
            try:
                ...
                self.assertEqual(row["reply"], "아 그래도 아는 사람들이었네")
    ...
    def test_scheduled_presend_readiness_failure_is_safely_requeued(self):
        module = self._load_auto_reply_module(
            "auto_reply_scheduled_presend_requeue_test"
        )
        now = int(time.time())
    ...
    def test_scheduled_send_failure_requeues_only_before_sending_transition(self):
        module = self._load_auto_reply_module(
            "auto_reply_scheduled_send_phase_test"
        )
        now = int(time.time())
    ...
    def test_post_send_failure_with_successor_stays_delivery_unknown(self):
        module = self._load_auto_reply_module(
            "auto_reply_post_send_successor_fence_test"
        )
        now = int(time.time())
    ...
    def test_presend_requeue_cas_projects_concurrent_successor(self):
        module = self._load_auto_reply_module(
            "auto_reply_presend_requeue_race_test"
        )
        now = int(time.time())
    ...
    def test_processing_error_race_projects_new_successor(self):
        module = self._load_auto_reply_module("auto_reply_burst_error_race_test")
        with tempfile.TemporaryDirectory() as temporary:
            module.QUEUE = Path(temporary) / "reply-queue.sqlite3"
    ...
    def test_send_reply_enforces_laughter_policy_before_any_outbound_preflight(self):
        module = self._load_auto_reply_module(
            "auto_reply_laughter_presend_guard_test"
        )
    ...
    def test_send_reply_forwards_enrollment_path_and_digest_to_local_send(self):
        module = self._load_auto_reply_module("auto_reply_send_enrollment_env_test")
        captured = {"calls": []}
        ...
        self.assertIn("--reply-to", captured["calls"][1][0])
        self.assertIn("question", captured["calls"][1][0])
    ...
    def test_send_reply_retries_preflight_lock_contention_then_sends(self):
        module = self._load_auto_reply_module("auto_reply_preflight_retry_test")
        captured = {"preflight": 0, "send": 0}

        def fake_run(command, **_kwargs):
            if "--preflight" in command:
                captured["preflight"] += 1
                if captured["preflight"] < 3:
        ...
        with (
            ...
            mock.patch.object(module, "PRE_SEND_PREFLIGHT_RETRY_SECONDS", 0),
            mock.patch.object(module, "_run_bounded_process", side_effect=fake_run),
        ):
            self.assertTrue(
                module.send_reply(
        ...
        self.assertEqual(captured["preflight"], 3)
        self.assertEqual(captured["send"], 1)

    def test_send_reply_leftover_occupied_composer_still_sends(self):
        module = self._load_auto_reply_module("auto_reply_leftover_occupied_send")
        captured = {"preflight": 0, "send": 0}
        ...
        with tempfile.TemporaryDirectory() as temporary:
            module.QUEUE = Path(temporary) / "42" / "reply-queue.sqlite3"
            connection = self._worker_queue_connection(module)
            try:
                now = time.time()
                connection.execute(
                ...
                module._append_job_transition(
                    connection,
                    event["event_id"],
                    component="ax",
                    from_state="ready",
                    to_state="sending",
                    code="ax_mutation_authorized",
                )
                connection.commit()
                with (
                    mock.patch.object(module, "BIN", Path("/usr/bin/true")),
                ...
                self.assertEqual(captured["preflight"], 1)
                self.assertEqual(captured["send"], 1)
            finally:
                connection.close()
                if previous is None:
    ...
    def test_send_reply_occupied_composer_sends_without_prior_ax_mutation(self):
        module = self._load_auto_reply_module("auto_reply_occupied_send_no_ax")
        captured = {"preflight": 0, "send": 0}
    ...
    def test_parse_cli_json_object_accepts_pretty_printed_stdout(self):
        module = self._load_auto_reply_module("auto_reply_cli_json_parse_test")
        payload = {
            "chat_name": "부자멘토멘티",
            "status": "preflight_ready",
            "preflight_ready": True,
        ...
        pretty = json.dumps(payload, indent=2, ensure_ascii=False).encode()
        compact = json.dumps(payload, ensure_ascii=False).encode()
        self.assertEqual(module._parse_cli_json_object(pretty), payload)
        self.assertEqual(module._parse_cli_json_object(compact), payload)
        self.assertEqual(module._parse_cli_json_object(b"}"), None)
        self.assertEqual(module._parse_cli_json_object(b""), None)

    def test_send_reply_reverts_exact_no_mutation_failure_and_hides_reply_from_preflight(self):
        module = self._load_auto_reply_module("auto_reply_no_mutation_send_test")
    ...
    def test_send_reply_never_mutates_when_sending_journal_commit_fails(self):
        module = self._load_auto_reply_module("auto_reply_journal_send_fault_test")
    ...
    def test_post_mutation_journal_failure_is_immediately_delivery_unknown(self):
        module = self._load_auto_reply_module("auto_reply_post_send_journal_fault")
    ...
    def test_burst_projection_retries_audit_without_delivery_authority(self):
        module = self._load_auto_reply_module("auto_reply_burst_projection_retry_test")
        with tempfile.TemporaryDirectory() as temporary:
            module.QUEUE = Path(temporary) / "reply-queue.sqlite3"
    ...
    def test_stale_processing_with_successor_recovers_for_projection(self):
        module = self._load_auto_reply_module("auto_reply_burst_recovery_test")
        with tempfile.TemporaryDirectory() as temporary:
            module.QUEUE = Path(temporary) / "reply-queue.sqlite3"
    ...
    def test_stale_queue_heals_only_from_compatible_terminal_context(self):
        module = self._load_auto_reply_module(
            "auto_reply_terminal_projection_recovery_test"
        )
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            module.QUEUE = root / "reply-queue.sqlite3"
    ...
    def test_stale_processing_without_send_attempt_is_safely_requeued(self):
        module = self._load_auto_reply_module(
            "auto_reply_processing_recovery_test"
        )
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            module.QUEUE = root / "reply-queue.sqlite3"
    ...
    def test_stale_recovery_slow_context_update_does_not_block_journal_writer(self):
        module = self._load_auto_reply_module(
            "auto_reply_stale_recovery_unlocked_context_test"
        )
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            module.QUEUE = root / "reply-queue.sqlite3"
    ...
    def test_stale_recovery_context_snapshot_uses_exact_queue_cas(self):
        module = self._load_auto_reply_module(
            "auto_reply_stale_recovery_context_cas_test"
        )
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            module.QUEUE = root / "reply-queue.sqlite3"
    ...
    def test_stale_recovery_context_update_does_not_record_after_queue_race(self):
        module = self._load_auto_reply_module(
            "auto_reply_stale_recovery_post_context_cas_test"
        )
        with tempfile.TemporaryDirectory() as temporary:
            module.QUEUE = Path(temporary) / "reply-queue.sqlite3"
            queue = self._worker_queue_connection(module)
    ...
    def test_skip_projection_lost_ack_does_not_create_delivery_unknown(self):
        module = self._load_auto_reply_module(
            "auto_reply_skip_projection_lost_ack_test"
        )
    ...
    def test_strict_skip_projection_contract_rejects_ambiguous_terminal_rows(self):
        module = self._load_auto_reply_module(
            "auto_reply_strict_skip_projection_test"
        )
    ...
    def test_strict_skip_projection_never_reclassifies_a_sending_attempt(self):
        module = self._load_auto_reply_module(
            "auto_reply_sending_skip_projection_test"
        )
        with tempfile.TemporaryDirectory() as temporary:
            module.QUEUE = Path(temporary) / "reply-queue.sqlite3"
            queue = self._worker_queue_connection(module)
    ...
    def test_scheduled_skip_paths_use_strict_lost_ack_projection(self):
        module = self._load_auto_reply_module(
            "auto_reply_scheduled_strict_skip_test"
        )
        cases = (
    ...
    def test_stale_delivery_unknown_projection_is_idempotent_and_nonstarving(self):
        module = self._load_auto_reply_module(
            "auto_reply_terminal_unknown_recovery_test"
        )
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            module.QUEUE = root / "reply-queue.sqlite3"
    ...
    def test_delivery_unknown_heals_from_exact_sent_context_projection(self):
        module = self._load_auto_reply_module(
            "auto_reply_unknown_sent_projection_test"
        )
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            module.QUEUE = root / "reply-queue.sqlite3"
    ...
    def test_delivery_unknown_heals_only_exact_skipped_context_and_reopens(self):
        module = self._load_auto_reply_module(
            "auto_reply_unknown_skip_projection_test"
        )
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            module.QUEUE = root / "reply-queue.sqlite3"
    ...
    def test_reply_worker_health_is_private_and_reports_real_progress(self):
        module = self._load_auto_reply_module("auto_reply_worker_health_test")
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            root.chmod(0o700)
            room = root / "42"
    ...
    def test_untrusted_runner_defers_and_is_visible_in_worker_health(self):
        module = self._load_auto_reply_module(
            "auto_reply_untrusted_runner_health_test"
        )
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            root.chmod(0o700)
    ...
    def test_supervisor_rejects_stale_or_fenced_reply_worker_progress(self):
        supervisor = self._load_supervisor_module(
            "auto_reply_supervisor_worker_health_test"
        )
        now = time.time()
        status = {
            "schema_version": 1,
    ...
    def test_supervisor_ax_lease_covers_bounded_poll_and_fences_true_staleness(self):
        supervisor = self._load_supervisor_module(
            "auto_reply_supervisor_ax_heartbeat_lease_test"
        )
    ...
    def test_image_only_event_reaches_model_with_attested_media_evidence(self):
        module = self._load_auto_reply_module(
            "auto_reply_image_only_media_evidence_test"
        )
    ...
    def test_fresh_media_unavailable_clarification_uses_standard_scheduler(self):
        module = self._load_auto_reply_module(
            "auto_reply_media_unavailable_schedule_test"
        )
    ...
    def test_stale_media_unavailable_replay_is_terminal_skip(self):
        module = self._load_auto_reply_module(
            "auto_reply_media_unavailable_stale_test"
        )
    ...
    def test_media_unavailable_clarification_requires_exact_singleton_shape(self):
        module = self._load_auto_reply_module(
            "auto_reply_media_unavailable_shape_test"
        )
    ...
    def test_distinct_image_digests_and_fallback_prior_do_not_false_duplicate(self):
        module = self._load_auto_reply_module(
            "auto_reply_image_digest_duplicate_test"
        )
        with (
    ...
    def test_database_media_missing_or_invalid_never_uses_ax_or_model(self):
        module = self._load_auto_reply_module(
            "auto_reply_database_media_fail_closed_test"
        )
        environment = {
            module.DB_MODE_ENV: "database_authoritative",
            "OPENKAKAO_ALLOW_IMAGE_ANALYSIS": "1",
    ...
    def test_generate_reply_passes_ordered_multi_image_flags(self):
        with tempfile.TemporaryDirectory(dir=Path.home()) as temporary:
            root = Path(temporary)
            module, _runner = self._load_trusted_codex_module(
                "auto_reply_ordered_multi_image_test", root
            )
    ...
    def test_gjc_generate_reply_sends_prompt_on_stdin_with_at_images(self):
        with tempfile.TemporaryDirectory(dir=Path.home()) as temporary:
            root = Path(temporary)
            runner = root / "gjc"
            self._write_executable(runner, "#!/bin/sh\nexit 0\n")
            environment = {
                "OPENKAKAO_REPLY_RUNNER": str(runner),
                "OPENKAKAO_REPLY_RUNNER_KIND": "gjc",
                "OPENKAKAO_REPLY_RUNNER_SHA256": hashlib.sha256(runner.read_bytes()).hexdigest(),
                "OPENKAKAO_REPLY_MODEL": "omlx/Qwen3.6-35B-A3B-8bit",
                "OPENKAKAO_REPLY_REASONING_EFFORT": "high",
                "OPENKAKAO_REPLY_SERVICE_TIER": "default",
                "OPENKAKAO_REPLY_QUEUE": str(root / "reply-queue.sqlite3"),
            }
            with mock.patch.dict(os.environ, environment, clear=False):
                module = self._load_auto_reply_module("auto_reply_gjc_stdin_image_test")
    ...
    def test_image_bundle_manifest_is_complete_ordered_and_hash_bound(self):
        module = self._load_auto_reply_module(
            "auto_reply_image_bundle_manifest_test"
        )
        with self._owned_image_bundle(module, count=2, message_type=27) as media:
    ...
    def test_cleanup_media_bundle_removes_every_member_marker_and_directory(self):
        module = self._load_auto_reply_module(
            "auto_reply_image_bundle_cleanup_test"
        )
    ...
    def test_image_analysis_opt_in_disabled_skips_without_capture_or_model(self):
        module = self._load_auto_reply_module(
            "auto_reply_image_analysis_opt_in_test"
        )
        with self._owned_image_bundle(module) as media:
            event = self._burst_event(
                module,
    ...
    def test_unknown_image_hook_ack_cleans_bundle_and_fences_without_retry(self):
        module = self._load_db_watch_module(
            "auto_reply_unknown_image_ack_fence_test"
        )
    ...
    def test_self_author_unconfirmed_hook_ack_is_durable_skip(self):
        module = self._load_db_watch_module(
            "auto_reply_self_author_unconfirmed_ack_test"
        )
        message = {"chat_id": 42, "log_id": 11, "is_self": True, "message": "ㄷㄷ;"}
        with mock.patch.object(module, "emit", return_value=None):
            self.assertEqual(
                module._emit_with_ack_fence(
                    message, None, skip_reason="self_author"
                ),
                "skipped",
            )
        with mock.patch.object(module, "emit", side_effect=RuntimeError("hook down")):
            self.assertEqual(
                module._emit_with_ack_fence(
                    message, None, skip_reason="self_author"
                ),
        ...
        with mock.patch.object(module, "emit", return_value=None):
            with self.assertRaisesRegex(module.DbFence, "delivery_ack_uncertain"):
                module._emit_with_ack_fence(message, None)

    def test_parse_model_decision_coerces_decision_without_should_reply(self):
        module = self._load_auto_reply_module(
            "auto_reply_decision_without_should_reply_test"
        )
        parsed = module._parse_model_decision(
            {
                "decision": "REPLY",
                "reply": "쌈밥정식에 제육이랑 우렁된장 나오네",
                "reason": "menu photo",
            },
            {"recent:1", "media:abc"},
            bind_image_evidence=True,
        )
        self.assertIsNotNone(parsed)
        self.assertTrue(parsed["should_reply"])
        skipped = module._parse_model_decision(
            {"decision": "skip", "reply": None, "reason": "no ask"},
            {"recent:1"},
        )
    ...
    def test_delivery_unknown_removes_bundle_and_scrubs_durable_event(self):
        module = self._load_auto_reply_module(
            "auto_reply_delivery_unknown_media_scrub_test"
        )
        with tempfile.TemporaryDirectory() as temporary:
            module.QUEUE = Path(temporary) / "reply-queue.sqlite3"
            with self._owned_image_bundle(module) as media:
    ...
    def test_author_terminal_removes_bundle_and_scrubs_durable_event(self):
        module = self._load_auto_reply_module(
            "auto_reply_author_terminal_media_scrub_test"
        )
        with tempfile.TemporaryDirectory() as temporary:
            module.QUEUE = Path(temporary) / "reply-queue.sqlite3"
            with self._owned_image_bundle(module) as media:
    ...
    def test_image_reply_requires_exact_media_evidence(self):
        module = self._load_auto_reply_module(
            "auto_reply_required_media_evidence_test"
        )
        ...
        parsed_missing_citation = module._parse_model_decision(
            decision,
            supplied,
            required_evidence_ids={media_id},
        )
        self.assertIsNotNone(parsed_missing_citation)
        self.assertTrue(parsed_missing_citation["should_reply"])
        self.assertIn(media_id, parsed_missing_citation["evidence_ids"])
        decision["evidence_ids"] = ["recent:1", media_id]
        parsed_cited = module._parse_model_decision(
            decision,
            supplied,
            required_evidence_ids={media_id},
        )
        self.assertIsNotNone(parsed_cited)
        self.assertIn(media_id, parsed_cited["evidence_ids"])

    def test_parse_model_decision_binds_image_evidence_and_coerces_category(self):
        module = self._load_auto_reply_module(
            "auto_reply_bind_image_evidence_test"
        )
        media_id = f"media:{'b' * 64}"
        supplied = {"recent:1", media_id}
        long_reason = "menu-board-recommendation-for-two-or-more-people-plus-sides" * 2
        self.assertGreater(len(long_reason), 80)
        decision = {
            "should_reply": True,
            "reply": "**쌈밥정식**이 제육, 고등어구이까지 나와서 구성이 제일 알차고 보이네요! " * 3,
            "reason": long_reason,
            "category": "menu_recommendation",
            "evidence_ids": ["image-00.png"],
        }
        parsed_without_bind = module._parse_model_decision(
            decision,
            supplied,
            required_evidence_ids={media_id},
        )
        self.assertIsNotNone(parsed_without_bind)
        self.assertEqual(parsed_without_bind["category"], "information")
        self.assertIn(media_id, parsed_without_bind["evidence_ids"])
        parsed = module._parse_model_decision(
            decision,
            supplied,
            required_evidence_ids={media_id},
        ...
        self.assertIsNotNone(parsed)
        self.assertTrue(parsed["should_reply"])
        self.assertEqual(parsed["category"], "information")
        self.assertLessEqual(len(parsed["reply"]), 220)
        self.assertNotIn("**", parsed["reply"])
        self.assertEqual(parsed["evidence_ids"], [media_id])
        self.assertLessEqual(len(parsed["reason"]), 80)

    def test_parse_model_decision_accepts_gemini_minimal_image_payload(self):
        module = self._load_auto_reply_module(
            "auto_reply_gemini_minimal_image_parse_test"
        )
        media_id = f"media:{'c' * 64}"
        supplied = {"recent:1", media_id}
        parsed = module._parse_model_decision(
            {
                "should_reply": True,
                "reply": "오 쌈밥 맛있겠다! 제육이랑 우렁된장 나오네 ㅎㅎ",
            },
            supplied,
            required_evidence_ids={media_id},
            bind_image_evidence=True,
        ...
        self.assertIsNotNone(parsed)
        self.assertEqual(parsed["category"], "information")
        self.assertEqual(parsed["reason"], "image_reply")
        self.assertEqual(parsed["reply"], "오 쌈밥 맛있겠다! 제육이랑 우렁된장 나오네")
        self.assertEqual(parsed["evidence_ids"], [media_id])
        parsed_decision = module._parse_model_decision(
            {
                "should_reply": True,
                "decision": "REPLY",
                "reason": "상대방이 쌈밥집 메뉴판 사진을 공유했으므로 메뉴에 호응하며 쌈밥정식을 제안하는 자연스러운 답장을 작성합니다.",
                "reply": "오 맛있겠다! 쌈밥정식이랑 제육, 고등어구이까지 나오네",
            },
            supplied,
            required_evidence_ids={media_id},
            bind_image_evidence=True,
        )
        self.assertIsNotNone(parsed_decision)
        self.assertEqual(parsed_decision["category"], "information")
        self.assertTrue(parsed_decision["should_reply"])
        self.assertEqual(parsed_decision["evidence_ids"], [media_id])

        skipped = module._parse_model_decision(
            {
                "should_reply": True,
                "decision": "REPLY",
                "reason": "메뉴판만 왔고 질문은 없음",
                "reply": None,
            },
            supplied,
            required_evidence_ids={media_id},
        ...
        self.assertIsNotNone(skipped)
        self.assertFalse(skipped["should_reply"])
        self.assertEqual(skipped["reply"], "")
        self.assertEqual(skipped["evidence_ids"], [])

    def test_kakao_image_kind_masks_photo_flag_bits(self):
        module = self._load_auto_reply_module("auto_reply_kakao_image_kind_test")
        self.assertEqual(module.kakao_image_kind(2), 2)
        self.assertEqual(module.kakao_image_kind(16386), 2)
        self.assertEqual(module.kakao_image_kind(14), 14)
        self.assertIsNone(module.kakao_image_kind(1))
        self.assertIsNone(module.kakao_image_kind(16385))
        self.assertIsNone(module.kakao_image_kind(True))

    def test_parse_model_decision_accepts_gemini_skip_and_reply_without_media_ids(self):
        module = self._load_auto_reply_module(
            "auto_reply_gemini_flexible_image_parse_test"
        )
        supplied = {"recent:1"}
        skipped = module._parse_model_decision(
            {
                "should_reply": False,
                "reply": None,
                "reason": "사용자의 질문이나 요청 없이 식당 메뉴판 이미지만 전송되었으므로 응답하지 않습니다.",
            },
            supplied,
            bind_image_evidence=True,
        )
        self.assertIsNotNone(skipped)
        self.assertFalse(skipped["should_reply"])
        self.assertEqual(skipped["category"], "uncertain")
        self.assertEqual(skipped["evidence_ids"], [])
        parsed = module._parse_model_decision(
            {
                "should_reply": True,
        ...
        self.assertIsNotNone(parsed)
        self.assertTrue(parsed["should_reply"])
        self.assertEqual(parsed["evidence_ids"], ["recent:1"])
        self.assertEqual(parsed["category"], "information")

    def test_parse_model_decision_salvages_qwen_drafts_and_missing_category(self):
        module = self._load_auto_reply_module(
            "auto_reply_qwen_schema_salvage_test"
        )
        supplied = {"recent:3916029262503000064", "recent:3916020718949042176"}
        parsed = module._parse_model_decision(
            {
                "should_reply": True,
                "reply": "메뉴판 잘 보이네",
                ...
                "reason": "social_reply",
                "evidence_ids": [
                    "recent:3916020718949042176",
                    "recent:3916029262503000064",
                    "recent:3916029262503000064",
                ],
                "drafts": ["메뉴판 잘 보이네", "이 집 괜찮다", "사진 또 보내줘"],
            },
            supplied,
        )
        self.assertIsNotNone(parsed)
        self.assertTrue(parsed["should_reply"])
        self.assertEqual(parsed["reply"], "메뉴판 잘 보이네")
        self.assertEqual(
            parsed["evidence_ids"],
            ["recent:3916020718949042176", "recent:3916029262503000064"],
        )
        gemini = module._parse_model_decision(
            {
                "should_reply": True,
                "decision": "reply",
                "reply": "쌈밥정식에 제육이랑 우렁된장 나오네",
                "reason": "메뉴판 사진을 보고 대표 메뉴를 안내한다.",
            },
            {"recent:1"},
        )
        self.assertIsNotNone(gemini)
        self.assertTrue(gemini["should_reply"])
        self.assertEqual(gemini["category"], "information")
        self.assertEqual(gemini["evidence_ids"], ["recent:1"])
        skipped = module._parse_model_decision(
            {
                "should_reply": True,
                "reply": None,
                "reason": "질문 없이 메뉴판만 왔다",
            },
            {"recent:1"},
        )
        self.assertIsNotNone(skipped)
    ...
    def test_parse_model_decision_accepts_live_gemini_extra_keys(self):
        module = self._load_auto_reply_module(
            "auto_reply_live_gemini_extra_keys_test"
        )
        supplied = {"recent:1", "recent:3916029262503000064"}
        parsed = module._parse_model_decision(
            {
                "should_reply": True,
                "decision": "reply",
                "reply": "맛있는 쌈밥집 메뉴판이네요! 제육에 고등어구이, 우렁된장까지 나오는 쌈밥정식 맛있겠어요.",
                "reason": "사용자가 공유한 식당 메뉴판 사진을 확인하고 메뉴 구성과 주문 조건을 안내하는 자연스러운 답장을 작성합니다.",
            },
            supplied,
            inbound="사진",
            bind_image_evidence=True,
        ...
        self.assertTrue(parsed["should_reply"])
        self.assertEqual(parsed["category"], "information")
        self.assertTrue(parsed["reply"])

    def test_model_decision_candidates_salvages_truncated_gemini_reply(self):
        module = self._load_auto_reply_module(
            "auto_reply_truncated_gemini_salvage_test"
        )
        stdout = (
            '{"should_reply": true, "reason": "사용자가 공유한 식당 메뉴판 사진을 확인하고, '
            "쌈밥정식의 구성(제육, 고등어구이, 된장치개, 우렁된장)과 2인 이상 주문 조건에 "
            '맞춰 자연스럽고 친절한 카카오톡 답장을 생성합니다.", "reply": '
            '"쌈밥정식 구성(제육, 고등어구이, 된장치개, 우렁된장)이 아주 푸짐하고 맛있어 보'
        )
        candidates = module._model_decision_candidates(stdout)
        ...
        parsed = module._parse_model_decision(
            ...
            inbound="사진",
            bind_image_evidence=True,
            required_evidence_ids={"media:abc"},
        )
        self.assertIsNotNone(parsed)
        self.assertTrue(parsed["should_reply"])
        self.assertIn("쌈밥정식", parsed["reply"])
        self.assertIn("제육", parsed["reply"])
        self.assertEqual(parsed["evidence_ids"], ["media:abc"])

    def test_parse_model_decision_bind_image_uses_reaction_register_fallback(self):

        module = self._load_auto_reply_module(
            "auto_reply_gemini_reaction_register_parse_test"
        )
        supplied = {module.reaction_register()["evidence_id"]}
        parsed = module._parse_model_decision(
            {
                "should_reply": True,
                "reply": "오 쌈밥 맛있겠다! 제육이랑 우렁된장 나오네 ㅎㅎ",
        ...
        self.assertIsNotNone(parsed)
        self.assertTrue(parsed["should_reply"])
        self.assertEqual(parsed["evidence_ids"], [module.reaction_register()["evidence_id"]])
        self.assertEqual(parsed["category"], "information")

    def test_same_author_recent_photo_event_accepts_flagged_photo_type(self):
        module = self._load_auto_reply_module(
            "auto_reply_flagged_photo_predecessor_test"
        )
        event = {
            "author_id": 206894008,
            "log_id": 2,
            "chat_id": 475009326077605,
            "sent_at": 1787765865,
            "message": "ㅇㅇ?",
            "recent_messages": [
                {
        ...
        predecessor = module._same_author_recent_photo_event(event)
        self.assertIsNotNone(predecessor)
        self.assertEqual(predecessor["log_id"], 1)
        self.assertEqual(predecessor["message_type"], 16386)

    def test_parse_model_decision_bind_image_drops_unsupplied_required_ids(self):
        module = self._load_auto_reply_module(
            "auto_reply_bind_image_drop_required_test"
        )
        media_id = f"media:{'d' * 64}"
        supplied = {media_id, "recent:1"}
        parsed = module._parse_model_decision(
            {
                "should_reply": True,
                "reply": "쌈밥정식(16,000원)에 제육, 고등어구이, 된장치개, 우렁된장이까지 나와서 든든하겠네요! 전 메뉴 2인 이상 주문 가능하니 참고하세요~",
                "reason": "공유된 식당 메뉴판 이미지를 인식하여 대표 메뉴와 가격을 파악하고 답한다.",
            },
            supplied,
            required_evidence_ids={media_id, "recent:999"},
            inbound="사진",
            bind_image_evidence=True,
        )
        self.assertIsNotNone(parsed)
    ...
    def test_same_author_recent_photo_event_uses_streak_predecessor(self):
        module = self._load_auto_reply_module(
            "auto_reply_same_author_recent_photo_test"
        )
        event = {
            "author_id": 206894008,
            "log_id": 3916029288519475201,
            "chat_id": 475009326077605,
            "sent_at": 1787764269,
            "message": "ㅇㅇ?",
            "recent_messages": [
        ...
        with tempfile.TemporaryDirectory(dir=Path.home()) as temporary:
            ...
            with self._owned_image_bundle(module, count=1, message_type=2) as media:
                ...
                inner = {
                    "should_reply": True,
                    "reason": "공유된 식당 메뉴판 이미지를 인식하여 대표 메뉴와 가격을 파악하고 답한다.",
                    "reply": "쌈밥집 메뉴판이네요! 쌈밥정식은 제육이랑 우렁된장 나와요",
                }
                wrapped = {
                    "type": "result",
                    "result": "```json\n"
                ...
                self.assertEqual(result["evidence_ids"], [evidence_id])
                self.assertNotEqual(result.get("model_failure_class"), "invalid_output")

    def test_image_path_within_cap_accepts_system_tmp_when_tmpdir_differs(self):
        module = self._load_auto_reply_module("auto_reply_tmp_media_root_test")
        system_tmp = Path("/tmp").resolve(strict=True)
        directory = Path(
            tempfile.mkdtemp(prefix=module.MEDIA_DIR_PREFIX, dir=str(system_tmp))
        )
        try:
            directory.chmod(0o700)
            marker = directory / module.MEDIA_ACTIVE_MARKER
            ...
            image.chmod(0o600)
            self.assertTrue(module._path_is_direct_temp_child(directory))
            self.assertTrue(module._image_path_within_cap(image, marker))
        finally:
            for child in directory.iterdir():
                child.unlink()
            directory.rmdir()

    def test_generate_reply_structured_json_does_not_open_invalid_output_on_lease_loss(self):
        with tempfile.TemporaryDirectory(dir=Path.home()) as temporary:
            root = Path(temporary)
            runner = root / "gjc"
            self._write_executable(runner, "#!/bin/sh\nexit 0\n")
            ...
            with mock.patch.dict(os.environ, environment, clear=False):
                module = self._load_auto_reply_module(
                    "auto_reply_structured_json_lease_loss_test"
                )
            inner = {
                "should_reply": True,
                "reply": "확인해볼게",
                "reason": "answer",
                "category": "social",
                "evidence_ids": ["fabricated:2"],
            }
            stdout = (json.dumps(inner, ensure_ascii=False) + "\n").encode()
            ...
            with (
                ...
                mock.patch.object(
                    module, "_finish_model_call_success", return_value=False
                ),
                mock.patch.object(
                    module, "_run_bounded_process", side_effect=fake_run
                ),
                ...
                result = module.generate_reply("확인 좀", [], [], [], [])
            self.assertFalse(result["should_reply"])
            self.assertNotEqual(result.get("model_failure_class"), "invalid_output")
            self.assertEqual(result.get("model_failure_class"), "circuit_unavailable")

    def test_generate_reply_rc0_truncated_json_does_not_open_invalid_output(self):
        with tempfile.TemporaryDirectory(dir=Path.home()) as temporary:
            root = Path(temporary)
            runner = root / "gjc"
            self._write_executable(runner, "#!/bin/sh\nexit 0\n")
            ...
            with mock.patch.dict(os.environ, environment, clear=False):
                module = self._load_auto_reply_module(
                    "auto_reply_truncated_json_skip_test"
                )
            stdout = b"not-json [[[[ leftover"
            ...
            with (
                ...
                result = module.generate_reply("ㅇㅇ?", [], [], [], [])
            self.assertFalse(result["should_reply"])
            self.assertNotEqual(result.get("model_failure_class"), "invalid_output")
            self.assertEqual(result["reason"], "unparsed_model_decision")

    def test_recover_local_media_bundle_downloads_into_owned_dir(self):
        with tempfile.TemporaryDirectory() as temporary:
            runner = Path(temporary) / "cli"
            runner.write_text(
                "#!/bin/sh\n"
                "umask 077\n"
                'dir=""\n'
                ...
                "done\n"
                'printf "\\x89PNG\\r\\n\\x1a\\n" > "$dir/image-00.png"\n'
                'printf \'{"status":"ok","paths":["%s/image-00.png"]}\\n\' "$dir"\n',
                encoding="utf-8",
            )
            runner.chmod(0o700)
            before = self._media_dir_names()
            event = {
                "chat_id": 475009326077605,
                "log_id": 123,
                "author_id": 206894008,
            ...
            environment = {
                "OPENKAKAO_BINARY": str(runner),
                "OPENKAKAO_ALLOW_IMAGE_ANALYSIS": "1",
            }
            with mock.patch.dict(os.environ, environment, clear=False):
                module = self._load_auto_reply_module(
                    "auto_reply_media_recovery_ok_test"
                )
                paths = module._recover_local_media_bundle(event)
            self.assertTrue(paths)
            recovered = paths[0]
            self.assertTrue(recovered.is_file())
            self.assertTrue(recovered.parent.name.startswith(module.MEDIA_DIR_PREFIX))
            self.assertEqual(recovered.read_bytes(), b"\x89PNG\r\n\x1a\n")
            import stat as stat_module

            self.assertEqual(
                stat_module.S_IMODE(recovered.stat().st_mode) & 0o077, 0
            )
            after = self._media_dir_names()
            self.assertEqual(len(after - before), 1)

    def test_recover_local_media_bundle_cleans_up_on_failure(self):
        with tempfile.TemporaryDirectory() as temporary:
            runner = Path(temporary) / "cli"
            ...
            with mock.patch.dict(os.environ, environment, clear=False):
                module = self._load_auto_reply_module(
                    "auto_reply_media_recovery_fail_test"
                )
                self.assertIsNone(module._recover_local_media_bundle(event))
            self.assertEqual(self._media_dir_names(), before)

    @staticmethod
    def _media_dir_names() -> set[str]:
        import re as re_module

        return {
            name
            for name in os.listdir(tempfile.gettempdir())
            if re_module.match(r"^auto-reply-db-media-", name)
        }

    def test_db_watch_downloads_exact_local_author_bound_image_bundle(self):
        module = self._load_db_watch_module(
            "auto_reply_db_watch_local_image_bundle_test"
        )
    ...
    def test_db_watch_rejects_partial_manifest_and_cleans_the_bundle(self):
        module = self._load_db_watch_module(
            "auto_reply_db_watch_partial_image_bundle_test"
        )
    ...
    def test_db_watch_rejects_download_response_identity_mismatches(self):
        module = self._load_db_watch_module(
            "auto_reply_db_watch_image_identity_mismatch_test"
        )
    ...
    def test_db_watch_rejects_attachment_changed_between_poll_and_download(self):
        module = self._load_db_watch_module(
            "auto_reply_db_watch_image_attachment_drift_test"
        )
    ...
    def test_db_watch_forwards_byte_exact_poll_attachment_digest(self):
        module = self._load_db_watch_module(
            "auto_reply_db_watch_poll_attachment_digest_test"
        )
    ...
    def test_generate_reply_preserves_attested_database_media_marker(self):
        module = self._load_auto_reply_module("auto_reply_db_media_marker_test")
        image = Path("/tmp/openkakao-auto-reply-media-owned/1_image.png")
    ...
    def test_model_defer_keeps_owned_image_for_retry(self):
        module = self._load_auto_reply_module(
            "auto_reply_deferred_image_lifecycle_test"
        )
        with self._owned_image_bundle(module) as media:
            event = self._burst_event(
                module,
    ...
    def test_ephemeral_ax_image_is_cleaned_even_when_model_defers(self):
        module = self._load_auto_reply_module(
            "auto_reply_deferred_ax_image_lifecycle_test"
        )
        with tempfile.TemporaryDirectory() as temporary:
            captured = Path(temporary) / "auto-reply-ax-image-owned.png"
    ...
    def test_db_watch_service_commands_receive_eof_on_stdin(self):
        db_watch = self._load_db_watch_module("auto_reply_db_stdin_test")
        with tempfile.TemporaryDirectory() as temporary:
...
""",
    ...
    def test_supervisor_service_child_receives_eof_on_stdin(self):
        supervisor = self._load_supervisor_module("auto_reply_supervisor_stdin_test")
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
...
""",
    ...
    def test_db_watch_hook_keeps_payload_stdin_pipe(self):
        db_watch = self._load_db_watch_module("auto_reply_hook_stdin_test")
        interpreter = Path(sys.executable).resolve()
        for version in ((3, 11), (3, 12), (3, 13)):
            self.assertTrue(
...
""",
    ...
    def test_bounded_cli_background_group_is_not_stopped_by_sigttin(self):
        ...
        db_watch = self._load_db_watch_module("auto_reply_sigttin_test")
        with tempfile.TemporaryDirectory() as temporary:
            probe = Path(temporary) / "stdin-probe"
            self._write_executable(
                probe,
...
""",
    ...
    def test_polled_state_bootstraps_under_running_owner_lease(self):
        spec = importlib.util.spec_from_file_location(
            "auto_reply_db_bootstrap_save_test",
            SCRIPTS / "auto-reply-db-watch.py",
        )
        db_watch = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
    ...
    def test_heartbeat_save_skips_generation_lock(self):
        spec = importlib.util.spec_from_file_location(
            "auto_reply_db_heartbeat_save_test",
            SCRIPTS / "auto-reply-db-watch.py",
        )
        db_watch = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
    ...
    def test_save_state_persists_owner_only_mode(self):
        spec = importlib.util.spec_from_file_location(
            "auto_reply_db_save_state_mode_test",
            SCRIPTS / "auto-reply-db-watch.py",
        )
        db_watch = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        ...
        try:
            ...
            self.assertEqual(stat.S_IMODE(db_watch.STATE.stat().st_mode), 0o600)
        finally:
    ...
    def test_proactive_topic_uses_vector_tokens_and_skips_when_conversation_advanced(self):
        ...
        unknown_again = module.maybe_enqueue_proactive_topic(
            connection,
            now=now + 40_000,
            response_time=stats,
            style_profile=style,
            last_observed_sent_at=int(now) - 40_000,
            conversation_advanced=False,
            source_event=source,
            enqueue=enqueue,
            search=search,
        )
        self.assertIsNone(unknown_again)
        self.assertEqual(len(queued), 1)

    def test_silence_source_uses_last_authorized_inbound_after_self_tail(self):
        module = self._load_auto_reply_module("auto_reply_silence_self_tail_test")
        module.CHAT = "부자멘토멘티"
        with tempfile.TemporaryDirectory() as temporary:
    ...
    def test_silence_source_falls_back_to_context_authorized_inbound(self):
        module = self._load_auto_reply_module("auto_reply_silence_context_fallback_test")
        module.CHAT = "부자멘토멘티"
        with tempfile.TemporaryDirectory() as temporary:
            state_path = Path(temporary) / "db-watch-state.json"
            payload = {
    ...
    def test_empty_tail_uses_context_inbound_as_silence_not_advanced(self):
        module = self._load_auto_reply_module("auto_reply_empty_tail_silence_test")
        module.CHAT = "부자멘토멘티"
        with tempfile.TemporaryDirectory() as temporary:
            state_path = Path(temporary) / "db-watch-state.json"
    ...
    def test_geeknews_daily_slots_block_repeat_posts(self):
        module = self._load_auto_reply_module("auto_reply_geeknews_slots_test")
        with tempfile.TemporaryDirectory() as temporary:
            module.QUEUE = Path(temporary) / "reply-queue.sqlite3"
    ...
    def test_geeknews_digest_posts_unseen_feed_items_once(self):
        ...
        module = self._load_auto_reply_module("auto_reply_geeknews_digest_test")
        with tempfile.TemporaryDirectory() as temporary:
            module.QUEUE = Path(temporary) / "reply-queue.sqlite3"
            feed = """
            <feed>
    ...
    def test_geeknews_enqueue_does_not_persist_seen_until_confirmed(self):
        module = self._load_auto_reply_module("auto_reply_geeknews_confirm_seen")
        with tempfile.TemporaryDirectory() as temporary:
            module.QUEUE = Path(temporary) / "reply-queue.sqlite3"
            feed = """
            <feed>
    ...
    def test_geeknews_operator_preview_does_not_persist_seen(self):
        module = self._load_auto_reply_module("auto_reply_geeknews_operator_preview")
        with tempfile.TemporaryDirectory() as temporary:
            module.QUEUE = Path(temporary) / "reply-queue.sqlite3"
            feed = """
            <feed>
    ...
    def test_pre_send_model_temporarily_unavailable_unknown_reopens_pending(self):
        module = self._load_auto_reply_module("auto_reply_model_retry_leftover_heal")
        previous = os.environ.get("OPENKAKAO_TARGET_CHAT_ID")
        os.environ["OPENKAKAO_TARGET_CHAT_ID"] = "42"
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            ...
            try:
                ...
                queue.execute(
                    ...
                    INSERT INTO reply_jobs(
                        ...
                        ?, NULL, 'model_temporarily_unavailable', 'uncertain', NULL, NULL,
                        'model_invalid_output', 1.0, 1.0, 2
                    )
                    """,
                    (due,),
                )
                ...
                with mock.patch.object(module, "complete_event") as complete:
                    module.recover_stale_jobs(queue)
                row = queue.execute(
                    "SELECT status,decision,reason,due_at,reply FROM reply_jobs WHERE event_id='db:42:11'"
                ).fetchone()
                self.assertEqual(row["status"], "pending")
                self.assertIsNone(row["decision"])
                self.assertEqual(row["reason"], "model_temporarily_unavailable")
    ...
    def test_pre_send_model_temporarily_unavailable_unknown_gives_up_after_retries(
        self,
    ):
        module = self._load_auto_reply_module(
            "auto_reply_model_retry_leftover_give_up"
        )
        previous = os.environ.get("OPENKAKAO_TARGET_CHAT_ID")
        os.environ["OPENKAKAO_TARGET_CHAT_ID"] = "42"
        with tempfile.TemporaryDirectory() as temporary:
            ...
            try:
                queue.execute(
                    ...
                    INSERT INTO reply_jobs(
                        ...
                        'db:42:12', '{"chat_id":42,"log_id":12,"message":"사진"}',
                        'delivery_unknown',
                        ?, NULL, 'model_temporarily_unavailable', 'uncertain', NULL, NULL,
                        'model_invalid_output', 1.0, 1.0, 3
                    )
                    """,
                    (time.time() + 120.0,),
                )
                ...
                with mock.patch.object(module, "complete_event") as complete:
                    module.recover_stale_jobs(queue)
                row = queue.execute(
                    "SELECT status,decision,reason,reply FROM reply_jobs WHERE event_id='db:42:12'"
                ).fetchone()
                self.assertEqual(row["status"], "skipped")
                self.assertEqual(row["decision"], "skip")
                self.assertEqual(row["reason"], "stale_backlog")
                self.assertIsNone(row["reply"])
                complete.assert_called_once_with("db:42:12", "")
            finally:
                queue.close()
                if previous is None:
                    os.environ.pop("OPENKAKAO_TARGET_CHAT_ID", None)
    ...
    def test_pre_send_retrieval_failed_unknown_heals_to_skip_without_retransmit(self):
        module = self._load_auto_reply_module("auto_reply_retrieval_failed_leftover_heal")
        previous = os.environ.get("OPENKAKAO_TARGET_CHAT_ID")
        os.environ["OPENKAKAO_TARGET_CHAT_ID"] = "42"
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            ...
            try:
                queue.execute(
                    ...
                    INSERT INTO reply_jobs(
                        ...
                        'db:42:3910758421842413569', '{"chat_id":42,"log_id":3910758421842413569}',
                        'delivery_unknown', NULL, 'skip', 'retrieval_command_failed',
                        'uncertain', NULL, NULL, 'reconcile_required', 1.0, 1.0, 3
                    )
                    """
                )
                queue.commit()
                ...
                self.assertFalse(
                    module.leftover_unknown_has_ax_mutation(
                        queue, "db:42:3910758421842413569"
                    )
                )
                with mock.patch.object(module, "complete_event") as complete:
                    module.recover_stale_jobs(queue)
                row = queue.execute(
                    ...
                    "WHERE event_id='db:42:3910758421842413569'"
                ).fetchone()
                self.assertEqual(
                    tuple(row),
                    ("skipped", "skip", "retrieval_command_failed", None),
                )
                complete.assert_called_once_with("db:42:3910758421842413569", "")
            finally:
                queue.close()
                if previous is None:
                    os.environ.pop("OPENKAKAO_TARGET_CHAT_ID", None)
    ...
    def test_pre_send_blank_unknown_reopens_pending_without_retransmit(self):
        module = self._load_auto_reply_module("auto_reply_blank_unknown_leftover_heal")
        previous = os.environ.get("OPENKAKAO_TARGET_CHAT_ID")
        os.environ["OPENKAKAO_TARGET_CHAT_ID"] = "42"
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
    ...
    def test_empty_reconcile_required_unknown_give_up_unfences_without_retransmit(self):
        module = self._load_auto_reply_module("auto_reply_empty_reconcile_give_up")
        previous = os.environ.get("OPENKAKAO_TARGET_CHAT_ID")
        os.environ["OPENKAKAO_TARGET_CHAT_ID"] = "42"
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            module.QUEUE = root / "42" / "reply-queue.sqlite3"
            module.QUEUE.parent.mkdir(parents=True)
            queue = self._worker_queue_connection(module)
            try:
                queue.execute(
                    """
                    INSERT INTO reply_jobs(
                        event_id,event_json,status,due_at,decision,reason,category,
                        reply,scheduled_delay_seconds,error_class,created_at,updated_at,attempt_no
                    ) VALUES(
                        'db:42:11', '{}', 'delivery_unknown', NULL, NULL,
                        'reconcile_required', NULL, '', NULL,
                        'reconcile_required', 1.0, 1.0, 1
                    )
                    """
                )
                queue.commit()
                self.assertEqual(module._queue_reconciliation_blockers(queue), 0)
                released = module._auto_reconcile_give_up(queue, now=1.0 + module.RECONCILE_GIVE_UP_SECONDS)
                self.assertEqual(released, 1)
                row = queue.execute(
                    "SELECT status, reason, reply FROM reply_jobs WHERE event_id='db:42:11'"
                ).fetchone()
                self.assertEqual(tuple(row), ("skipped", "reconcile_gave_up", ""))
                self.assertEqual(module._queue_reconciliation_blockers(queue), 0)
            finally:
                queue.close()
                if previous is None:
    ...
    def test_identity_policy_blank_unknown_skips_without_retransmit(self):
        module = self._load_auto_reply_module("auto_reply_identity_unknown_skip")
        previous = os.environ.get("OPENKAKAO_TARGET_CHAT_ID")
        os.environ["OPENKAKAO_TARGET_CHAT_ID"] = "42"
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            ...
            try:
                now = time.time()
                event = self._burst_event(module, 11, "WTI", int(now), author="드리고", author_id=356939701)
                queue.execute(
                    "INSERT INTO reply_jobs("
                    "event_id,event_json,status,due_at,decision,reason,category,reply,"
                    "scheduled_delay_seconds,error_class,created_at,updated_at,attempt_no"
                ...
                self.assertEqual(tuple(row), ("skipped", "skip", "author_not_allowlisted", None))
    ...
    def test_high_attempt_blank_unknown_skips_without_retransmit(self):
        module = self._load_auto_reply_module("auto_reply_blank_unknown_high_attempt_skip")
        previous = os.environ.get("OPENKAKAO_TARGET_CHAT_ID")
        os.environ["OPENKAKAO_TARGET_CHAT_ID"] = "42"
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
    ...
    def test_high_attempt_formed_unknown_skips_without_retransmit(self):
        module = self._load_auto_reply_module("auto_reply_formed_unknown_high_attempt_skip")
        previous = os.environ.get("OPENKAKAO_TARGET_CHAT_ID")
        os.environ["OPENKAKAO_TARGET_CHAT_ID"] = "42"
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            try:
                ...
                self.assertEqual(tuple(row), ("skipped", "skip", "stale_backlog", "ㄹㅇ 쉽지 않네"))
                complete.assert_called_once_with("db:42:11", "ㄹㅇ 쉽지 않네")
            finally:
                queue.close()
                if previous is None:
                    os.environ.pop("OPENKAKAO_TARGET_CHAT_ID", None)
    ...
    def test_high_attempt_ax_authorized_unknown_skips_without_retransmit(self):
        module = self._load_auto_reply_module(
            "auto_reply_high_attempt_ax_authorized_unknown_skip"
        )
        previous = os.environ.get("OPENKAKAO_TARGET_CHAT_ID")
        os.environ["OPENKAKAO_TARGET_CHAT_ID"] = "42"
        event = {"chat_id": 42, "log_id": 11, "author_nickname": "현준", "is_self": False}
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            module.QUEUE = root / "42" / "reply-queue.sqlite3"
            queue = self._worker_queue_connection(module)
            try:
                ...
                queue.commit()
                self.assertTrue(module.leftover_unknown_has_ax_mutation(queue, "db:42:11"))
                self.assertFalse(
                    module.leftover_unknown_has_local_db_confirmed(queue, "db:42:11")
                )
                self.assertEqual(module._queue_reconciliation_blockers(queue), 1)
                with mock.patch.object(
                    module, "_confirmed_self_reply_log_id", return_value=None
                ), mock.patch.object(module, "complete_event") as complete:
                    module.recover_stale_jobs(queue)
                    self.assertEqual(module._queue_reconciliation_blockers(queue), 0)
                row = queue.execute(
                ...
                self.assertEqual(
                    tuple(row),
                    ("skipped", "skip", "stale_backlog", "아 맞네요, 그게 그런 거였군요"),
                )
                complete.assert_called_once_with(
                    "db:42:11", "아 맞네요, 그게 그런 거였군요"
                )
            finally:
                queue.close()
                if previous is None:
    ...
    def test_formed_ordinary_unknown_reschedules_without_retransmit(self):
        module = self._load_auto_reply_module("auto_reply_formed_ordinary_unknown_heal")
    ...
    def test_ax_confirmed_unknown_heals_to_sent_without_retransmit(self):
        module = self._load_auto_reply_module("auto_reply_ax_confirmed_unknown_sent")
        previous = os.environ.get("OPENKAKAO_TARGET_CHAT_ID")
        os.environ["OPENKAKAO_TARGET_CHAT_ID"] = "42"
        digest = "GeekNews TOP5 · 2026-08-19 12:23 KST\n\n1. example"
        event = {
            "chat_id": 42,
            "log_id": 1787110980,
            "proactive": True,
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            try:
                ...
                queue.commit()
                self.assertTrue(module.leftover_unknown_has_ax_mutation(queue, "db:42:1787110980"))
                self.assertTrue(
                    module.leftover_unknown_has_local_db_confirmed(queue, "db:42:1787110980")
                )
                self.assertEqual(module._queue_reconciliation_blockers(queue), 0)
                with mock.patch.object(
                    module, "_confirmed_self_reply_log_id", return_value=None
                ...
                row = queue.execute(
                    "SELECT status,decision,reason,reply FROM reply_jobs "
                    "WHERE event_id='db:42:1787110980'"
                ).fetchone()
                self.assertEqual(tuple(row), ("sent", "reply", "geeknews_rss", digest))
                complete.assert_called_once_with("db:42:1787110980", digest)
            finally:
                queue.close()
                if previous is None:
                    os.environ.pop("OPENKAKAO_TARGET_CHAT_ID", None)
    ...
    def test_ax_authorized_unknown_without_confirm_reschedules_same_text(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            try:
                ...
                queue.commit()
                self.assertTrue(module.leftover_unknown_has_ax_mutation(queue, "db:42:11"))
                self.assertFalse(module.leftover_unknown_has_local_db_confirmed(queue, "db:42:11"))
                with mock.patch.object(
                    module, "_confirmed_self_reply_log_id", return_value=None
                ), mock.patch.object(module, "complete_event") as complete:
                    module.recover_stale_jobs(queue)
    ...
    def test_empty_stale_backlog_unknown_skips_without_blocking(self):
        module = self._load_auto_reply_module("auto_reply_empty_stale_unknown_heal")
        previous = os.environ.get("OPENKAKAO_TARGET_CHAT_ID")
        os.environ["OPENKAKAO_TARGET_CHAT_ID"] = "42"
        event = {"chat_id": 42, "log_id": 11, "author_nickname": "현준"}
        with tempfile.TemporaryDirectory() as temporary:
    ...
    def test_proactive_unix_unknown_heals_to_stale_backlog_without_retransmit(self):
        module = self._load_auto_reply_module("auto_reply_proactive_unix_leftover_heal")
        previous = os.environ.get("OPENKAKAO_TARGET_CHAT_ID")
        os.environ["OPENKAKAO_TARGET_CHAT_ID"] = "42"
        event = {
            "chat_id": 42,
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            try:
                ...
                module._geeknews_slot_at = lambda stamp: None
                with mock.patch.object(module, "complete_event") as complete:
                    module.recover_stale_jobs(queue)
                row = queue.execute(
                    "SELECT status,decision,reason,reply FROM reply_jobs "
    ...
    def test_formed_proactive_unix_unknown_skips_without_occupying_next_slot(self):
        module = self._load_auto_reply_module(
            "auto_reply_formed_proactive_unix_leftover_heal"
        )
        previous = os.environ.get("OPENKAKAO_TARGET_CHAT_ID")
        os.environ["OPENKAKAO_TARGET_CHAT_ID"] = "42"
        digest = "GeekNews TOP5 · 2026-08-19 08:49 KST\n\n1. example"
        event = {
            "chat_id": 42,
            "log_id": 1787096940,
            "proactive": True,
            ...
            "proactive_source_log_id": 3910246708481685505,
            "sent_at": 1787096940,
            "message": digest,
        }
        with tempfile.TemporaryDirectory() as temporary:
            ...
            try:
                queue.execute(
                    ...
                    INSERT INTO reply_jobs(
                        ...
                        'db:42:1787096940', ?, 'delivery_unknown',
                        NULL, 'reply', 'geeknews_rss', 'proactive', ?, NULL,
                        'reconcile_required', 1.0, 1.0
                    )
                    """,
                    (json.dumps(event, ensure_ascii=False), digest),
                )
                queue.commit()
                self.assertEqual(module._queue_reconciliation_blockers(queue), 1)
                module._geeknews_slot_at = lambda stamp: None
                ...
                row = queue.execute(
                    "SELECT status,decision,reason,reply FROM reply_jobs "
                    "WHERE event_id='db:42:1787096940'"
                ).fetchone()
                self.assertEqual(
                    tuple(row),
                    ("skipped", "skip", "stale_backlog", digest),
                )
                complete.assert_called_once_with("db:42:1787096940", digest)
                self.assertEqual(module._queue_reconciliation_blockers(queue), 0)
            finally:
                queue.close()
                if previous is None:
    ...
    def test_proactive_real_tail_unknown_heals_to_stale_backlog_without_retransmit(self):
        module = self._load_auto_reply_module("auto_reply_proactive_real_tail_leftover")
        previous = os.environ.get("OPENKAKAO_TARGET_CHAT_ID")
        os.environ["OPENKAKAO_TARGET_CHAT_ID"] = "42"
        event = {
            "chat_id": 42,
    ...
    def test_proactive_process_job_defines_analysis_event(self):
        module = self._load_auto_reply_module("auto_reply_proactive_analysis_event")
        previous = os.environ.get("OPENKAKAO_TARGET_CHAT_ID")
        os.environ["OPENKAKAO_TARGET_CHAT_ID"] = "42"
    ...
    def test_in_slot_proactive_unknown_retries_instead_of_stale_skip(self):
        module = self._load_auto_reply_module("auto_reply_proactive_in_slot_retry")
        previous = os.environ.get("OPENKAKAO_TARGET_CHAT_ID")
        os.environ["OPENKAKAO_TARGET_CHAT_ID"] = "42"
        now = 1_787_052_957
        event = {
            "chat_id": 42,
            "log_id": 3910049572027543553,
            "proactive": True,
            "sent_at": now,
            "author_nickname": "문승현",
            "proactive_source_log_id": 3910049014299035649,
            ...
            try:
                queue.execute(
                    ...
                    INSERT INTO reply_jobs(
                        ...
                        'db:42:1787052957', ?, 'delivery_unknown',
                        NULL, NULL, NULL, NULL, NULL, NULL,
                        'reconcile_required', 1.0, 1.0
                    )
                    """,
                ...
                queue.commit()
                module._geeknews_slot_at = lambda stamp: ("2026-08-18", "evening")
                with mock.patch.object(module, "complete_event") as complete:
                    module.recover_stale_jobs(queue)
                row = queue.execute(
                    "SELECT status,decision,reason,reply FROM reply_jobs "
                    "WHERE event_id='db:42:1787052957'"
                ).fetchone()
                self.assertEqual(row["status"], "pending")
    ...
    def test_proactive_process_job_schedules_without_response_time_stats(self):
        module = self._load_auto_reply_module("auto_reply_proactive_no_response_time")
        previous = os.environ.get("OPENKAKAO_TARGET_CHAT_ID")
        os.environ["OPENKAKAO_TARGET_CHAT_ID"] = "42"
        event = self._burst_event(
            module, 120, "GeekNews TOP5 · 2026-08-18 20:35 KST", int(time.time())
        )
        event["event_id"] = "db:42:1787052957"
        event["canonical_event_id"] = "db:42:1787052957"
        event["proactive"] = True
        event["proactive_source_log_id"] = 99
        event["author_nickname"] = "문승현"
        event["author_id"] = 7
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            try:
                ...
                with (
                    ...
                    mock.patch.object(module, "privacy_attestation_current", return_value=True),
                    mock.patch.object(module, "numeric_author_identity_status", return_value="allowed"),
                    mock.patch.object(module, "sample_response_delay_for_analysis") as sample,
                    mock.patch.object(module, "record_context_decision") as record,
                    mock.patch.object(module, "analyze_event") as analyze,
                ):
                    module.process_job(job, "pending", queue)
                ...
                analyze.assert_not_called()
                row = queue.execute(
                    "SELECT status,decision,reason,category,reply,scheduled_delay_seconds "
                    "FROM reply_jobs WHERE event_id=?",
                    (event["event_id"],),
                ).fetchone()
                self.assertEqual(row["status"], "scheduled")
                ...
                self.assertEqual(row["reason"], "geeknews_rss")
                self.assertEqual(row["category"], "proactive")
                self.assertIn("GeekNews TOP5", row["reply"])
                self.assertEqual(row["scheduled_delay_seconds"], module.MIN_REPLY_DELAY_SECONDS)
            finally:
                queue.close()
                if previous is None:
                    os.environ.pop("OPENKAKAO_TARGET_CHAT_ID", None)
    ...
    def test_geeknews_digest_skips_ordinary_draft_policy(self):
        module = self._load_auto_reply_module("auto_reply_geeknews_draft_policy")
        digest = (
            "GeekNews TOP5 · 2026-08-20 08:32 KST\n\n"
            "1. 7,704명 연구에서 원격 근무자의 웰빙이 가장 높게 나타남 "
            "https://news.hada.io/topic?id=32675\n\n"
            "2. OpenRouter가 Stripe에 합류함 https://news.hada.io/topic?id=32674\n\n"
            "3. 시민 위생 — 경찰국가에 악용될 기술을 만들지 말 것 (2013) "
            "https://news.hada.io/topic?id=32673\n\n"
            "4. Moderna, 흑색종 mRNA 신생항원 치료제 최초의 긍정적 3상 결과 발표 "
            "https://news.hada.io/topic?id=32672\n\n"
            "5. 농담으로 산 도메인이 지정학적 전쟁에 휘말린 과정 "
            "https://news.hada.io/topic?id=32671"
        )
        self.assertGreater(len(" ".join(digest.split())), 220)
        self.assertFalse(module._policy_valid_draft(digest, digest))
        self.assertTrue(
            module._constructed_geeknews_outbound(
                {"proactive_query": "geeknews-rss", "message": digest},
                {"reason": "geeknews_rss"},
            )
        )
        self.assertTrue(
        ...
        self.assertTrue(
            module._constructed_geeknews_outbound(
                {"proactive": True, "message": digest},
                {"reason": None},
            )
        )

    def test_scheduled_geeknews_digest_is_not_laughter_skipped(self):
        module = self._load_auto_reply_module("auto_reply_geeknews_scheduled_policy")
        previous = os.environ.get("OPENKAKAO_TARGET_CHAT_ID")
        os.environ["OPENKAKAO_TARGET_CHAT_ID"] = "42"
        digest = (
            "GeekNews TOP5 · 2026-08-20 08:32 KST\n\n"
        ...
        now = int(time.time())
        event = self._burst_event(module, 120, digest, now)
        event["event_id"] = "db:42:1787182321"
        event["canonical_event_id"] = "db:42:1787182321"
        event["proactive"] = True
                ...
                try:
                    connection.execute(
                        ...
                        INSERT INTO reply_jobs(
                            ...
                            ?, ?, 'scheduled', ?, 'reply', 'geeknews_rss', 'proactive',
                            ?, 0.4, NULL, 1.0, 1.0
                        )
                        """,
                        (
                    ...
                    with (
                        ...
                        mock.patch.object(
                            module,
                            "_proactive_unknown_is_closed_slot",
                            return_value=False,
                        ),
                        mock.patch.object(module, "send_reply") as sender,
                        mock.patch.object(
                    ...
                    self.assertNotEqual(
                        row["reason"], "reply_laughter_policy_violation"
                    )
                    self.assertEqual(row["decision"], "reply")
                    self.assertEqual(row["reason"], "geeknews_rss")
                    self.assertEqual(row["category"], "proactive")
    ...
    def test_geeknews_skipped_attempt_occupies_current_slot(self):
        ...
        connection = sqlite3.connect(":memory:")
        connection.execute(
            "CREATE TABLE reply_jobs(event_id TEXT, event_json TEXT, status TEXT, reason TEXT)"
        )
        queued = []
        morning = datetime(2026, 8, 16, 8, 40, tzinfo=module.GEEKNEWS_SLOT_TZ)
        start, _end = module._geeknews_slot_window(
            morning.date(), "morning", 8, 40, 20
        )
        inside = start + 60
        digest_event = {
            "event_id": "db:42:%s" % int(inside),
            "canonical_event_id": "db:42:%s" % int(inside),
            "chat_id": 42,
            "log_id": 99,
            "author_id": 7,
            "author_nickname": "문승현",
            ...
            "proactive_query": "geeknews-rss",
            "sent_at": int(inside),
            "message": "GeekNews TOP5",
        }
        connection.execute(
            "INSERT INTO reply_jobs VALUES (?, ?, ?, ?)",
            (
                digest_event["event_id"],
                json.dumps(digest_event, ensure_ascii=False),
                "skipped",
                None,
            ),
        )
        source = {
            ...
            "reply_authorized": True,
            "message": "요즘 세금 어때",
            "sent_at": int(inside) - 10_000,
        }
        again = module.maybe_enqueue_proactive_topic(
            connection,
            now=inside,
            response_time=stats,
            style_profile={"common_tokens_json": "{}"},
            last_observed_sent_at=int(inside) - 10_000,
            conversation_advanced=False,
            source_event=source,
        ...
        self.assertIsNone(again)
        self.assertEqual(queued, [])
        lunch_start, _ = module._geeknews_slot_window(
            morning.date(), "lunch", 12, 35, 15
        )
        later = module.maybe_enqueue_proactive_topic(
            connection,
            now=lunch_start + 10,
            response_time=stats,
            style_profile={"common_tokens_json": "{}"},
            last_observed_sent_at=int(lunch_start) - 10_000,
            conversation_advanced=False,
            source_event=source,
            enqueue=lambda event: queued.append(event) or True,
            search=lambda query: [
    ...
    def test_in_slot_formed_proactive_high_attempts_reopens_instead_of_skip(self):
        module = self._load_auto_reply_module("auto_reply_proactive_in_slot_high_attempt")
        previous = os.environ.get("OPENKAKAO_TARGET_CHAT_ID")
        os.environ["OPENKAKAO_TARGET_CHAT_ID"] = "42"
        digest = "GeekNews TOP5 · 2026-08-21 19:30 KST\n\n1. example"
        now = 1_787_308_223
        event = {
            "chat_id": 42,
            "log_id": 3912139758343886849,
            "proactive": True,
            "sent_at": now,
            "author_nickname": "현준",
            "proactive_query": "geeknews-rss",
            "proactive_source_log_id": 3912186594074877953,
            "message": digest,
        }
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            ...
            try:
                queue.execute(
                    ...
                    INSERT INTO reply_jobs(
                        ...
                        'db:42:1787308223', ?, 'delivery_unknown',
                        NULL, 'reply', 'geeknews_rss', 'proactive', ?, 0.4,
                        'reconcile_required', 1.0, 1.0, 14
                    )
                    """,
                    (json.dumps(event, ensure_ascii=False), digest),
                )
                queue.commit()
                module._geeknews_slot_at = lambda stamp: ("2026-08-21", "evening")
                with mock.patch.object(module, "complete_event") as complete:
                    module.recover_stale_jobs(queue)
                row = queue.execute(
                    "SELECT status, decision, reason, category FROM reply_jobs "
                    "WHERE event_id='db:42:1787308223'"
                ).fetchone()
                self.assertEqual(row["status"], "scheduled")
                self.assertEqual(row["decision"], "reply")
                self.assertEqual(row["reason"], "geeknews_rss")
    ...
    def test_operator_forced_geeknews_enqueues_outside_slot(self):
        ...
        queued = []
        outside = datetime(2026, 8, 20, 16, 15, tzinfo=module.GEEKNEWS_SLOT_TZ).timestamp()
        source = {
            "event_id": "db:42:99",
            "canonical_event_id": "db:42:99",
            "chat_id": 42,
            ...
            "reply_authorized": True,
            "message": "요즘 세금 어때",
            "sent_at": int(outside) - 10,
        }
        refused = module.maybe_enqueue_proactive_topic(
            connection,
            now=outside,
            response_time=stats,
            style_profile={"common_tokens_json": "{}"},
            last_observed_sent_at=int(outside) - 10,
            conversation_advanced=False,
            source_event=source,
            enqueue=lambda event: queued.append(event) or True,
            search=lambda query: [
        ...
        forced = module.maybe_enqueue_proactive_topic(
            connection,
            now=outside,
            response_time=stats,
            style_profile={"common_tokens_json": "{}"},
        ...
        self.assertEqual(queued[0]["operator_forced"], True)
        self.assertFalse(
            module._proactive_unknown_is_closed_slot(queued[0], int(outside), now=outside)
        )

    def test_closed_slot_pending_geeknews_skips_without_send(self):
        module = self._load_auto_reply_module("auto_reply_geeknews_closed_slot")
        previous = os.environ.get("OPENKAKAO_TARGET_CHAT_ID")
        os.environ["OPENKAKAO_TARGET_CHAT_ID"] = "42"
        digest = "GeekNews TOP5 · closed slot"
        now = int(time.time())
        event = self._burst_event(module, 120, digest, now)
        event["event_id"] = "db:42:1787183029"
        event["canonical_event_id"] = "db:42:1787183029"
        event["proactive"] = True
        event["proactive_query"] = "geeknews-rss"
        event["proactive_token"] = "긱뉴스"
        try:
            with tempfile.TemporaryDirectory() as temporary:
                ...
                try:
                    ...
                    row = connection.execute(
                        """
                        SELECT status, decision, reason, category, error_class
                        FROM reply_jobs WHERE event_id = ?
                        """,
                        (event["event_id"],),
                    ).fetchone()
    ...
    def test_lexical_overlay_recalls_room_keyword_without_shared_vector(self):
        module = self._load_auto_reply_module("auto_reply_lexical_overlay")
        inbound = "어제 번호 이야기해줫던거 머더라"
        event = {
            "chat_id": 417780809780519,
            "log_id": 99,
            "message": inbound,
        ...
        hits = [
            {
                ...
                "is_self": False,
                "sender_name": "문승현",
                "message": "사업자 번호 027-12-34567 로 내면 됨",
                "attachment": "",
                "message_type": 1,
                "sent_at": 1,
            }
        ]
        with mock.patch.object(module, "_run_json_command", return_value=hits) as run:
            merged = module._lexical_context_overlay(inbound, event, context)
        self.assertTrue(any("사업자 번호" in str(item.get("message")) for item in merged))
        self.assertEqual(merged[0]["mode"], "lexical")
        commands = [call.args[0] for call in run.call_args_list]
        self.assertTrue(commands)
        self.assertTrue(all("local-search" in command for command in commands))
        self.assertTrue(all("--chat-id" in command for command in commands))
        self.assertTrue(all("417780809780519" in command for command in commands))
        self.assertIn("번호", [command[2] for command in commands])
        self.assertEqual(module._lexical_overlay_terms(inbound)[0], "번호")

    def test_youtube_oembed_fills_empty_html_title(self):
        module = self._load_auto_reply_module("auto_reply_youtube_oembed")
        oembed = json.dumps(
            {"title": "가장 얇은 아이폰", "author_name": "GeekNews"},
            ensure_ascii=False,
        ).encode()

        def fake_http(url, deadline):
            if "oembed" in url:
                return oembed, len(oembed)
            return b"<html><title></title><body></body></html>", 42

        with mock.patch.object(module, "_fetch_pinned_http", side_effect=fake_http):
            preview, _size = module._fetch_link_preview_once(
                "https://www.youtube.com/watch?v=abc",
                time.monotonic() + 2,
            )
        self.assertTrue(preview["complete"])
        self.assertEqual(preview["title"], "가장 얇은 아이폰")
        self.assertIn("GeekNews", preview["text"])

    def test_non_youtube_empty_title_skips_oembed(self):
        module = self._load_auto_reply_module("auto_reply_non_youtube_title")
        html = b"<html><title>  </title><body>hello world text</body></html>"
        with mock.patch.object(
            module, "_fetch_pinned_http", return_value=(html, len(html))
        ) as fetch:
            preview, _size = module._fetch_link_preview_once(
                "https://example.com/x",
                time.monotonic() + 2,
    ...
    def test_fetch_link_previews_includes_recent_conversation_urls(self):
        module = self._load_auto_reply_module("auto_reply_recent_link_previews")
        calls = []

        def fake_once(url, deadline):
            calls.append(url)
            return {"url": url, "title": "t", "text": "x", "complete": True}, 10

        with mock.patch.object(module, "_fetch_link_preview_once", side_effect=fake_once):
            previews = module.fetch_link_previews(
                "그거 봤어 https://example.com/a",
                extra_messages=["참고 https://youtu.be/abc"],
            )
        self.assertEqual(calls, ["https://example.com/a", "https://youtu.be/abc"])
        self.assertTrue(
            module.links_fully_retrieved(
                "그거 봤어 https://example.com/a",
                previews,
            )
        )
    ...
    def test_kakao_article_redirect_follows_one_daum_hop(self):
        module = self._load_auto_reply_module("auto_reply_kakao_daum_redirect")
        html = (
            "<html><head><title>AI 호황에도 양극화 커진 대만</title>"
            '<meta property="og:description" content="라이칭더 총통이 1인당 1만 대만달러">'
            "</head><body>민심 달래기 AI 배당</body></html>"
        ).encode("utf-8")
        hosts = []
        ...
        def open_pinned(parsed, hostname, port, addresses, deadline):
            del parsed, port, addresses, deadline
            hosts.append(hostname)
            if hostname == "v.kakao.com":
                return Conn(), Resp(301, "https://v.daum.net/v/20260818174953765")
            if hostname == "v.daum.net":
                return Conn(), Resp(200, body=html)
            raise AssertionError(hostname)

        with mock.patch.object(
            module,
            "_resolve_link_addresses",
            return_value=[(0, 0, 0, "", ("8.8.8.8", 443))],
        ), mock.patch.object(module, "_open_pinned_link", side_effect=open_pinned):
            preview, size = module._fetch_link_preview_once(
                "https://v.kakao.com/v/20260818174953765",
                time.monotonic() + 2,
            )
        self.assertEqual(hosts, ["v.kakao.com", "v.daum.net"])
        self.assertGreater(size, 0)
        self.assertTrue(preview["complete"])
        self.assertIn("호황", preview["title"])
        self.assertIn("라이칭더", preview["text"])

    def test_kakao_open_redirect_is_rejected(self):
        module = self._load_auto_reply_module("auto_reply_kakao_open_redirect")
        ...
        with mock.patch.object(
            ...
            module, "_open_pinned_link", return_value=(Conn(), Resp())
        ):
            with self.assertRaisesRegex(ValueError, "link redirect not allowed"):
                module._fetch_pinned_http(
                    "https://v.kakao.com/v/20260818174953765",
                    time.monotonic() + 2,
                )

    def test_fetch_link_previews_includes_event_card_urls(self):
        module = self._load_auto_reply_module("auto_reply_event_card_urls")
        calls = []
        ...
        with mock.patch.object(module, "_fetch_link_preview_once", side_effect=fake_once):
            previews = module.fetch_link_previews(
                "샵검색: #대만 44만원",
                extra_urls=["https://v.kakao.com/v/20260818174953765"],
            )
        self.assertEqual(calls, ["https://v.kakao.com/v/20260818174953765"])
        self.assertTrue(
            module.links_fully_retrieved(
                "샵검색: #대만 44만원",
                previews,
    ...
    def test_github_link_preview_appends_readme_text(self):
        module = self._load_auto_reply_module("auto_reply_github_readme_preview")
        html = (
            b"<html><head><title>fivetaku/tikeytaka</title></head>"
            b"<body>GitHub repository page</body></html>"
        )
        readme = b"# tikeytaka\n\nOffline-first football tactics board.\n"

        def fake_http(url, deadline):
            if "raw.githubusercontent.com" in url and url.endswith("/README.md"):
                self.assertIn("/fivetaku/tikeytaka/", url)
                return readme, len(readme)
            return html, len(html)

        with mock.patch.object(module, "_fetch_pinned_http", side_effect=fake_http):
            preview, _size = module._fetch_link_preview_once(
                "https://github.com/fivetaku/tikeytaka",
                time.monotonic() + 2,
            )
        self.assertTrue(preview["complete"])
        self.assertIn("tikeytaka", preview["text"])
    ...
    def test_rerank_client_consumes_late_sidecar_greeting(self):
        ...
        client._readline = mock.Mock(
            side_effect=[None, '{"ok": true, "ready": true}']
        )
        self.assertEqual(client.ensure(), "warming")
        self.assertFalse(client.ready)
        self.assertEqual(client.ensure(), "ready")
    ...
    def test_self_chat_row_requires_positive_author_id(self):
        ...
        self.assertFalse(module._is_self_chat_row(other))
        self.assertEqual(module._render_message_line(other), "최연우: 이건 다른 사람")
        named_other = {
            "is_self": False,
            "author_id": 206894008,
            "author_nickname": "문승현",
        ...
        self.assertFalse(module._is_self_chat_row(named_other))
        self.assertEqual(module._render_message_line(named_other), "문승현: 법무부네 이건")
        mine = {
            "is_self": True,
            "author_id": 199453377,
            "author_nickname": "최연우",
        ...
        self.assertTrue(module._is_self_chat_row(mine))
        self.assertEqual(module._render_message_line(mine), "그건 텍스트 안 뜸")
        receipt = {
            "is_self": True,
            "self_receipt": True,
            "author_nickname": "최연우",
        ...
        self.assertTrue(module._is_self_chat_row(receipt))
        self.assertEqual(module._render_message_line(receipt), "방금 보냄")
        legacy = {"is_self": True, "message": "옛 영수증"}
        self.assertTrue(module._is_self_chat_row(legacy))
        self.assertEqual(module._render_message_line(legacy), "옛 영수증")
        zero = {
            "is_self": True,
            "author_id": 0,
            "author_nickname": "최연우",
        ...
        self.assertFalse(module._is_self_chat_row(zero))
        self.assertEqual(
            module.numeric_author_identity_status(
                {
                    "is_self": True,

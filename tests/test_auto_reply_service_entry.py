...
""",
    ...
    def _service_env(self, root, fixture, launchctl, plutil, **updates):
        ...
        plist = launch_agents / "com.openkakao.auto-reply.autoreply.plist"
        env = {
            "HOME": str(home),
            "PATH": "/usr/bin:/bin",
            "TMPDIR": str(root),
    ...
    def _assert_post_bootstrap_install_fenced(
        ...
        self.assertEqual(generated["Label"], "com.openkakao.auto-reply.autoreply")
    ...
    def test_owned_file_keeps_homebrew_keg_python_string(self):
        module = load_entry("auto_reply_owned_file_keg_test")
    ...
    def test_selector_declaration_is_bounded_canonical_and_duplicate_free(self):
        module = load_entry("auto_reply_multi_selector_bounds_test")
        self.assertEqual(
    ...
    def test_preflight_rejects_duplicate_or_cross_root_target_identity(self):
        module = load_entry("auto_reply_multi_target_identity_test")
        base = {
            "valid": True,
            "check": True,
            "will_send": False,
    ...
    def test_session_vanished_python_publishes_typed_interpreter_miss(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            def missing_python(*_args):
                raise SystemExit(
                    "python_interpreter_missing: AutoReply python_interpreter is unavailable"
                )
    ...
    def test_session_reconciliation_fence_does_not_retry_or_spawn(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            def unclean_leftover(*_args):
                raise SystemExit(
                    "reconciliation_required: AutoReply room leftover requires reconciliation before restart"
                )
...
""",
    ...
    def test_owned_child_termination_escalates_only_attested_group(self):
        module = load_entry("auto_reply_terminate_group_test")
    ...
    def test_installer_production_replaces_only_after_verified_bootout(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(
                ...
                [
                    f"print gui/{os.getuid()}/com.openkakao.auto-reply.autoreply",
                    f"bootout gui/{os.getuid()}/com.openkakao.auto-reply.autoreply",
                    f"print gui/{os.getuid()}/com.openkakao.auto-reply.autoreply",
    ...
    def test_status_accepts_terminal_monitor_with_fresh_multi_room_proofs(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            monitor_plist = (
                launch_agents / "com.openkakao.auto-reply.session-monitor.plist"
            )
    ...
    def test_session_monitor_uninstaller_fences_before_bootout_and_preserves_state(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(len(backups), 1)
            self.assertIn(
                f"bootout gui/{os.getuid()}/com.openkakao.auto-reply.session-monitor",

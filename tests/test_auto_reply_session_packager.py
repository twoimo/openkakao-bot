...
PACKAGER = ROOT / "scripts" / "prepare-auto-reply-session-runtime.py"
...
class SessionRuntimePackagerTests(unittest.TestCase):
    ...
    def test_stages_private_immutable_multi_room_runtime_without_activation(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertIn("auto-reply-tui.py", manifest["assets"])
            self.assertEqual(stat.S_IMODE((runtime / "scripts").stat().st_mode), 0o700)
            self.assertEqual(
                stat.S_IMODE((runtime / "scripts/auto-reply-tui.py").stat().st_mode),
                0o500,
            )
            self.assertEqual(
            ...
            watchdog = runtime / "start-auto-reply-session.command"
            tui = runtime / "open-auto-reply-tui.command"
            ...
            # The launcher must not drive Terminal through Apple Events: that
            # made macOS ask to allow each freshly packaged binary to control
            # Terminal.app, and the permission never persisted across bakes.
            self.assertNotIn("osascript", watchdog_text)
            self.assertNotIn("close w saving no", watchdog_text)
            self.assertNotIn("busy of w", watchdog_text)
            ...
            self.assertEqual(plist["ProgramArguments"][1], "auto-reply-host")
            self.assertEqual(plist["ProgramArguments"][2], "--tick")
    ...
    def test_rejects_symlink_or_mutable_asset_and_never_leaves_partial_release(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            (source / "auto-reply-schema.json").unlink()
            (source / "auto-reply-schema.json").symlink_to(real_schema)
    ...
    def test_packaged_assets_are_copies_not_links(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            runtime = Path(result["runtime_root"])
            staged = runtime / "scripts/auto-reply-tui.py"

...
- `macOS cargo test` job은 이 브랜치에서도 `Audit menu layout` step에서 실패한다. 남은 문제는 `layout: settings-dark: 375~399행이 비어 있습니다 (25pt)` 하나뿐이고, 검사기는 24pt를 초과하는 빈 구간만 보고한다(`scripts/audit-menubar-layout.py`의 `bands(min_points=24)`). 이 실패는 이 브랜치의 회귀가 아니라 `main`에도 있는 기존 실패다: `main`의 최근 실행 3건(`35395258008`·`35390808490`·`35390187741`, `0fe7ba87`·`974c9dcd`·`50ed275e`)은 같은 step에서 `log-dark` 227pt·`rooms` 348pt·위아래 여백 불일치·`menu-panel-rooms` 손배치 간격 불일치 등 8건으로 실패한다. 즉 이 브랜치는 그 8건을 1건(25pt)으로 줄였고, 남은 25pt는 legacy Swift 설정 패널의 dark 테마 여백이다. `macos/AutoReplyMenu/main.swift`와 `scripts/check-menubar-layout.py`는 보존 대상인 `5a1c` 작업트리에서 수정 중이므로 이 브랜치에서 중복 수정하지 않았고, legacy Swift Extra는 계획상 Tauri 앱으로 대체되는 호환 감사 대상이다.

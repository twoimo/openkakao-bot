...
- Stock openWakeWord `hey_jarvis` on a 0.96 s Korean TTS clip of “헤이 자비스” peaked at **0.001959** against `WAKE_THRESHOLD=0.65` (20 ms int16 frames; 80 ms diagnostic 0.00136). The English stock model misses this pronunciation; the threshold was not lowered. `OpenWakeVadFrontend.analyze()` now converts 640-byte frames to numpy int16 before `predict()`. Custom Korean wake-model training, live microphone, and signed Extra cutover remain open. See [unit5-live-proofs.md](docs/architecture/unit5-live-proofs.md).

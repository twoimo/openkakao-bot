...
- Voice settings now show the locked 0.65 Korean wake path. Bundled `hey_jarvis_ko_ridge.onnx` is selected when it validates. A file pipeline accepted custom wake at **0.784** (stock 2e-6); Flash-Next reply was `generation_error` because Extra still owns the resident model. Live mic and signed cutover remain open.

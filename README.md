...
- 이 트리에서 `tests.test_auto_reply_service_entry`는 **54개 중 20 failures + 6 errors**로 red다. 원인은 코드가 아니라 이 호스트의 Homebrew 인터프리터다: 모듈이 `PINNED_PYTHON = /opt/homebrew/opt/python@3.11/bin/python3.11`을 하드코딩하는데 그 3.11.16 Cellar의 `Python.framework/Versions/3.11/Python`이 없어 자식 프로세스가 `dyld: Library not loaded ... (-6)`으로 죽는다. 경로가 존재하기만 하면 `sys.executable` 대체로 내려가지 않는 구조라 실패가 그대로 드러난다. 이 브랜치는 `scripts/auto-reply-service.py`와 이 테스트 모듈을 바꾸지 않았고(`git diff origin/main...HEAD`가 비어 있다), 이 모듈은 CI 목록에 없어 그동안 드러나지 않았다. 그래서 production service 진입 경로의 검증은 지금 비어 있다고 취급한다. 호스트 인터프리터 문제이므로 이 브랜치에서 제품 로직이나 하네스를 고치지 않고 사실만 남긴다.

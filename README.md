...
- DPO 로그확률 수집·평가 경로를 실제 로컬 게이트웨이로 처음 끝까지 실행했다(2026-09-22 KST). `127.0.0.1:11234/v1`에서 `probe_response_scoring`은 `reason="echo_unsupported"`·`supports_response_scoring=false`를 반환했고(echo 요청이 프롬프트 대신 생성 토큰 `" epsilon"`만 돌려줬다), 같은 엔드포인트에 `logprobs=true`를 주면 생성 토큰별 로그확률이 온다. 실제 수집은 프롬프트 `한 문장으로 답하세요: 오늘 서울 날씨 어때?`에 대해 `max_tokens=1` → `"저"` 1토큰 합계 `-0.351563`, `max_tokens=6` → `"저는 실시간 데이터에 접근"` 6토큰 합계 `-1.313843`이었다. 이 두 값을 `--dpo-pairs`·`--dpo-ref-pairs`(동일 기준값)로 넣은 CLI 실행은 exit 0, `objective="dpo"`, `evaluated=1`, `loss=0.6931471805599453`(마진 0일 때의 정확한 값 ln 2)을 냈고 `string_similarity_used=false`였다. 기준 파일을 빼면 같은 실행이 `missing_reference_logprobs`로 평가 불가가 된다. 이는 로그확률 수집과 표준 DPO 산술의 실측이며 어댑터 학습이나 모델 승격을 입증하지 않는다. 고정 Python 3.11에서 `tests.test_auto_reply_finetune`은 **60 tests, OK**, CI focused 11개 모듈은 **389 tests, OK**다.

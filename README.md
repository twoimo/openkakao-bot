...
- 이 트리에서 `tests.test_auto_reply_service_entry`는 **54개 중 20 failures + 6 errors**로 red다(작업 트리와 추적 파일만의 새 체크아웃 모두 같은 결과). 이 브랜치는 `scripts/auto-reply-service.py`와 이 테스트 모듈을 바꾸지 않았고(`git diff origin/main...HEAD`가 비어 있다), 이 모듈은 CI 목록에 없어 그동안 드러나지 않았다. 실패 지점은 `run_preflight`/`run_production`의 `SystemExit("auto-reply preflight failed")` 계열이므로 production service 진입 경로의 검증은 지금 비어 있다고 취급한다. 의도 확인 없이 26건을 고치면 기존 로직을 왜곡할 위험이 있어 이 브랜치에서는 고치지 않고 사실만 남긴다.

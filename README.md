...
Read-only host checks on 2026-09-24 returned HTTP 200 from the local MLX `/health` and `/v1/models` endpoints. Flash-Next is loaded; Qwen3.8 27B and Qwen3-TTS are not. The latest synthetic Flash-Next generation probe disconnected at 45.060 s with zero output tokens and was not repeated, so generation remains unverified. A post-probe read showed 1,040.38 MiB free swap, 1,007.62 MiB below the 2,048 MiB voice admission threshold; Whisper/TTS loading is blocked and a complete wake→STT→LLM→TTS turn remains unverified. The loaded model advertises no embedding capability, so live GraphRAG remains BM25-only and dense/RRF is unavailable. The auto-reply host reports unhealthy: its session watchdog is circuit-open after repeated preflight failures, and configured rooms are stopped or fenced. Read-only AX checks found a transcript mismatch in one room and no open exact window for another. These checks sent no messages and started no workers. No model load or server takeover was performed. See [engineering status](docs/engineering-status.md) for the measurements and limits.

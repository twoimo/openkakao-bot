...
- Current 2026-09-21 probe evidence for the existing `http://127.0.0.1:11234/v1` MLX gateway: Qwen3.8 Flash-Next returned `OK` in **8211 ms** under a **90000 ms** maximum timeout. This is current external probe evidence, not reuse of a persisted/stale success and not a probe executed by the DREAM-RSI report command.
- Qwen3.8 27B is gateway-advertised with `loaded=false` / `state=unloaded`; no model switch was performed.

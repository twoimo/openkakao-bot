...
- DREAM-RSI paper provenance is available through `scripts/dream_rsi_alphaxiv.py`, but the persistent `PATH` has no alphaXiv CLI that implements the required `search`, `context`, and `paper summary` commands, so paper analysis continues to fail closed. PyPI `alphaxiv==0.0.13` is instead a TTY-based W&B experiment synchronization CLI and does not provide those commands. The installed `orx` openresearch CLI is not used as an alphaXiv CLI substitute. See [dream-rsi-alphaxiv-provenance.md](docs/dream-rsi-alphaxiv-provenance.md).

...
- 독립 서브에이전트 리뷰에서 나온 4개 결함을 수정하고 재검증했다. (1) `orx` provider의 실패 provenance가 실제 실행과 어긋나던 `isolated_context`를 false로 맞추고 alphaxiv 실패는 true를 유지한다. (2) 외부 CLI `stderr`는 제어문자 제거, 공백 정규화, 32자 이상 token run redaction, 200자 상한을 거친 뒤에만 직렬화한다. (3) `collect_orx_paper_report`와 `collect_paper_report`가 극단 `timeout`에서도 `OverflowError`를 포함해 total하게 닫히고 `invalid_timeout`을 반환한다. (4) `replay_guarantee`에 `incumbent_included`를 추가하고 문서와 다이어그램 문구를 조건부로 바꿔, caller가 incumbent 없는 후보 집합을 넘기면 보증 대신 `not_applicable`을 기록한다. 고정 Python 3.11에서 `tests.test_dream_rsi_alphaxiv`와 `tests.test_auto_reply_dream_rsi`는 **71 tests, OK**, CI focused Python 목록은 **283 tests, OK**이고, `scripts/dream_rsi_alphaxiv.py --paper-source orx --paper-id 2609.14858`는 여전히 exit 0, status ok, provider orx_cli, summary 14,515자다.

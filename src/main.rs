...
enum Commands {
    ...
    Doctor {
        ...
        #[arg(long)]
        loco: bool,
        /// Run the pure-local self-check (닥터): diagnose + bounded auto-repair,
        /// no external contact (R7)
        #[arg(long)]
        self_check: bool,
    },
    /// Build the local RAG dataset from KakaoTalk conversations (no server contact)
    Dataset {
        #[command(subcommand)]
        command: DatasetCommand,
    },
    /// Run LLM quality/speed experiments over the available providers (R4)
    Experiment {
        #[command(subcommand)]
...
#[derive(Subcommand, Debug)]
enum DatasetCommand {
    /// Build the dataset locally from the KakaoTalk database (R3).
    Build {
        /// Build with a local-only embedder. This is the only supported mode:
        /// the dataset is never sent to an external network (R3.12).
        #[arg(long)]
        local_only: bool,
    },
}
...
enum ExperimentCommand {
    /// Run the fixed question set against every discovered LLM with a prompt
    /// version, recording latency and local quality score (R4.2–R4.6, R4.8).
    Run {
        /// The prompt version to run (e.g. `v1`, `v2`). Re-running with an
        /// improved version enables comparison against earlier results (R4.7).
        #[arg(long)]
        prompt: String,
    },
    /// Compare two previously-run prompt versions by quality and speed (R4.7).
    Compare {
        /// The first prompt version to compare.
        version_a: String,
        /// The second prompt version to compare.
        version_b: String,
    },
}
...
fn resolve_auto_reply_author_bindings(
    ...
    for nickname in configured {
        ...
        if ids.is_empty() {
            // A configured member simply not present in this room is skipped,
            // not an error.
            continue;
        }
        if ids.len() > 1 {
            // Fail closed: a configured display name that resolves to more than
            // one author ID is ambiguous and could bind the wrong person.
            anyhow::bail!(
                "AutoReply reply author {nickname:?} in room {room_name:?} maps to multiple author IDs"
            );
        }
        let author_id = *ids.iter().next().expect("one author ID");
        match nicknames_by_id.get(&author_id) {
            // The bound author must carry exactly this one display name.
            Some(names) if names.len() == 1 && names.contains(nickname) => {}
            // Display-name drift: the same author ID appears under more than one
            // name. Fail closed rather than risk binding a renamed identity.
            Some(names) if names.contains(nickname) => anyhow::bail!(
                "AutoReply reply author {nickname:?} (ID {author_id}) in room {room_name:?} has drifting display names"
            ),
            _ => continue,
        }
...
fn auto_reply_attest_explicit_bindings(
    ...
    const AX_ATTEST_COUNT: usize = 40;
...
fn main() -> Result<()> {
    ...
    match cli.command {
            ...
            loop {
                ...
                let events = envelope
                    ...
                    .map(|message| openkakao_cli::context::LiveContextEvent {
                        ...
                        message: message.message.trim().to_string(),
                        sent_at: message.sent_at,
                        is_self: local_db::is_self_author(message.author_id, account_user_id),
        ...
        Commands::Doctor { loco, self_check } => {
            if self_check {
                commands::doctor::cmd_doctor_self_check(json, &config)?
            } else {
                commands::doctor::cmd_doctor(json, loco, &config)?
            }
        }
        Commands::Dataset { command } => match command {
            DatasetCommand::Build { local_only } => {
                commands::dataset::cmd_dataset_build(local_only, json)?
            }
        },
        Commands::Experiment { command } => match command {
            ExperimentCommand::Run { prompt } => {
                commands::experiment::cmd_experiment_run(prompt, json, &config)?
            }

...
const RECIPIENT_STYLE_SEARCH_SQL: &str =
    "SELECT styles.id, styles.chat, styles.source, styles.date,
     ...
     FROM owner_recipient_style_samples samples
     JOIN owner_style styles ON styles.id = samples.style_message_id
...
pub struct RecipientStyleProfile {
    ...
    /// 존댓말/반말 파생 필드. 종결어미 통계에서 규칙 기반으로 도출한다(R3.3).
    pub honorific: Honorific,
    /// 격식/비격식 파생 필드. 종결어미와 이모지·구두점·축약 토큰 통계를
    /// 결합해 규칙 기반으로 도출한다(R3.3).
    pub formality: Formality,
    pub profile: StyleProfile,
}

/// 최연우가 특정 상대방에게 쓰는 말투가 존댓말인지 반말인지 구분한다(R3.3).
...
/// 말투가 격식체인지 비격식체인지 구분한다(R3.3).
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize)]
#[serde(rename_all = "snake_case")]
pub enum Formality {
    /// 격식체(합쇼체 위주, 이모지·축약이 적음).
    Formal,
    /// 비격식체(해요체·반말, 이모지·축약이 섞임).
    Informal,
}

/// 종결어미 하나의 분류. 존댓말/격식 파생에만 쓰는 내부 신호다.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum StyleEndingClass {
    /// 합쇼체 종결어미(`습니다`/`합니다`/`됩니다`) — 격식·존댓말 신호.
    Deferential,
    /// 해요체 종결어미(`요`/`죠`) — 존댓말 신호.
    Honorific,
    /// 웃음·축약 종결(`ㅋㅋㅋ`/`ㅠㅠ` 등) — 비격식 신호.
    Contraction,
    /// 그 밖의 반말 종결(`어`/`아`/`야`/`지` 등).
    Casual,
}

/// 종결어미 문자열을 존댓말/격식 파생용 분류로 매핑한다. 기존 말투 통계에
/// 등장하는 종결어미만 다룬다.
pub fn classify_style_ending_class(ending: &str) -> StyleEndingClass {
    match ending {
        "습니다" | "합니다" | "됩니다" => StyleEndingClass::Deferential,
        "요" | "죠" => StyleEndingClass::Honorific,
        "ㅋㅋㅋ" | "ㅠㅠ" | "ㅜㅜ" | "ㅠ" | "ㅜ" => StyleEndingClass::Contraction,
        _ => StyleEndingClass::Casual,
    }
}

/// 토큰이 웃음·축약 토큰(`ㅋㅋ`, `ㅎㅎ`, `ㅠㅠ`, `ㅜㅜ` 계열)인지 판단한다.
/// 자음/모음 축약만으로 이루어진 토큰을 비격식 신호로 본다.
pub fn is_contraction_token(token: &str) -> bool {
    let trimmed = token.trim();
    !trimmed.is_empty()
        && trimmed
            .chars()
            .all(|ch| matches!(ch, 'ㅋ' | 'ㅎ' | 'ㅠ' | 'ㅜ' | 'ㅡ' | 'ㅗ' | 'ㄷ' | 'ㅇ'))
}

/// 존댓말/격식 파생에 쓰는 규칙 기반 신호 묶음. 종결어미·이모지·구두점·축약
/// 토큰 통계에서만 도출하며 원문은 담지 않는다(R3.3).
#[derive(Debug, Clone, Copy, Default, PartialEq, Eq)]
pub struct StyleSignals {
    pub sample_count: usize,
    pub honorific_ending_count: usize,
    pub deferential_ending_count: usize,
...
impl StyleSignals {
    /// 이미 집계된 말투 프로필의 종결어미·이모지·구두점·축약 통계에서 신호를
    /// 뽑아낸다. `common_endings_json`으로 존댓말/반말/격식 종결을 분류하고,
    /// `casual_ending_counts_json`·`common_tokens_json`에서 축약 신호를 더한다.
    pub fn from_style_profile(profile: &StyleProfile) -> Self {
        let mut signals = StyleSignals {
            sample_count: profile.sample_count,
            emoji_count: profile.emoji_count,
            punctuation_count: profile.punctuation_count,
            ..StyleSignals::default()
        };
        if let Ok(map) =
            serde_json::from_str::<BTreeMap<String, usize>>(&profile.common_endings_json)
        {
            for (ending, count) in map {
                match classify_style_ending_class(&ending) {
                    StyleEndingClass::Deferential => signals.deferential_ending_count += count,
                    StyleEndingClass::Honorific => signals.honorific_ending_count += count,
                    StyleEndingClass::Contraction => signals.contraction_count += count,
                    StyleEndingClass::Casual => signals.casual_ending_count += count,
                }
            }
        }
        // 축약/웃음 신호는 별도 통계에서만 추가로 집계한다(반말 종결과 중복 계산 방지).
        if let Ok(map) =
            serde_json::from_str::<BTreeMap<String, usize>>(&profile.casual_ending_counts_json)
        {
            for (ending, count) in map {
                if classify_style_ending_class(&ending) == StyleEndingClass::Contraction {
                    signals.contraction_count += count;
                }
            }
        }
        if let Ok(map) =
            serde_json::from_str::<BTreeMap<String, usize>>(&profile.common_tokens_json)
        {
...
/// 규칙 기반으로 존댓말/반말과 격식/비격식을 도출한다(R3.3).
///
/// - 존댓말/반말: 존댓말 종결(해요체+합쇼체) 신호가 반말 종결 신호보다 많으면
///   존댓말로 본다.
/// - 격식/비격식: 존댓말이면서 합쇼체 종결이 있고, 그 합쇼체 신호가
///   비격식 신호(이모지+축약+반말 종결)보다 우세할 때만 격식체로 본다.
///   나머지는 모두 비격식체다(해요체·반말·이모지 섞인 존댓말 포함).
pub fn derive_honorific_formality(signals: &StyleSignals) -> (Honorific, Formality) {
    let honorific_signal = signals.honorific_ending_count + signals.deferential_ending_count;
    let casual_signal = signals.casual_ending_count;
    let honorific = if honorific_signal > casual_signal {
        Honorific::Honorific
    } else {
        Honorific::Casual
    };
    ...
    let informal_signal =
        signals.emoji_count + signals.contraction_count + signals.casual_ending_count;
    let formal_signal = signals.deferential_ending_count;
    let formality = if honorific == Honorific::Honorific
        && formal_signal > 0
        && formal_signal >= informal_signal
    {
        Formality::Formal
    } else {
        Formality::Informal
...
/// 말투 프로필에서 곧바로 존댓말/격식을 도출하는 편의 함수.
pub fn derive_style_profile_honorific_formality(profile: &StyleProfile) -> (Honorific, Formality) {
    derive_honorific_formality(&StyleSignals::from_style_profile(profile))
}
...
pub fn redact_absolute_paths(value: &mut serde_json::Value) {
    match value {
...
fn refresh_live_context_summaries(
    ...
    conn.execute(
        "DELETE FROM owner_style_profile
    ...
    conn.execute(
        "DELETE FROM owner_recipient_style_profile
    ...
    for (recipient, accumulator) in recipient_accumulators {
        ...
        conn.execute(
            "INSERT INTO owner_recipient_style_profile(
...
fn live_source_has_required_summaries(conn: &Connection, source: &str, chat: &str) -> Result<bool> {
    let style_samples = conn
        .query_row(
            "SELECT sample_count FROM owner_style_profile
             WHERE chat = ?1 AND source = ?2 AND user_name = ?3
                    ...
                    if features.style_eligible {
                        ...
                        for (recipient, confidence) in confidences {
                            recipient_style_samples += tx.execute(
                                "INSERT OR IGNORE INTO owner_recipient_style_samples(
...
fn recipient_style_profile_with_connection(
    ...
    let has_recipient_table = conn.query_row(
        "SELECT EXISTS(
            SELECT 1 FROM sqlite_master
            WHERE type = 'table' AND name = 'owner_recipient_style_profile'
         )",
        [],
        |row| row.get::<_, i64>(0),
    )? != 0;
    let direct = if has_recipient_table {
        let mut stmt = conn.prepare(
            "SELECT source, sample_count, confidence_sum, average_character_length,
                    ...
                    policy_version
             FROM owner_recipient_style_profile
    ...
    // 최소표본/최소신뢰도 미만이면 방 전체 폴백 프로필을 유지하고, 존댓말/격식은
    // 그 폴백 프로필의 통계에서 그대로 도출한다(R3.3).
    let fallback =
        style_profile_with_connection(conn, chat, STYLE_USER, preferred_source.as_deref(), true)?;
    Ok(fallback.map(|profile| {
        let (honorific, formality) = derive_style_profile_honorific_formality(&profile);
...
pub fn search_with_embedder(
    ...
    let conn = open_db_readonly(db_path)?;
    ensure_retrieval_index_current(&conn)?;
    search_with_connection(&conn, chat, source, query, mode, limit, embedder)
}
...
fn search_with_connection(
    ...
    search_with_connection_excluding(
        conn,
        chat,
        source,
        query,
...
/// Rename legacy `choi_yeonwoo_*` style tables to their `owner_*` names when an
/// older database is opened, so its data survives the owner-scope
/// generalization (R10.6). Only renames when the legacy table exists and the
/// renamed table does not, so it is safe to run on every open and on fresh
/// databases (where it does nothing).
fn rename_legacy_owner_tables(conn: &Connection) -> Result<()> {
    ...
    for (legacy, renamed) in RENAMES {
        if table_present(legacy)? && !table_present(renamed)? {
            conn.execute_batch(&format!("ALTER TABLE {legacy} RENAME TO {renamed}"))?;
        }
    }
    Ok(())
}
...
fn open_db(path: &Path) -> Result<Connection> {
    ...
    conn.execute_batch("PRAGMA journal_mode = DELETE; PRAGMA secure_delete = ON;
        ...
        CREATE TABLE IF NOT EXISTS owner_style(id INTEGER PRIMARY KEY, source TEXT NOT NULL, chat TEXT NOT NULL, date TEXT NOT NULL, user_name TEXT NOT NULL CHECK(user_name = '최연우'), message TEXT NOT NULL, vector BLOB NOT NULL, source_row INTEGER NOT NULL DEFAULT 0, content_kind TEXT NOT NULL DEFAULT 'legacy', style_eligible INTEGER NOT NULL DEFAULT 1, policy_version TEXT NOT NULL DEFAULT 'legacy-v1', features_json TEXT NOT NULL DEFAULT '{}');
        CREATE INDEX IF NOT EXISTS idx_owner_style_chat_source ON owner_style(chat, source);
        CREATE TABLE IF NOT EXISTS owner_style_profile(
            chat TEXT NOT NULL,
        ...
        CREATE INDEX IF NOT EXISTS idx_owner_style_profile_chat_user ON owner_style_profile(chat, user_name);
    ...
    let style_columns = conn
        .prepare("PRAGMA table_info(owner_style)")?
    ...
    for (name, declaration) in [
        ...
        if !style_columns.iter().any(|column| column == name) {
            conn.execute(
                &format!("ALTER TABLE owner_style ADD COLUMN {name} {declaration}"),
                [],
            )?;
        }
    }
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_owner_style_eligible
         ON owner_style(chat, source, style_eligible)",
        [],
    )?;
...
fn migrate_style_policy(conn: &Connection) -> Result<()> {
    ...
    let tx = conn.unchecked_transaction()?;
    let style_rows = {
        let mut stmt = tx.prepare("SELECT id, message FROM owner_style ORDER BY id ASC")?;
        let rows = stmt
            .query_map([], |row| {
    ...
    tx.execute(
        "DELETE FROM owner_recipient_style_samples
         WHERE style_message_id IN (
             SELECT id FROM owner_style
    ...
    let now = Utc::now().to_rfc3339();
    tx.execute("DELETE FROM owner_style_profile", [])?;
    let room_sources = {
        let mut stmt = tx.prepare(
            "SELECT DISTINCT source, chat FROM owner_style
    ...
    tx.execute("DELETE FROM owner_recipient_style_profile", [])?;
    let recipient_rows = {
        let mut stmt = tx.prepare(
...
fn migrate_live_context_schema(conn: &Connection) -> Result<()> {
    ...
    tx.execute_batch(
        ...
        CREATE TABLE IF NOT EXISTS owner_recipient_style_samples(
            source TEXT NOT NULL,
            chat_id INTEGER NOT NULL CHECK(chat_id > 0),
            reply_log_id INTEGER NOT NULL CHECK(reply_log_id > 0),
            recipient TEXT NOT NULL,
        ...
        CREATE INDEX IF NOT EXISTS idx_recipient_style_samples_recipient
            ON owner_recipient_style_samples(source, chat_id, recipient);
        CREATE INDEX IF NOT EXISTS idx_recipient_style_samples_lookup
            ON owner_recipient_style_samples(
                source, recipient, reply_log_id DESC, style_message_id DESC
            );
        CREATE TABLE IF NOT EXISTS owner_recipient_style_profile(
            source TEXT NOT NULL,
            chat_id INTEGER NOT NULL CHECK(chat_id > 0),
            chat TEXT NOT NULL,
            recipient TEXT NOT NULL,
        ...
        CREATE INDEX IF NOT EXISTS idx_recipient_style_profile_lookup
            ON owner_recipient_style_profile(chat, recipient, sample_count);
...
fn hybrid_search_excluding(
    ...
    let vector = if embedder.embed(query).iter().all(|value| *value == 0.0) {
        Vec::new()
    } else {
        vector_search_excluding(conn, chat, source, query, excluded_context_ids, embedder)?
    };
...
/// 임베딩 공급원 추상화(착탈식 임베딩 인터페이스). 검색이 사용하는 벡터
/// 표현을 이 트레이트 뒤로 감춰, 기본 로컬 구현 외에 로컬 임베딩 모델이나
/// ANN 인덱스를 붙일 수 있게 한다. 기본 구현은 외부 네트워크 없이 동작한다.
pub trait Embedder {
    /// 임베딩 벡터의 차원 수.
    fn dim(&self) -> usize;
    /// 입력 문자열을 벡터로 인코딩한다.
    fn embed(&self, text: &str) -> Vec<f32>;
    /// 외부 네트워크 사용 없이 로컬에서만 동작하는지 여부.
    /// `Dataset_Builder`는 반드시 `true`인 임베더만 허용한다(R3-12).
    fn is_local_only(&self) -> bool;
}

/// 기본 임베더. 기존 결정적 해시 벡터(`encode_vector`)를 그대로 감싼다.
/// 외부 없이 동작하며 재현 가능한(결정적) 벡터를 만든다. 이 구현을 기본으로
/// 주입하면 검색 동작이 리팩터링 이전과 완전히 동일하게 유지된다.
...
mod tests {
    ...
    fn style_profile_with_stats(
        ...
        emoji_count: usize,
        punctuation_count: usize,
        common_tokens: &[(&str, usize)],
    ) -> StyleProfile {
        let endings: BTreeMap<String, usize> = common_endings
            .iter()
            .map(|(k, v)| (k.to_string(), *v))
            .collect();
        let tokens: BTreeMap<String, usize> = common_tokens
            .iter()
            .map(|(k, v)| (k.to_string(), *v))
            .collect();
        let sample_count = common_endings.iter().map(|(_, v)| *v).sum::<usize>().max(1);
        StyleProfile {
            chat: "부자멘토멘티".to_string(),
            source: "local-db".to_string(),
            user: STYLE_USER.to_string(),
            sample_count,
            average_character_length: 10.0,
            median_character_length: 10.0,
            p90_character_length: 12.0,
            casual_ending_count: 0,
            casual_ending_counts_json: "{}".to_string(),
            question_count: 0,
            emoji_count,
            punctuation_count,
            common_endings_json: serde_json::to_string(&endings).unwrap(),
            common_tokens_json: serde_json::to_string(&tokens).unwrap(),
            policy_version: STYLE_POLICY_VERSION.to_string(),
        }
    }
    ...
    #[test]
    fn derive_casual_endings_are_informal_casual() {
        let profile = style_profile_with_stats(
            &[("어", 9), ("야", 4), ("지", 3), ("요", 1)],
            1,
            0,
        ...
        let (honorific, formality) = derive_style_profile_honorific_formality(&profile);
        assert_eq!(honorific, Honorific::Casual);
        assert_eq!(formality, Formality::Informal);
    }

    #[test]
    fn derive_no_ending_evidence_defaults_to_informal_casual() {
        let profile = style_profile_with_stats(&[], 0, 0, &[]);
        let (honorific, formality) = derive_style_profile_honorific_formality(&profile);
        assert_eq!(honorific, Honorific::Casual);
        assert_eq!(formality, Formality::Informal);
    }
    ...
    #[test]
    fn contraction_token_detection() {
        assert!(is_contraction_token("ㅋㅋ"));
        assert!(is_contraction_token("ㅠㅠㅠ"));
        assert!(!is_contraction_token("회의"));
        assert!(!is_contraction_token(""));
    }
...
2026-01-01 13:00:00,문승현,토큰 부자ㄷㄷ\n",
    ...
    fn builds_rebuilds_and_serializes_style_profile_from_eligible_rows() {
        ...
        let rows = conn
            .prepare(
                "SELECT source_row, content_kind, style_eligible
                 FROM owner_style WHERE source = ?1 ORDER BY source_row",
            )
            .unwrap()
        ...
        assert_eq!(
            open_db(&db)
                .unwrap()
                .query_row(
                    "SELECT COUNT(*) FROM owner_style_profile
                     WHERE chat = ?1 AND source = ?2",
                    params!["프로필방", source],
                    |row| row.get::<_, i64>(0),
                )
    ...
    fn live_ingest_does_not_treat_a_nickname_collision_as_self() {
        ...
        assert_eq!(
            conn.query_row(
                "SELECT COUNT(*) FROM owner_style WHERE source=?1",
                [&result.source],
                |row| row.get::<_, i64>(0),
            )
            .unwrap(),

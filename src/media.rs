...
const MAX_JPEG_TRAILING_BYTES: usize = 8 * 1024;
...
/// KakaoTalk ORs extra flags onto NTChatMessage.type (for example 16386 =
/// photo `2 | 0x4000`). Image download and AutoReply routing use the low 8
/// bits, which keep 2/14/27 while leaving flagged text (`1 | 0x4000`) alone.
pub fn kakao_still_image_kind(message_type: i32) -> Option<i32> {
    let kind = message_type & 0xff;
    matches!(kind, 2 | 14 | 27).then_some(kind)
}
...
pub fn parse_image_download_sources(
    ...
    let message_type = kakao_still_image_kind(message_type)
        .ok_or_else(|| anyhow::anyhow!("Message type is not a supported image attachment"))?;
    match message_type {
        2 | 14 => {
...
pub fn media_extension(msg_type: i32) -> &'static str {
    match kakao_still_image_kind(msg_type).unwrap_or(msg_type) {
        2 | 27 => "jpg",
        3 => "mp4",
        12 => "m4a",
        14 => "gif",
...
fn attested_image_bytes_for_decode<'a>(
    ...
    // Kakao photo rows often omit `mt` and a locator extension. JPEG SOI plus
    // an attested byte length is enough to strip a bounded post-EOI trailer.
    let allows_jpeg_trailer = match expected_media_type {
        None | Some("jpeg") => true,
        Some(_) => false,
    };
    if declared_size == Some(original_size) && allows_jpeg_trailer {
        if let Some(jpeg) = jpeg_prefix_with_exact_sef_trailer(data) {
            return Ok(jpeg);
        }
        if let Some(jpeg) = jpeg_prefix_with_bounded_trailer(data) {
            return Ok(jpeg);
        }
    }
...
/// Keep a complete JPEG when Kakao (or a CDN) appends a short opaque trailer
/// after a unique-or-final EOI. Missing EOI is never synthesized.
fn jpeg_prefix_with_bounded_trailer(data: &[u8]) -> Option<&[u8]> {
    if !data.starts_with(&JPEG_START_OF_IMAGE) {
        return None;
    }
    let eoi_offset = data
        .windows(JPEG_END_OF_IMAGE.len())
        .rposition(|window| window == JPEG_END_OF_IMAGE)?;
    let eoi_end = eoi_offset.checked_add(JPEG_END_OF_IMAGE.len())?;
    let trailer = data.len().checked_sub(eoi_end)?;
    (trailer > 0 && trailer <= MAX_JPEG_TRAILING_BYTES).then(|| &data[..eoi_end])
}
...
pub fn normalize_downloaded_image(
    ...
    if kakao_still_image_kind(message_type).unwrap_or(message_type) != 14 && media_type == "gif" {
...
mod tests {
    ...
    #[test]
    fn still_image_kind_masks_kakao_photo_flag_bits() {
        assert_eq!(kakao_still_image_kind(2), Some(2));
        assert_eq!(kakao_still_image_kind(16386), Some(2));
        assert_eq!(kakao_still_image_kind(14), Some(14));
        assert_eq!(kakao_still_image_kind(27), Some(27));
        assert_eq!(kakao_still_image_kind(1), None);
        assert_eq!(kakao_still_image_kind(16385), None);
        let flagged = parse_image_download_sources(
            r#"{
                "url":"https://talk.kakaocdn.net/dn/photo.png?token=signed",
                "k":"safe/photo.png",
                "s":28428,
                "w":456,
                "h":613,
        ...
        .expect("flagged photo attachment");
        assert_eq!(flagged.len(), 1);
        assert_eq!(flagged[0].expected_media_type.as_deref(), Some("png"));
    }
    ...
    #[test]
    fn bounded_trailing_data_after_eoi_is_stripped() {
        let encoded = encoded_test_jpeg(3, 2);
        ...
        .expect("short opaque trailers after EOI are Kakao padding");
        ...
        let mut partial_marker = encoded.clone();
        partial_marker.pop();
    ...
    #[test]
    fn kakao_padded_sef_trailer_that_does_not_end_with_seft_is_stripped() {
        // Live kakao-test photo 3916098366958290944: Image_UTC_Data / SEFH / SEFT
        // plus trailing NUL padding so the file does not end on SEFT.
        let encoded = encoded_test_jpeg(3, 2);
        let trailer = hex::decode(
            "",
        )
        .expect("live Kakao trailer fixture");
        assert_eq!(trailer.len(), 382);
        assert_ne!(&trailer[trailer.len() - 4..], b"SEFT");

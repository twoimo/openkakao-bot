...
use crate::logging::{FlowKind, HistoryStore, PipelineEvent, Stage, StageStatus, StoreError};
use crate::safety::{FenceReason, SendDecision};

pub const WATCH_SERVICE_LABEL: &str = "com.openkakao.auto-reply.watch";
pub const HEALTH_SERVICE_LABEL: &str = "com.openkakao.auto-reply.health";
...
pub fn default_state_root(home: &Path) -> PathBuf {
    let parent = home
        ...
        .join("openkakao");
    let modern = parent.join("auto-reply");
    let legacy = parent.join("bujamentor");
...
// ===========================================================================
// Automatic-reply pipeline instrumentation (R2.1, R2.2, R3.11)
// ===========================================================================
//
// The live per-message reply loop is driven by the spawned workers, but every
// decision-bearing stage funnels through the Rust core. This section gives the
// runtime a single, redaction-safe way to emit a [`PipelineEvent`] at each
// stage so the history window (R2) can show the whole flow in order with a
// status per stage.
//
// Two flows are instrumented:
//
// * The automatic-reply flow, in stage order
//   Detect -> Authorize -> Retrieve -> Model -> Schedule -> PreSend -> Commit
//   (R2.1).
// * The GeekNews immediate-send flow, in stage order
//   Detect -> Authorize -> PreSend -> Commit (R2.2).
//
// The owner-exclusion point (R3.11) is made explicit: when 최연우's own
// self-authored outgoing messages are dropped from the reply-target set at
// detection, an [`OWNER_SELF_EXCLUDED_CODE`] event is recorded so the exclusion
// is visible in the journal, not merely implied.
//
// Every event goes through [`HistoryStore::append`], which redacts and
// validates before any write, so instrumentation can never leak a raw body,
// prompt, URL, or absolute path (R2.8, R11.8). The [`ObservedMessage`] the
// selection works over carries no chat body for the same reason.

/// Ordered stages of the automatic-reply pipeline, as shown in the history
/// window (R2.1).
pub const AUTO_REPLY_STAGES: [Stage; 7] = [
    Stage::Detect,
    Stage::Authorize,
    Stage::Retrieve,
    Stage::Model,
...
/// Ordered stages of the GeekNews immediate-send pipeline (R2.2).
pub const GEEKNEWS_STAGES: [Stage; 4] = [
    Stage::Detect,
    Stage::Authorize,
    Stage::PreSend,
    Stage::Commit,
...
/// Result code recorded at [`Stage::Detect`] when the owner's own
/// (self-authored) messages are excluded from the reply-target set (R3.11).
pub const OWNER_SELF_EXCLUDED_CODE: &str = "owner_self_excluded";

/// Result code recorded at [`Stage::Detect`] once a batch has been detected.
pub const DETECTED_CODE: &str = "detected";

/// Result code recorded when the safety gate authorizes a send.
pub const AUTHORIZED_CODE: &str = "authorized";

/// A message observed on the live feed, reduced to only the fields the
/// reply-target decision needs.
///
/// It intentionally carries no chat body, sender name, or prompt, so
/// instrumenting the pipeline over these values can never leak content into a
/// log record (R2.8, R11.8).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct ObservedMessage {
    /// Chat the message belongs to.
    pub chat_id: i64,
    /// Stable local-DB log id of the message.
    pub log_id: i64,
    /// Numeric author id from the local DB. Compared against the
    /// database-authoritative account user id to detect the owner.
    pub author_id: i64,
}

/// Select the eligible reply targets from an observed message stream, given the
/// database-authoritative account user id.
///
/// The owner's own (self-authored) messages are **never** included in the
/// returned set (Correctness Property 2, R3.11); upstream they are retained
/// only for context and style learning. Self-authorship uses the numeric-only
/// proof in [`crate::local_db::is_self_author`] — a display name or a zero
/// author id can never mark a message as the owner's, and a non-positive
/// account user id (identity unknown) excludes nothing.
///
/// Order is preserved: the result is the input with self-authored messages
/// removed.
pub fn select_reply_targets(
    ...
    messages
        .iter()
        .copied()
        .filter(|message| !crate::local_db::is_self_author(message.author_id, account_user_id))
        .collect()
}

/// Current wall-clock time in Unix epoch milliseconds, for a recorded event's
/// `at` field.
fn now_epoch_ms() -> i64 {
    Utc::now().timestamp_millis()
}

/// Map a safety-gate decision to the status and short result code recorded for
/// an authorize / pre-send stage. A fenced decision is a failed stage carrying
/// the specific fence reason, so the history window can explain why a send was
/// held back without exposing any sensitive detail.
fn decision_stage_outcome(decision: &SendDecision) -> (StageStatus, &'static str) {
    match decision {
        SendDecision::Allow => (StageStatus::Success, AUTHORIZED_CODE),
        SendDecision::Fenced(reason) => (StageStatus::Failed, fence_reason_code(reason)),
    }
}

/// A short, stable machine code for each fence reason (no sensitive content).
fn fence_reason_code(reason: &FenceReason) -> &'static str {
    match reason {
        FenceReason::AxSendDisabled => "fenced_ax_send_disabled",
        FenceReason::AutoReplyDisabled => "fenced_auto_reply_disabled",
        FenceReason::ChatNotAllowlisted => "fenced_chat_not_allowlisted",
        FenceReason::IdentityMismatch => "fenced_identity_mismatch",
        FenceReason::StateMismatch => "fenced_state_mismatch",
    }
}

/// Records ordered [`PipelineEvent`]s for one processing flow into a
/// [`HistoryStore`].
///
/// A recorder is bound to a single flow ([`FlowKind`]) and `trace_id`, so every
/// event it emits groups together in the history window. Each `record*` call
/// appends one stage event; the store performs redaction validation before any
/// write, so a recorder can never persist sensitive content.
pub struct FlowRecorder<'a> {
    store: &'a dyn HistoryStore,
    flow: FlowKind,
    trace_id: String,
}

impl<'a> FlowRecorder<'a> {
    /// Bind a recorder to a store, flow, and trace id.
    pub fn new(store: &'a dyn HistoryStore, flow: FlowKind, trace_id: impl Into<String>) -> Self {
        Self {
            store,
            flow,
    ...
    /// A recorder for the automatic-reply flow.
    pub fn auto_reply(store: &'a dyn HistoryStore, trace_id: impl Into<String>) -> Self {
        Self::new(store, FlowKind::AutoReply, trace_id)
    }

    /// A recorder for the GeekNews immediate-send flow.
    pub fn geeknews(store: &'a dyn HistoryStore, trace_id: impl Into<String>) -> Self {
        Self::new(store, FlowKind::GeekNews, trace_id)
    }

    /// Emit one stage event. `result_code` must be a short machine token; the
    /// store refuses (fail-closed) anything that looks like a URL, absolute
    /// path, or raw body / prompt.
    pub fn record(
        &self,
        stage: Stage,
        status: StageStatus,
        ...
        self.store.append(PipelineEvent {
    ...
    /// Record the Detect stage and select the reply-target set, emitting an
    /// explicit [`OWNER_SELF_EXCLUDED_CODE`] event whenever 최연우's own
    /// self-authored messages are dropped from that set (R3.11).
    ///
    /// Returns the reply-target set (the observed stream with the owner's own
    /// messages removed). The exclusion is always logged as a distinct event so
    /// it is visible in the history window, never merely implied.
    pub fn detect_reply_targets(
        &self,
        account_user_id: i64,
    ...
    /// Record the Retrieve stage (context and style lookup).
    pub fn retrieve(
        &self,
        status: StageStatus,
        result_code: &str,
        ...
        self.record(Stage::Retrieve, status, result_code, duration_ms)
    }

    /// Record the Model stage (reply generation).
    pub fn model(
        &self,
        status: StageStatus,
        result_code: &str,
        ...
        self.record(Stage::Model, status, result_code, duration_ms)
    }

    /// Record the Schedule stage (delayed-send reservation).
    pub fn schedule(
        &self,
        status: StageStatus,
        result_code: &str,
        ...
        self.record(Stage::Schedule, status, result_code, duration_ms)
    }
    ...
    /// Record the Commit stage (the send was committed, or failed to commit).
    pub fn commit(
        &self,
        status: StageStatus,
        result_code: &str,
        ...
        self.record(Stage::Commit, status, result_code, duration_ms)
    }
}
...
mod instrumentation_tests {
    ...
    fn msg(chat_id: i64, log_id: i64, author_id: i64) -> ObservedMessage {
        ObservedMessage {
            chat_id,
            log_id,
            author_id,
    ...
    #[test]
    fn select_reply_targets_drops_owner_and_keeps_others_in_order() {
        let account = 900;
        let stream = [
            msg(1, 10, 501), // partner
            msg(1, 11, 900), // owner (self) -> excluded
            msg(1, 12, 502), // partner
            msg(1, 13, 900), // owner (self) -> excluded
        ];
        let targets = select_reply_targets(account, &stream);
        assert_eq!(targets, vec![msg(1, 10, 501), msg(1, 12, 502)]);
        // No target may be self-authored.
        assert!(targets
            .iter()
            .all(|m| !crate::local_db::is_self_author(m.author_id, account)));
    }

    #[test]
    fn select_reply_targets_with_unknown_account_excludes_nothing() {
        // A non-positive account user id means identity is unknown; the
        // numeric-only self proof excludes nothing.
        let stream = [msg(1, 10, 0), msg(1, 11, 900)];
        assert_eq!(select_reply_targets(0, &stream), stream.to_vec());
    }

    #[test]
    fn detect_reply_targets_emits_owner_exclusion_event() {
        let store = SqliteHistoryStore::open_in_memory().expect("open");
        let recorder = FlowRecorder::auto_reply(&store, "trace-detect");
        let stream = [msg(1, 10, 501), msg(1, 11, 900)];
        let targets = recorder
            .detect_reply_targets(900, &stream, 5)
            .expect("detect");
        assert_eq!(targets, vec![msg(1, 10, 501)]);

        let events = store.recent(10).expect("recent");
        let codes: Vec<&str> = events.iter().map(|e| e.result_code.as_str()).collect();
        assert!(codes.contains(&OWNER_SELF_EXCLUDED_CODE));
        assert!(codes.contains(&DETECTED_CODE));
        // Every detect event belongs to the auto-reply flow at the Detect stage.
        assert!(events
            .iter()
            .all(|e| e.flow == FlowKind::AutoReply && e.stage == Stage::Detect));
    }

    #[test]
    fn detect_reply_targets_without_owner_emits_no_exclusion() {
        let store = SqliteHistoryStore::open_in_memory().expect("open");
        let recorder = FlowRecorder::auto_reply(&store, "trace-noexcl");
        let stream = [msg(1, 10, 501), msg(1, 12, 502)];
        recorder
            .detect_reply_targets(900, &stream, 0)
            .expect("detect");
        let events = store.recent(10).expect("recent");
        assert!(events
            .iter()
            .all(|e| e.result_code != OWNER_SELF_EXCLUDED_CODE));
    }

    #[test]
    fn auto_reply_flow_records_seven_stages_in_order() {
        let store = SqliteHistoryStore::open_in_memory().expect("open");
        let recorder = FlowRecorder::auto_reply(&store, "trace-full");
        recorder
            .detect_reply_targets(900, &[msg(1, 10, 501)], 1)
            .expect("detect");
        recorder
            .authorize(&SendDecision::Allow, 1)
            .expect("authorize");
        recorder
            .retrieve(StageStatus::Success, "context_ready", 2)
            .expect("retrieve");
        recorder
            .model(StageStatus::Success, "reply_generated", 3)
            .expect("model");
        recorder
            .schedule(StageStatus::Success, "delayed", 0)
            .expect("schedule");
        recorder
            .pre_send(&SendDecision::Allow, 1)
            .expect("pre_send");
        recorder
            .commit(StageStatus::Success, "sent", 4)
            .expect("commit");

        // recent() is newest-first; reverse to compare against emission order.
        let mut events = store.recent(100).expect("recent");
        events.reverse();
        let stages: Vec<Stage> = events
            .iter()
            .filter(|e| e.result_code != OWNER_SELF_EXCLUDED_CODE)
            .map(|e| e.stage)
            .collect();
        assert_eq!(stages, AUTO_REPLY_STAGES.to_vec());
    }

    #[test]
    fn geeknews_flow_records_four_stages() {
        let store = SqliteHistoryStore::open_in_memory().expect("open");
        let recorder = FlowRecorder::geeknews(&store, "trace-geeknews");
        recorder
            .record(Stage::Detect, StageStatus::Success, DETECTED_CODE, 0)
            .expect("detect");
        recorder
            .authorize(&SendDecision::Allow, 0)
            .expect("authorize");
        recorder
            .pre_send(&SendDecision::Allow, 0)
            .expect("pre_send");
        recorder
            .commit(StageStatus::Success, "posted", 0)
            .expect("commit");

        let mut events = store.recent(100).expect("recent");
        events.reverse();
        let stages: Vec<Stage> = events.iter().map(|e| e.stage).collect();
        assert_eq!(stages, GEEKNEWS_STAGES.to_vec());
        assert!(events.iter().all(|e| e.flow == FlowKind::GeekNews));
    }

    #[test]
    fn fenced_decision_records_failed_stage_with_reason() {
        let store = SqliteHistoryStore::open_in_memory().expect("open");
        ...
        assert_eq!(events[0].result_code, "fenced_chat_not_allowlisted");
    }
}

...
fn cmd_download_local(
    ...
    let message_type = crate::media::kakao_still_image_kind(attachment.message_type)
        .ok_or_else(|| anyhow::anyhow!("Message type is not a supported image attachment"))?;
    let sources = parse_image_download_sources(&attachment.attachment, message_type)?;
    ...
    if json {
        crate::util::output_json(&serde_json::json!({
            ...
            "path": path_strings[0],
            "paths": path_strings,
            "media_type": crate::util::message_type_label(message_type),
            "size": total_bytes,

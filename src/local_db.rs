    ...
    for selector in selectors {
        let chat = match selector {
            ChatSelector::Id(id) => {
                ...
                // An unnamed group room has no deterministic AX window title,
                // so an ID-only selector cannot be attested against the open
                // window. Require an explicit binding for those rooms.
                if chat.chat_type == 1 && chat.chat_name.is_empty() {
                    anyhow::bail!(
                        "chat ID {id} is an unnamed group room; use bind:{id}:<AX name> to attest its exact window"
                    );
                }
                // A shared AX name would make window-title attestation ambiguous.
                if !chat.chat_name.is_empty() {
                    if let Some(ids) = by_name.get(chat.chat_name.as_str()) {
                        if ids.len() != 1 || ids[0] != *id {
                            anyhow::bail!(
                                "chat ID {id} shares its AX name {:?} with other local chats: {ids:?}",
                                chat.chat_name
                            );
                        }
                    }
...
fn get_user_id_from_plist_with_mode(mode: IdentityCacheMode) -> Result<i64> {
    ...
    // Fast path: the canonical plist name is deterministic. Directory enumeration
    // can stall while the sandbox container is unhealthy, but lookup-by-name and
    // parse keep working.
    let canonical_plist = container_prefs.join("com.kakao.KakaoTalkMac.plist");
    if canonical_plist.is_file() {
        plist_paths.push(canonical_plist);
    }
    ...
    // Last-resort fallback: trust the uid-0600 identity cache when it matches
...
fn find_database_path(db_name: &str) -> Result<PathBuf> {
    ...
    // Fast path: the database file name is deterministic, so open it directly.
    // Directory enumeration can stall while the sandbox container is unhealthy,
    // but lookup-by-name keeps working.
    let direct_path = container_dir.join(db_name);
    if let Ok(metadata) = std::fs::symlink_metadata(&direct_path) {
        let under_root = direct_path
            .canonicalize()
        ...
        #[cfg(not(unix))]
        if metadata.file_type().is_file() && under_root {
            return Ok(direct_path);
        }
    }
        ...
        .query_row(
            ...
            |row| {
                Ok(LocalMediaAttachment {
                    ...
                    is_self: is_self_author(row.get::<_, i64>(2).unwrap_or(0), account_user_id),
...
mod tests {
    ...
    #[test]
    fn is_self_author_ignores_zero_ids_and_display_names() {
        assert!(is_self_author(900, 900));
        assert!(!is_self_author(0, 900));

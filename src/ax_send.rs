...
fn deleted_control_log_id(message: &crate::local_db::LocalMessage) -> Option<i64> {
    ...
    // Bot/feed cards emit hidden:true with feedType 25 while the bubble stays
    // visible in the already-open AX window. Tombstoning that log ID drops
    // the still-rendered row from the local suffix and breaks attestation.
    // feedType 14 is the real AX-deleted leftover path and must still hide.
    if parsed.get("feedType").and_then(serde_json::Value::as_i64) == Some(25) {
        return None;
    }
...
fn normalize_local_sharp_search_binding(attachment: &str) -> anyhow::Result<String> {
    ...
    let feed_body = content
        ...
        .map(str::trim)
        .filter(|value| !value.is_empty());
    if let Some(body) = feed_body {
        return Ok(normalize_binding_message(body));
    }
...
pub fn normalize_local_binding_suffix(
    ...
    for message in messages {
        ...
        match local_binding_token(message) {
            ...
            Ok(_) => {}
            Err(_) if message.message_type == 71 => continue,
            Err(_) => suffix.clear(),
        }
    }
    suffix
...
fn transcript_truncated_prefix_matches(ax: &str, local: &str, min_prefix_bytes: usize) -> bool {
    if let Some(prefix) = truncated_prefix(ax) {
        return prefix.len() >= min_prefix_bytes
            && local.len() > prefix.len()
            && local.starts_with(prefix);
    }
    // KakaoTalk often clips a long bubble without an ellipsis. A long common
    // prefix still attests the same already-open room.
    if ax.len() >= min_prefix_bytes
        && local.len() > ax.len()
        && local.starts_with(ax)
        && ax.chars().count() >= 24
    {
        return true;
    }
...
fn is_geeknews_composer_draft(value: &str) -> bool {
    value.trim_start().starts_with("GeekNews TOP5")
}
const COMPOSER_ALLOWLIST_ENV: &str = "OPENKAKAO_COMPOSER_ALLOWLIST";

/// Texts this same pipeline previously staged but could not confirm. A
/// composer still holding one of these is our own leftover, safe to replace;
/// anything else is treated as a human draft and never touched.
fn permitted_leftover_drafts() -> Vec<String> {
    let Ok(path) = std::env::var(COMPOSER_ALLOWLIST_ENV) else {
        return Vec::new();
    };
...
fn is_permitted_leftover(value: &str, message: &str) -> bool {
    if is_geeknews_composer_draft(value) && is_geeknews_composer_draft(message) {
        return true;
    }
    permitted_leftover_drafts()
        .iter()
        .any(|draft| composer_equivalent(draft, value))
}
...
/// Prefer KakaoTalk's 전송 button. Return is the fallback when the button
/// is missing or the found button fails to press. A successful 전송 press
/// still must not also press Return.
    ...
    Escalate: FnMut() -> anyhow::Result<()>,
{
    ...
    match initial.as_deref() {
        ...
        Some(value) if is_permitted_leftover(value, message) => {
            // A previous pipeline draft can sit unsent after
            // accepted_unconfirmed. Replace only that leftover with the
            // current outbound text, never a human draft.
        }
...
mod match_tests {
    ...
    #[test]
    fn transcript_match_accepts_long_clipped_latest_endpoint_without_ellipsis() {
        let prefix = "가".repeat(86);
        assert!(prefix.len() >= BOUND_TRANSCRIPT_MIN_TRUNCATED_PREFIX_UTF8_BYTES);
        let older = [
            "서로 다른 첫 번째 메시지입니다".to_string(),
        ...
        let ax = [older[0].clone(), older[1].clone(), prefix.clone()];
        let local = [
            older[0].clone(),
            older[1].clone(),
            format!("{prefix} 뒤에 남아 있는 원문"),
        ];
        let matched = match_transcript_suffix(&ax, &local);
        assert_eq!(matched.matched_count, 3);
        assert!(matched.is_strong());
    ...
    #[test]
    fn unparseable_sharp_search_does_not_wipe_local_suffix() {
        let mut older = local_message(1, "첫번째 문장입니다", String::new());
        older.log_id = 1;
        let mut search = local_message(71, "샵검색: #쿠팡", "{not-json".to_string());
        search.log_id = 2;
        let mut later = local_message(1, "두번째 문장입니다", String::new());
        later.log_id = 3;
        let suffix = normalize_local_binding_suffix(&[older, search, later]);
        assert_eq!(
            suffix.iter().map(|(log_id, _)| *log_id).collect::<Vec<_>>(),
            vec![1, 3]
        );
        ...
        let matched = match_transcript_suffix(&ax, &texts);
        assert_eq!(matched.matched_count, 2);
        assert!(matched.is_strong());
    }
    #[test]
    fn sharp_search_feed_body_binds_when_title_empty() {
        let attachment = serde_json::json!({
            "C": {
                "THC": 1,
                "TI": {
        ...
        .to_string();
        let card = local_message(71, "CPNG, 쿠팡 (미국, 뉴욕)", attachment);
        assert_eq!(
            normalize_local_binding_message(&card).unwrap(),
            "CPNG, 쿠팡 (미국, 뉴욕)\n\n정규장(장마감) 8/25 05:00"
        );
    }
    #[test]
    fn feed_hidden_control_keeps_visible_local_row_in_suffix() {
        let mut first = local_message(1, "쿠팡 진입 너무 좋은데", String::new());
        first.log_id = 1;
        let mut visible = local_message(1, "쫌 해당 가격 오래 유지되면 좋겠음 ㄷㄷ", String::new());
        visible.log_id = 2;
        let mut latest = local_message(1, "ㅋㅋㅋㅋㅋㅋ과연 얼마나 유지할라나", String::new());
        latest.log_id = 3;
        let mut hide = local_message(
            0,
            r#"{"logId":2,"targetRevision":1,"hidden":true,"feedType":25}"#,
            String::new(),
        );
        hide.log_id = 4;
        let suffix = normalize_local_binding_suffix(&[first, visible, latest, hide]);
        assert_eq!(
            suffix.iter().map(|(log_id, _)| *log_id).collect::<Vec<_>>(),
            vec![1, 2, 3]
        );
        let texts = suffix
            ...
            .collect::<Vec<_>>();
        let ax = [
            "쿠팡 진입 너무 좋은데".to_string(),
            "쫌 해당 가격 오래 유지되면 좋겠음 ㄷㄷ".to_string(),
            "ㅋㅋㅋㅋㅋㅋ과연 얼마나 유지할라나".to_string(),
        ];
        let matched = match_transcript_suffix(&ax, &texts);
        assert_eq!(matched.matched_count, 3);
        assert!(matched.is_strong());
    ...
    fn url_scrap_card_binds_title_so_file_tail_can_attest() {
        ...
        let aliases: Vec<_> = local_binding_token(&card)
            .unwrap()
            .candidates()
            .map(str::to_string)
            .collect();
        assert!(aliases.contains(&card.message));
    ...
    fn kakao_send_button_labels_match_korean_and_english() {
        ...
        assert!(is_kakao_send_button_label("보내기"));
        assert!(is_kakao_send_button_label("전송 버튼"));
    ...
    #[test]
    fn submit_falls_back_to_return_after_a_failed_send_button() {
        use std::cell::Cell;
        let returns = Cell::new(0);
        submit_composer_once(
        ...
        .expect("an inert 전송 button must fall back to Return");
        assert_eq!(returns.get(), 1);
    }
...
mod imp {
    ...
    /// Bring the target chat window to the front of KakaoTalk so pid-level
    /// keyboard events (Return) land in THIS room, not whichever other chat
    /// window happens to be key when multiple rooms are open.
    fn raise_chat_window(window: &AXUIElement) -> Result<()> {
        window

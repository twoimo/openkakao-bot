# AutoReply launchd supervision

**Do not use watch/health (`com.openkakao.auto-reply.watch` / `.health`) as a reply owner.** The current reply-owner layout is session-monitor only. `scripts/status-auto-reply-launchd.sh` is named after the old pair but is the **current** status probe and must print `service_kind=terminal_monitor`. `scripts/install-auto-reply-service.sh` is **non-current**.

### Artifact status

| Artifact | Status |
|---|---|
| `scripts/auto-reply-session-monitor.py` | **current** host |
| `scripts/prepare-auto-reply-session-runtime.py` | **current** bake |
| `scripts/auto-reply-service.py` | **current** Terminal session entry |
| `scripts/auto-reply-supervisor.py` | **current** |
| `scripts/auto-reply-db-watch.py` | **current** |
| `scripts/auto-reply-worker.py` | **current** |
| `scripts/auto_reply_transition_journal.py` | **current** |
| `scripts/auto_reply_ax_ui.py` | **current** |
| `scripts/auto_reply_metrics.py` | **current** |
| `scripts/auto-reply-tui.py` | **current** operator UI |
| `scripts/uninstall-auto-reply-session-monitor.sh` | **current companion** |
| `scripts/auto-reply-schema.json` | **current** bake data asset |
| `scripts/status-auto-reply-launchd.sh` | **current** despite the name; prints `service_kind=terminal_monitor` |
| `scripts/status-auto-reply-service.sh` | **non-current** companion of the old direct installer |
| `scripts/install-auto-reply-service.sh` | **non-current** |
| `scripts/uninstall-auto-reply-service.sh` | **non-current** companion |
| `scripts/install-auto-reply-launchd.sh` | **legacy** watch/health installer — not a reply owner |
| `scripts/uninstall-auto-reply-launchd.sh` | **legacy** |
| `scripts/doctor-auto-reply-launchd.sh` | **legacy-observation** |
| `scripts/test-auto-reply-launchd-artifacts.sh` | **legacy test helper** |
| `scripts/verify-launchd-watch.sh` | **legacy-watch-adjacent** |
| `scripts/auto-reply-apple-watch.py` | **legacy-observation**; not a reply owner |
| `src/bin/openkakao-auto-reply-health.rs` | **legacy-observation** binary; keep; not a reply owner |
| `src/auto_reply_service.rs` LaunchAgent labels | **legacy identity** |
| `src/auto_reply_service.rs` status/log/hook helpers | **current shared** (`ax-watch`, health binary) |
...
`com.openkakao.auto-reply.session-monitor`. It is a one-shot process limited to
...
starts `auto-reply-service.py --mode session` in Terminal with
...
state_root = "/Users/me/Library/Application Support/openkakao/auto-reply"
...
The `[auto_reply.room_reply_authors]` keys are canonical positive chat IDs. An
...
exec '/absolute/path/to/python3' -E -B -S \
  '/absolute/private/runtime/auto-reply-service.py' \
  --mode session \
  --python '/absolute/path/to/python3' \
  --entry '/absolute/private/runtime/auto-reply-service.py' \
  --bin '/absolute/private/runtime/openkakao-cli' \
  --config '/Users/me/.config/openkakao/config.toml' \
  --chat 'bind:417780809780519:부자멘토멘티' \
  --state-root '/Users/me/Library/Application Support/openkakao/auto-reply' \
  </dev/null \
  >>'/Users/me/Library/Application Support/openkakao/auto-reply/session-service/watchdog.out.log' \
  2>>'/Users/me/Library/Application Support/openkakao/auto-reply/session-service/watchdog.err.log'
```
...
```json
{
  ...
  "state_root": "/Users/me/Library/Application Support/openkakao/auto-reply",
  "command": {
    "path": "/absolute/private/runtime/start-auto-reply-session.command",
...
`auto-reply-session-monitor.py`, `--manifest <absolute-manifest>`, and
...
```bash
launchctl print \
  "gui/$(id -u)/com.openkakao.auto-reply.session-monitor"

jq '{state,reason,updated_at_unix_ns}' \
  "$HOME/Library/Application Support/openkakao/auto-reply/session-monitor-status.json"
jq '{mode,state,attempt,restart_count,consecutive_failures,updated_at_unix_ns}' \
  "$HOME/Library/Application Support/openkakao/auto-reply/session-watchdog-status.json"
jq '{readiness,mode,target_chat_id,fence_reason,readiness_reasons}' \
  "$HOME/Library/Application Support/openkakao/auto-reply/rooms/417780809780519/supervisor-status.json"
```
...
`prepare-auto-reply-session-runtime.py` packages this layout without activating
...
```bash
python3 scripts/prepare-auto-reply-session-runtime.py \
...
- `start-auto-reply-session.command`, the static multi-room watchdog command;
- `open-auto-reply-tui.command`, the content-redacted dashboard launcher;
- `session-monitor-manifest.json`, which pins the watchdog command digest; and
- a candidate `com.openkakao.auto-reply.session-monitor.plist`.
...
```bash
python3 scripts/auto-reply-tui.py \
  --state-root "$HOME/Library/Application Support/openkakao/auto-reply" \
  --room 417780809780519 \
  --room 123456789

# Scriptable, content-redacted snapshots
python3 scripts/auto-reply-tui.py --once
python3 scripts/auto-reply-tui.py --once --json
```
...
run `scripts/uninstall-auto-reply-session-monitor.sh`. That helper creates the
...
`install-auto-reply-service.sh --mode production` installs the
older direct service label `com.openkakao.auto-reply.autoreply`; it does not
...
- `com.openkakao.auto-reply.watch`
- `com.openkakao.auto-reply.health`
...
sh scripts/install-auto-reply-launchd.sh \
  ...
  --health-bin /absolute/path/to/openkakao-auto-reply-health \
  [--state-root /absolute/path/to/state-root]
```

Production mode keeps the same service-mode watcher argv and adds the fixed unattended flags, but the AutoReply supervisor still enforces the independent `OPENKAKAO_AUTO_REPLY_ENABLED=1` gate only after a healthy local DB preflight. It passes a direct `--hook-path` only:
...
```text
$HOME/Library/Application Support/openkakao/auto-reply
```
...
```bash
sh scripts/status-auto-reply-launchd.sh
...
`[auto_reply].state_root` from the CLI configuration.

```bash
sh scripts/uninstall-auto-reply-launchd.sh --purge-state [--state-root /absolute/path/to/state-root]
```

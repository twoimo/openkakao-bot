...
### Fixed
- DB 감시자가 빈 본문인 이모티콘 메시지 유형 12·20·22를 `[이모티콘]`으로 정규화해, 빈 입력으로 커서만 승인하고 답변 큐에 남기지 않던 누락을 막습니다.
- Astra HEAD 재평가 지적(P2 4건)을 반영했습니다: (1) 재랭커에서 `lenient_policy`로 선택된 초안이 예약 작업 전송 시 엄격 정책에 걸려 스킵되던 불일치를 해소하고 실제 거부 사유 코드(`policy_{reason}`)를 기록, (2) 학습된 soft style tell 캐시가 파일 mtime 변경 없이도 만료 시점(`expires_at`) 도달 시 자동으로 무효화되도록 수정, (3) Rust 컨텍스트 DB `reply_decisions`의 `evidence_json`에 Python의 `provenance`(재랭킹 결정 및 응답시간 성분 선택 근거)를 온전히 보존하도록 확장, (4) 프롬프트 예산 적용 전(`retrieved_evidence_ids`)과 적용 후(`prompt_evidence_ids`)의 근거 개수를 분리 계측하여 예산 절감량이 투명하게 드러나도록 수정.
...
- 메뉴바 앱이 실행 직후 죽던 문제를 고쳤습니다. 빌드 산출물을 `/Applications`로 복사할 때 **`com.apple.quarantine`이 붙어 macOS가 앱을 임의 경로(translocation)로 옮겨 실행**했고, 그 상태에서 dyld가 `unknown imports format`으로 중단(SIGABRT)했습니다. 설치 스크립트(`scripts/install-auto-reply-menubar.sh`)가 복사 직후 격리 속성을 지우고 실행합니다.
- 모델 변경마다 **토큰**을 부여해, 예약된 재확인(20·40·60초)이 그 사이에 시작된 새 변경을 덮거나 공통 잠금을 풀지 못하게 했습니다(Astra HEAD 0.7875 P0). 새 변경이 생기면 이전 재확인은 조용히 종료합니다.
- 이미지 모델 변경 성공 문구를 **"저장됨 · 다음 답변부터 사용"**으로 구분했습니다. 답변 모델은 독립 재조회로 확인하지만, 이미지 모델은 저장 응답의 일치하는 ID로 확인하므로 확인 근거가 다릅니다(Astra HEAD 지적 3-c).
- 준비 단계 응답의 **`prepared` 값을 실제로 확인**합니다. 저장이 확인됐어도 준비가 확인되지 않으면 "모델 준비를 확인하지 못했습니다. 다음 답변에서 다시 시도합니다."로 표시하고 완료로 말하지 않습니다(Astra HEAD 지적 1-b).
- 저장 응답이 없거나 파싱되지 않을 때 **곧바로 "변경하지 못했습니다"로 단정하지 않습니다.** 저장값을 다시 읽어 확인하고, 확인되지 않으면 "적용 결과를 확인하지 못했습니다" 상태로 두고 **실제로 후속 확인을 예약**합니다(20·40·60초, 총 3회). 확인되면 완료로 전환합니다(Astra HEAD 지적 3-a/3-b).
- 변경이 진행 중일 때 **늦게 도착한 조회 응답이 현재 선택값을 덮지 않습니다**(스냅샷 반영·목록 조회 모두). 또한 적용 완료 시 **확인된 새 선택값을 다시 세팅**해, 화면이 이전 값인데 "적용됨"이 표시되는 충돌을 없앴습니다(Astra HEAD 지적 2-a/2-b).
- oMLX 모델 준비 호출이 실패(연결 오류 등)하면 **`prepared=false`와 경고 문구**로 보고합니다. 이전에는 예외를 삼키고 `prepared=true`를 주장해, 준비가 확인되지 않았는데도 완료로 보였습니다(Astra HEAD 지적 1-a).
- `--no-wait`를 **값 없는 단독 플래그로 보내도 인식**합니다. `_argv_flag_value()`가 마지막 단독 플래그에 빈 문자열을 돌려주어 저장 전용 분기(준비 대기 생략)가 실행되지 않던 버그를 고쳤습니다(Astra HEAD 0.7825 지적).
- 호스트 상태 검사가 **자동답변 방만** 평가합니다. 긱뉴스 전용 방은 워커를 상주시키지 않아 상태 파일이 오래되는데, 이를 호스트 장애로 오탐해 `healthy=false`가 나오던 문제를 고쳤습니다(실측 `healthy=true`). 짧은 준비 리스 전환도 감독관이 running이고 상태가 신선하면 실패로 세지 않습니다.
- 모델 목록 새로 고침이 **성공하면 지난 실패 안내를 지우고**, 응답의 `ok`도 함께 확인합니다(실패 안내가 남아 있던 문제, Astra 5번 지적).
- 자동답변 준비 검사에서 스타일 프로파일·응답시간 통계를 화면 이름으로 찾지 못하면 `그룹:<id>` 자리표시자 이름으로 한 번 더 조회합니다. 이름 없는 그룹방(예: NIMDA)이 "style profile is unavailable"로 막히던 문제를 해결했습니다(실측 `valid=true`).
- 컨텍스트 동기화의 두 번째 신원 검사(수집 단계)에서도 `그룹:<id>` 자리표시자를 같은 방으로 인정합니다. 이 검사 때문에 화면 이름으로는 방을 동기화할 수 없었습니다.
- `auto-reply --check`에 `--chat`을 주면 **명시한 방만** 검사합니다. 이전에는 카탈로그의 다른 방까지 합쳐져, 인증이 되지 않는 방 하나가 모든 방의 검사를 실패시켰습니다(2026-09-12 호스트 정지).
- 로컬 DB에 이름이 없는 그룹방은 `그룹:<id>` 자리표시자로 저장되는데, 화면 이름으로 컨텍스트 동기화/시작을 요청하면 "source identity changed"/"not ready"로 막혔습니다. 같은 방으로 보고 통과시킵니다.
- 호스트 프리플라이트가 모든 방을 떨어뜨릴 때 **방별 사유를 `preflight-skipped.json`에 남깁니다**(이전에는 사유가 사라졌습니다).
- 모델 설정 창의 안내 문구와 역할 설명을 의도한 **12pt**로 맞췄습니다(`Chrome.hint`에 크기 인자 추가, 다른 창은 기존 11pt 유지).
- 이미지 행의 **자동 선택 여부를 코어가 명시적 필드(`auto_selected`)로 전달**합니다. 화면은 문자열 추정 대신 이 값을 쓰고, 값이 없을 때만 예전 추정으로 물러납니다.
- 모델 설정 창을 **행 상태 기계**로 바꿨습니다. `적용 중 / 결과 확인 중 / 적용됨 / 실패`를 상태로 보관하고 화면은 그 상태만 렌더링하며, 저장 → 저장값 재확인 → (준비가 필요한 모델만) 준비 순서로 진행해 확인 전에는 성공을 단정하지 않습니다.
- 적용이 끝나기 전에는 모델 선택 컨트롤을 비활성화해 **중복 변경을 직렬화**합니다.
- 되돌리기는 마지막으로 성공한 변경의 **대상과 이전 값을 하나의 이력**으로 보관하고, 버튼 라벨에 대상을 표시하며, 복구가 확인된 뒤에만 이력을 지웁니다.
- 목록 조회 실패 문구를 **기존 목록이 있을 때/없을 때 두 가지**로 나눴습니다.
...
- 적용 실패 시 **저장된 값도 이전 모델로 되돌립니다**. 코어가 새 모델을 저장한 뒤 준비를 최대 180초 기다리는데 앱은 8초에서 끊어 "화면은 이전, 저장은 새 모델"이 될 수 있었습니다. 호출 제한을 준비 대기에 맞추고, 되돌리기까지 실패하면 "이전 모델을 유지합니다"라고 말하지 않습니다.
- 되돌리기 버튼이 **이미지 모델도 되돌립니다**(이전에는 답변 모델만 처리해 이미지만 바꾼 경우 아무 동작도 하지 않았습니다). 성공한 항목의 이력만 지워 재시도가 가능합니다.
- 코어가 "쓰지 않음"(`enabled=false`, `source=unused`)을 알려 주면 오래된 스냅샷으로 보지 않고 받아들여, 답변 모델이 사진도 보는 상태가 화면에 반영됩니다.
- 모델 목록 새로 고침이 실패하면 **실패 안내를 보여 주고 이전 목록을 유지**하며, 완료 시 열려 있는 설정 창을 같은 목록으로 갱신합니다.
...
- 가져오기 source를 재계산할 때 응답시간 집계뿐 아니라 **말투 프로필도 함께 재생성**합니다. 응답시간만 다시 만들면 프로필이 방금 바뀐 행을 그대로 설명하게 됩니다.
- 생성 영수증의 `provenance.generation`에 검색·프롬프트 근거 수와 생성 시간도 함께 넣습니다. 조기 반환(deferred) 경로는 최상위 분석 키를 복사하지 않아 실패 영수증의 근거 수가 0으로 기록됐습니다.
- 가져오기(CSV/레거시) source의 응답시간 집계도 봇 기여분을 제거해 **재계산**합니다. 그 source는 개별 표본 행을 두지 않고 `response_time_stats`를 직접 쓰기 때문에, 예전에 측정된 봇 응답시간(예: 34초)은 표본 삭제만으로는 지워지지 않고 집계에 남았습니다. 이제 봇 행을 제외하고 source 자체 메시지에서 집계를 다시 만듭니다.
- 영수증이 **생성 당시 실제 모델**을 기록하고, 검색·프롬프트 근거 수를 같은 추출 함수로 셉니다. 이전에는 기록 모델이 실행 기본값이었고, 전(공급 ID 전체)과 후(프롬프트 4개 목록)의 범위가 달라 아무것도 잘리지 않아도 2→1로 보였습니다.
- 창 증거 팩(`scripts/ahp_window_pack.py`)이 모든 자료에 같은 시작·종료 범위를 적용하고, 실행 식별값(런타임·config 해시·PID)·정책·프로필 버전, 방 허용 여부, 주변 대화, source별 프로필·시간 집계, 확정 전송 대조 요약을 함께 냅니다.
- 반응 허용값이 없는 **과거 예약 작업**은 반응 토큰 초안을 보류합니다. 이전에는 값이 없으면 인바운드 토큰만으로 허용해, 질문·작업 제외와 프로필 하한을 우회했습니다(2차 리뷰 B3 잔여). 이제 예약 시점에 기록된 값이 `true`일 때만 허용하고, 없으면 보류합니다.
- 최종 만료 검사가 `response_window_upper_seconds`가 없으면 건너뛰던 것을, 초기 검사와 **같은 유효 윈도**(저장된 윈도, 없으면 기본 300초)로 통일했습니다. 두 검사가 다른 정책을 쓰면 그 사이에서 만료된 초안이 전송됩니다.
- 예약 시점에 계산해 저장한 반응 허용값을 최종 전송 검사에서도 그대로 적용합니다. 이전에는 인바운드에 `ㅋㅋㅋ`/`ㄷㄷ`가 있으면 저장된 `false`를 우회했습니다. 값이 없는 과거 작업만 인바운드 기준으로 재계산합니다.
- 종결어미 판정(`_ending_stem`)이 반응 토큰과 구두점을 한 번씩만 제거해 `그럼 딱 맞겠네 ㅋㅋㅋ!`가 `unmarked`로 분류되고 현준 존댓말 검사를 통과했습니다. 이제 교체가 멈출 때까지 반복 제거합니다.
- 실패·미파싱(deferred)·터미널 스킵 경로에도 같은 영수증을 남기고, 생성 영수증이 `provenance.generation`에만 있을 때도 프롬프트 크기·해시·근거 수를 읽도록 폴백을 추가했습니다. 이전에는 기록상 자료가 없었던 것처럼 보였습니다.
